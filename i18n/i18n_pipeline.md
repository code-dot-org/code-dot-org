# i18n Pipeline Diagrams

+ [i18n Pipeline Diagrams](#i18n-pipeline-diagrams)
  + [i18n level 0](#i18n-level-0)
  + [i18n level 1](#i18n-level-1)
  + [i18n sync-in level 0](#i18n-sync-in-level-0)
  + [i18n sync-in level 1](#i18n-sync-in-level-1)
  + [i18n sync-up](#i18n-sync-up)
  + [i18n sync-down](#i18n-sync-down)
  + [i18n sync-down and sync-out connection](#i18n-sync-down-and-sync-out-connection)
  + [i18n sync-out level 0](#i18n-sync-out-level-0)
  + [i18n sync-out level 1](#i18n-sync-out-level-1)

## i18n level 0

![i18n level 0 diagram](./diagrams/i18n_level_0.svg)

## i18n level 1

![i18n level 0 diagram](./diagrams/i18n_level_1.svg)

## i18n sync-in level 0

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-in level 0 diagram](./diagrams/sync_in_level_0.svg)

## i18n sync-in level 1

+ [Return to the TOC](#i18n-pipeline-diagrams)

- Note: The content of [pegasus/cache/i18n/en-US.yml](https://github.com/code-dot-org/code-dot-org/blob/staging/pegasus/cache/i18n/en-US.yml) is populated from [Pegasus/Data/i18n.gsheet](https://docs.google.com/spreadsheets/d/1Tq7VqZALgRA0wYk0HDfEOTyRI0TM2Dir2rloXIPGCgU/edit ) by [pegasus/rake/i18n.rake](https://github.com/code-dot-org/code-dot-org/blob/staging/pegasus/rake/i18n.rake)

![i18n sync-in level 1 diagram](./diagrams/sync_in_level_1.svg)

## i18n sync-up

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-up diagram](./diagrams/sync_up.svg)

## i18n sync-down

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-down diagram](./diagrams/sync_down.svg)

## i18n sync-down and sync-out connection

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-down and sync-out connection diagram](./diagrams/sync_down_out_connection.svg)

## i18n sync-out level 0

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-out level 0 diagram](./diagrams/sync_out_level_0.svg)

## i18n sync-out level 1

+ [Return to the TOC](#i18n-pipeline-diagrams)

![i18n sync-out level 1 diagram](./diagrams/sync_out_level_1.svg)
