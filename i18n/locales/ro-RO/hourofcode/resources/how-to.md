...
## 1) Priviți acest tutorial video <iframe width="560" height="315" src="//www.youtube.com/embed/tQeSke4hIds" frameborder="0" allowfullscreen></iframe>
...
Cei mai multi dintre copii nu stiu ce reprezinta stiinta calculatorului si programarea. Iata cateva idei:
...
## 6) Începeți-vă Ora de Programare/Ora de cod
...
**Cand elevii intampina dificultati**
...
**Cum pot printa certificatele pentru elevii mei?**

Fiecare elev primeste o sansa de a obtine un certificat prin intermediul e-mail-ului atunci cand termina tutorialele [Code.org ](http://studio.code.org). Poti da click pe certificat pentru a-l tipari. Cu toate acestea, daca va doriti sa faceti certificate noi pentru elevii dumneavostra, vizitati pagina noastra [Certificate](http://<%= codeorg_url() %>/certificates) pentru a printa cate diplome doriti, doar printr-o miscare!

**Ce urmeaza dupa Hour of Code?**

Hour of Code este abia primul pas catre o lunga calatorie pentru a afla mai multe despre cum functioneaza tehnologia si cum se pot crea aplicatiile software. <% if @country == 'uk' %> Hour of Code este abia primul pas al călătoriei pentru a afla mai multe despre modul în care funcționează tehnologia și cum poți crea aplicații software. Pentru ca aceasta calatorie sa continue, [incurajeaza-ti elevii sa invete online](http://uk.code.org/learn/beyond). <% else %> Pentru a continua calatoria, gasiti resurse aditionale pentru educatori [aici](http://<%= codeorg_url() %>/educate). Sau incurajeaza-ti elevii sa invete [online](http://<%= codeorg_url() %>/learn/beyond). <% end %>