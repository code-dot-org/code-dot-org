---
title: Ressourcen
---

{{ signup_button }}

<link rel="stylesheet" type="text/css" href="/css/promote-page.css"></link>

# Machen Sie Hour of Code bekannt

### Hier finden Sie alle Ressourcen, die Sie benötigen, um auf Ihre Hour of Code aufmerksam zu machen. Sie wissen nicht, wo Sie anfangen sollen? Beginnen Sie mit unserem [Leitfaden für eine Veranstaltung von Hour of Code]({{ how_to_url }})

* * *

{{ promote_handouts }} {{ promote_videos }}

<a id="posters"></a>

## Hängen Sie diese Plakate in Ihrer Schule aus

Print your own posters! Simply click an image below to view and save a printable version.

{{ promote_new_posters }}

* Suchen Sie nach Postern aus früheren Jahren? [Hier finden Sie sie]({{ promote/previous_posters_url }})!

<a id="social"></a>

## Posten Sie Beiträge in sozialen Medien

[![Bild](/images/social-media/fit-250/social-2.png)](/images/social-media/social-2.png)&nbsp;&nbsp;&nbsp;&nbsp; [![Bild](/images/social-media/fit-250/social-3.png)](/images/social-media/social-3.png)&nbsp;&nbsp;&nbsp;&nbsp;

[![Bild](/images/social-media/fit-250/malala_yousafzai.png)](/images/social-media/malala_yousafzai.png)&nbsp;&nbsp;&nbsp;&nbsp; [![Bild](/images/social-media/fit-250/chris_bosh.png)](/images/social-media/chris_bosh.png)&nbsp;&nbsp;&nbsp;&nbsp;

[![Bild](/images/social-media/fit-250/karlie_kloss.png)](/images/social-media/karlie_kloss.png)&nbsp;&nbsp;&nbsp;&nbsp; [![Bild](/images/social-media/fit-250/satya_nadella.png)](/images/social-media/satya_nadella.png)&nbsp;&nbsp;&nbsp;&nbsp; [![Bild](/images/social-media/fit-250/jeff_bezos.png)](/images/social-media/jeff_bezos.png)&nbsp;&nbsp;&nbsp;&nbsp;

<a id="logo"></a>

## Erzählen Sie es mithilfe des Logos von Hour of Code weiter

[![Bild]({{ hoc_logo_fit_200 }})]({{ hoc_logo }}) [![Bild]({{ hdc_logo_fit_200 }})]({{ hdc_logo }})

[Download der hochauflösenden Version](https://images.code.org/share/hour-of-code-logo.zip)

**"Hour of Code" und "Hora del Código" sind geschützte Marken. Die Verwendung soll nicht eingeschränkt werden, es müssen jedoch bestimmte Voraussetzungen erfüllt sein:**

1. Verweise auf "Hour of Code" oder "Hora del Código" dürfen nicht den Anschein erwecken, es sei Ihr eigener Markenname oder Ihre Privatinitiative.

<ul style="margin-top: 0px">
  <li>
    <strong>Beispiel für die richtige Verwendung: "Nehmen Sie an einer Hour of Code™ Veranstaltung bei ACMECor.com teil." </li> 
    
    <li>
      <strong>Beispiel für die falsche Verwendung</strong>: "Nehmen Sie an Hour of Code von UNSEREM UNTERNEHMEN teil."
    </li></ul> 
    
    <ol>
      <li>
        Verwenden Sie ein hochgestelltes "TM", wenn Sie "Hour of Code" erwähnen, und ein hochgestelltes "Ⓡ", wenn Sie "Hora del Código" erwähnen, sowohl auf Ihrer Website als auch in App-Beschreibungen.
      </li>
      
      <li>
        <p>
          Verwendet Sie auf Ihrer Seite (oder in der Fußzeile) folgenden Text sowie die Links zu den Websites CSEdWekk und <a href="{{ codeorg_link }}">Code.org</a>:
        </p>
        <p>
          <em>“The 'Hour of Code™'/'Hora del Código®' ist eine weltweite Initiative von Computer Science Education Week [csedweek.org] und Code.org [code.org], die Millionen Schüler zu einer Stunde Informatik und Programmierung einlädt.</em>
        </p>
      </li>
      
      <li>
        "Hour of Code" oder "Hora del Código" darf nicht in App-Namen verwendet werden.
      </li>
    </ol>
    
    <p>
      

<a id="stickers"></a>

    </p>
    
    <h2>
      Drucken Sie diese Sticker für Ihre SchülerInnen aus
    </h2>
    
    <p>
      (Sticker mit einem Durchmesser von 1 Zoll, 63 Sticker pro Seite)<br />
    </p>
    
    <p>
      <a href="/images/hour-of-code-stickers.pdf"><img src="/images/fit-250/hour-of-code-stickers.png" alt="Bild" /></a>
    </p>
    
    <p>
      

<a id="sample-emails"></a>

    </p>
    
    <h2>
      Laden Sie Ihre Gemeinschaft zu Hour of Code ein und werben Sie per E-Mail für Ihre Veranstaltung.
    </h2>
    
    <h3>
      Hier finden Sie <a href="{{ promote/stats_url }}">weitere Informationen und die verfügbaren Sprachen</a>, wenn Sie über Code of Hour sprechen.
    </h3>
    
    <hr />
    
    <p>
      

<a id="email"></a>

    </p>
    
    <h3>
      Bitten Sie Ihre Schule, Vorgesetzten oder Freunde, sich zu registrieren:
    </h3>
    
    <p>
      <strong>Betreff:</strong> Nehmen Sie gemeinsam mit mir und 100 Millionen andere SchülerInnen an einer "Hour of Code"-Veranstaltung teil <br />
    </p>
    
    <p>
      Computer sind überall und verändern alle Bereiche unserer Welt. Aber nur 51 % der Schulen bieten Informatik-Unterricht an. Die gute Nachricht ist, dass wir dabei sind, das zu ändern! Wenn Sie bereits von der Hour of Code gehört haben, wissen Sie vielleicht, dass sie Geschichte geschrieben hat. Mehr als 100 Millionen SchülerInnen haben bereits an der Hour of Code teilgenommen.
    </p>
    
    <p>
      Mit der Hour of Code findet sich die Informatik auf Homepages von Google, MSN, Yahoo!, und Disney. Über 100 Partner unterstützen gemeinsam diese Bewegung. Jeder Apple Store auf der Welt war Gastgeber einer Hour of Code. Prominente wie Präsident Obama und der kanadische Premierminister Justin Trudeau haben im Rahmen dieser Kampagne ihre ersten Zeilen Code geschrieben.
    </p>
    
    <p>
      Sorgen wir in diesem Jahr für einen noch größeren Bekanntheitsgrad. Ich möchte Sie deshalb einladen, bei Code of Hour {{ campaign_date/year }} mitzumachen. Bitte nehmen Sie an einer Veranstaltung von Hour of Code während der Informatikwoche {{ campaign_date/full }} teil.
    </p>
    
    <p>
      Geben Sie die Information weiter. Organisieren Sie eine Veranstaltung. Fragen Sie an den Schulen in Ihrer Umgebung nach, ob sie teilnehmen möchten, oder nehmen Sie selbst an der Hour of Code teil, jeder profitiert davon, die Grundlagen zu lernen.
    </p>
    
    <p>
      Beginnen Sie mit http://hourofcode.com/{{ country_language }} <br />
    </p>
    
    <hr />
    
    <p>
      

<a id="help-schools"></a>

    </p>
    
    <h3>
      Freiwilliger an einer Schule:
    </h3>
    
    <h4>
      <a href="{{ how_to_volunteers_url }}">Hier finden Sie weitere Ressourcen und Informationen über die Freiwilligenarbeit in Schulen</a>.
    </h4>
    
    <p>
      <strong>Betreff:</strong> Wie können wir Sie bei einer Veranstaltung von Hour of Code unterstützen?
    </p>
    
    <p>
      Vom {{ campaign_date/short }} feiern zehn Prozent der SchülerInnen überall auf der Welt die Informatikwoche mit einer Hour of Code in ihrer Schule. Jedes Kind erhält die Gelegenheit zu erfahren, wie die Technologie um uns herum funktioniert.
    </p>
    
    <p>
      [Unsere Organisation/Mein Name] möchte [Schulname] helfen, eine Hour of Code durchzuführen. Wir unterstützen LehrerInnen die Hour of Code in ihren Klassenzimmern durchzuführen. (Dazu brauchen wir nicht einmal einen Computer!) Wenn Sie möchten, können wir auch eine Schulveranstaltung durchführen, einen Referenten zur Verfügung stellen oder darüber sprechen, wie Technologie funktioniert und was es heißt, ein Software-Ingenieur zu sein.
    </p>
    
    <p>
      Die SchülerInnen werden ihre eigenen Apps oder Spiele programmieren und können sie ihren Eltern zeigen. Wir drucken auch Hour of Code-Zertifikate aus, die sie mit nach Hause nehmen können. Und es macht Spaß! Mit interaktiven, praktischen Aktivitäten lernen die SchülerInnen auf verständliche Weise den Computer kennen.
    </p>
    
    <p>
      Computer sind überall und verändern alle Bereiche unserer Welt. Aber nur 51 % der Schulen bieten Informatik-Unterricht an. Die gute Nachricht ist, dass wir dabei sind, das zu ändern! Wenn Sie bereits von Code of Hour gehört haben, wissen Sie vielleicht, dass wir Geschichte geschrieben haben. Mehr als 100 Millionen SchülerInnen überall auf der Welt haben Hour of Code ausprobiert. Sogar Präsident Obama und der kanadische Premierminister Justin Trudeau schrieben ihre erste Programmzeile im Rahmen unserer Kampagne.
    </p>
    
    <p>
      Unter http://hourofcode.com können Sie mehr über die Veranstaltung lesen, oder sprechen Sie mit uns darüber, wie [Schulname] teilnehmen kann. <br />
    </p>
    
    <hr />
    
    <p>
      

<a id="parents"></a>

    </p>
    
    <h3>
      Erzählen Sie den Eltern von der Schulveranstaltung:
    </h3>
    
    <p>
      <strong>Betreff:</strong> Unsere Schüler verändern die Zukunft mit einer Code of Hour
    </p>
    
    <p>
      Liebe Eltern,
    </p>
    
    <p>
      wir leben in einer technologischen Welt. Wir wissen, was auch immer unsere Kinder werden wollen, wenn sie einmal groß sind, ihr Erfolg wird zunehmend davon abhängen, ob sie etwas von Technologie verstehen.
    </p>
    
    <p>
      Aber wir lernen viel zu wenig darüber, <strong>wie</strong> Technologie funktioniert. Nur an 51 % aller Schulen wird Informatik unterrichtet.
    </p>
    
    <p>
      Aus diesem Grund beteiligt sich unsere gesamte Schule während der Informatikwoche Computer Science Education Week an der größten Lernveranstaltung der Geschichte – der Code of Hour ({{ campaign_date/full }}). Mehr als 100 Millionen SchülerInnen haben bereits bei der Hour of Code mitgemacht. Unsere Hour of Code ist ein Statement, dass [SCHULNAME] bereit ist, diese grundlegenden Fähigkeiten des 21. Jahrhunderts zu lehren. Um Ihren SchülerInnen weiterhin Programmieraktivitäten anzubieten, möchten wir unsere Hour of Code zu einem großen Ereignis machen. Ich rufe alle Freiwilligen auf, sich an die lokalen Medien und die Nachrichtenkanäle der sozialen Medien zu wenden und eine weitere Veranstaltung von Hour of Code in der Gemeinschaft durchzuführen.
    </p>
    
    <p>
      Das ist eine Chance, die Zukunft der Bildung in [NAME DER STADT/DES ORTES] zu gestalten.
    </p>
    
    <p>
      Weitere Informationen finden Sie unter http://hourofcode.com/{{ country_language }}. Verbreiten Sie die gute Nachricht. <br />
    </p>
    
    <hr />
    
    <p>
      

<a id="media-pitch"></a>

    </p>
    
    <h3>
      Laden Sie die Medien zu Ihrer Veranstaltung ein:
    </h3>
    
    <h4>
      <a href="{{ promote/press_kit_url }}">Sehen Sie sich unser Presse Kit an. Es enthält Informationen, wie Sie die Medien zu Ihrer Veranstaltung einladen.</a>
    </h4>
    
    <p>
      <strong>Betreff</strong>: Die Schulen schließen sich unserer Mission an, den SchülerInnen die Informatik näherzubringen.
    </p>
    
    <p>
      Computer sind überall und verändern jede Branche auf der Erde, aber nur 51 % aller Schulen unterrichten Informatik. Junge Frauen und SchülerInnen aus marginalisierten ethnischen Gruppen sind in Informatikkursen und in der Tech-Industrie stark unterrepräsentiert. Die gute Nachricht ist, dass wir dabei sind, das zu ändern!
    </p>
    
    <p>
      Mit der "Hour of Code" findet sich die Informatik auf Homepages von Google, MSN, Yahoo!, und Disney. Über 100 Partner unterstützen gemeinsam diese Bewegung. Jeder Apple Store in der Welt war Gastgeber einer Hour of Code. Auch Präsident Obama schrieb sein erstes Programm im Rahmen dieser Kampagne.
    </p>
    
    <p>
      Deshalb nehmen alle Schüler der [SCHOOL NAME] an der größten Bildungsveranstaltung der Geschichte teil, der Hour of Code, die während der Informatikwoche Computer Science Education Week stattfindet (6. bis 12. Dezember).
    </p>
    
    <p>
      Ich schreibe Ihnen, um Sie zu unserer Eröffnungsversammlung einzuladen, um zu erleben, wie die Kinder am [DATUM] mit der Aktivität beginnen.
    </p>
    
    <p>
      Hour of Code, die von der gemeinnützigen Organisation Code.org und mehr als 100 weiteren Organisationen organisiert wird, ist eine weltweite Bewegung, die davon überzeugt ist, dass die SchülerInnen von heute bereit sind für die entscheidenden Fähigkeiten des 21. Jahrhunderts. Bitte machen Sie mit.
    </p>
    
    <p>
      Kontakt: [IHR NAME], [TITEL], Telefon: (212) 555-5555 Wann: [DATUM und UHRZEIT Ihrer Veranstaltung] Wo: [ADRESSE und HINWEISE]
    </p>
    
    <p>
      Ich freue mich darauf, mit Ihnen in Kontakt zu bleiben.<br />
    </p>
    
    <hr />
    
    <p>
      

<a id="politicians"></a>

    </p>
    
    <h3>
      Laden Sie lokale Politiker zu Ihrer Schulveranstaltung ein:
    </h3>
    
    <h4>
      <a href="{{ how_to_public_officials_url }}">Möchten Sie mehr Informationen? Sehen Sie sich in unserem Info-Material an, wie Sie Politiker zu Ihrer Veranstaltung einladen</a>.
    </h4>
    
    <p>
      <strong>Betreffzeile</strong>: Besuchen Sie unsere Schule, denn wir verändern die Zukunft mit einer Hour of Code
    </p>
    
    <p>
      Sehr geehrte(r) [Bürgermeister/Gouverneur/Vertreter/Senator NACHNAME]:
    </p>
    
    <p>
      Wussten Sie, dass die Computerbranche in den USA die Nummer 1 bei den Löhnen ist? Es gibt in unserem Land mehr als 500.000 Informatikstellen, aber im letzten Jahr haben nur 42.969 Informatikstudenten ihren Abschluss gemacht, um ins Berufsleben einzusteigen.
    </p>
    
    <p>
      Informatik ist heute Grundlage für jede Branche. An den meisten Schulen wird dieses Fach jedoch nicht unterrichtet. Das wollen wir von der [SCHULNAME] ändern.
    </p>
    
    <p>
      Aus diesem Grund beteiligt sich unsere gesamte Schule während der Informatikwoche Computer Science Education Week an der größten Lernveranstaltung der Geschichte – der Code of Hour ({{ campaign_date/full }}). Mehr als 100 Millionen SchülerInnen haben bereits bei der Hour of Code mitgemacht.
    </p>
    
    <p>
      Ich möchte Sie mit diesem Brief zu unsere Hour of Code einladen und Sie bitten, bei unserer Eröffnungsveranstaltung zu sprechen. Sie findet am [DATUM, UHRZEIT, ORT] statt und wird ein deutliches Zeichen setzen, dass die SchülerInnen von [Name des Bundeslandes, der Stadt] die Fähigkeiten des 21. Jahrhunderts lernen werden. Wir möchten, dass unsere Schüler an vorderster Front die Technologie der Zukunft schaffen und nicht nur konsumieren.
    </p>
    
    <p>
      Bitte kontaktieren Sie mich unter [TELEFONNUMMER ODER E-MAIL-ADRESSE]. Ich freue mich auf Ihre Antwort.
    </p>
    
    <p>
      {{ signup_button }}
    </p>