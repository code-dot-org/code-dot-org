---
title: Darbības vadlīnijas
---

<style>
  ul {
    margin: 0px 20px 20px 20px;
  }
</style>

# Vadlīnijas pamācību un stundu izveidei un iesniegšanai “Programēšanas stunda™” un datorzinātņu izglītības nedēļā

Code.org rīko dažādas Programmas stundas™ aktivitātes, nodarbības un videoklipus Code.org un Kodēšanas stundas tīmekļa vietnēs. The current list is at [hourofcode.com/learn]({{ urls/learn }}).

Vēlaties iesniegt savu pašvadāmu pamācību, skolotāju vadītu nodarbību vai robotikas/veidotāja darbību, kas izskaidro datorzinātnes principu? Pievienojieties šai globālajai kustībai un palīdziet dalībniekiem visā pasaulē sākt darbu ar programmēšanas stundu vai doties tālāk ar vairāku stundu, dienu vai nedēļu ilgām aktivitātēm.

**Pēc vadlīniju lasīšanas jūs varat iesniegt savu darbību, izmantojot mūsu [kodēšanas stunda™ darbību iesniegšanas lapu](https://docs.google.com/forms/d/e/1FAIpQLSdsdTKU6pD5qXlPUQWlgrjESRlm2L02wkTcZQuL8hz9OEvzNQ/viewform). Jūs varat iesniegt darbību jebkurā laikā, bet termiņš iekļaušanai jebkurā kalendārajā gadā ir 1. oktobris. (Piemēram, visas darbības, kas saņemtas pēc 2023. gada 1. oktobra, netiks uzskaitītas 2023. gada kodēšanas stundā.)** Ja Jums ir kādi jautājumi par Jūsu darbības iesniegšanu, lūdzu, sazinieties ar mums pa e-pastu support@code.org.

Daži padomi:

1. **Submit more than one activity**: If you’ve built activities for different levels, different ages, or other categories, we list your activities separately so each teacher can find the right thing for their classroom. Iesniegt katru pamācību vai darbību atsevišķi. Ņemot vērā iesniegumu skaitu, ko esam redzējuši pēdējos gados, mums būs laiks pārskatīt **līdz 5 aktivitātēm** katram partnerim. Pēc tam mēs darīsim visu iespējamo, lai pārskatītu pēc iespējas vairāk pirms Programmēšanas stundas sākuma.

2. **Beyond iesācējs nodarbības**: Papildus nodarbības skolotājiem un studentiem, kuri mācās datorzinātnes pirmo reizi, mēs uzskaitīt mācību pieredzi datorzinātnes gudriem klasēs, kas vēlas iet mazliet tālāk! Palīdziet mums, iesniedzot nodarbības klasēm, kas jau ir apmierinātas ar pamatiem.

3. **Priekšmeta jomas**: Ir lieliska mācība ideja, kas integrē Computer Science matemātikā? Vēsture? Valodu māksla? Zinātne? Māksla? Vai kāds cits temats? Mums bija daudz pieprasījumu no skolotājiem, kuri vēlas savienot Programēšanas stundu ar savu tēmu jomu. Skolotāji var filtrēt savu klases veidu (klases josla vai priekšmeta apgabals), tāpēc mums ir nepieciešama jūsu palīdzība, aizpildot nepilnības, lai piedāvātu klases aktivitātes vai stundu plānus, kas attiecas uz CS katrai galvenajai mācību jomai dažādās pakāpju grupās. Mums arī turpmāk ir “Computer Science” kategorija skolotājiem, kuri meklē vispārējas CS aktivitātes.

<a id="top"></a>

## Indekss:

- [Ko iesniegt](#whatsubmit)
- [Vispārīgas vadlīnijas Programmēšanas stundas™ darbības izveidei](#guidelines)
- [Kā iesniegt](#submit)
- [Kā tiks novērtētas aktivitātes](#inclusion)
- [Ieteikumi, lai konstruētu jūsu darbības](#design)
- [Preču zīmes vadlīnijas](#tm)
- [Enkura punkts](#pixel)
- [Jūsu aktivitātes veicināšana, CSedWeek un Programmēšanas stunda](#promote)

<a id="whatsubmit"></a>

## Ko iesniegt

**Pašvadāmās puzzle vai spēle ([piemērs](https://code.org/dance))**

Šīs aktivitātes ir paredzētas, lai studenti varētu pašnodarboties ar pamācību. Tie neprasa daudz mācību no skolotāja vai skolotāja sagatavošanu darbu.

**Teacher Facilitated lesson ([example](https://studio.code.org/s/course1/lessons/2/levels/1), [template](https://docs.google.com/document/d/1DhaLNd6uim9rRPkHrglnOvVcgzGc8axKIzv8YI56JGA))**

Tagad, kad simtiem tūkstošu pedagogu ir izmēģinājuši Programēšanas stundu, daudzas klases ir gatavas vairāk radošām aktivitātēm, kas māca datorzinātņu pamatus. Lai palīdzētu progresīvākiem skolotājiem atrast iedvesmu, mēs apkopojam un kurējam “skolotāju vadītas” nodarbības un aktivitāšu plānus programmēšanas stundas veterāniem.

Viens no aktivitāšu veidiem, ko mēs piedāvājam pieredzējušiem skolotājiem, ir “atvērtās smilškastes” izveides projekti. Darbības, kas mudina skolēnus veidot savu lietotni, spēli, tīmekļa vietni vai citu projektu. Ja to atvieglo pareizi, vairāk atvērtu darbību var labāk demonstrēt datorzinātņu radošo dabu.

Daži pedagogi var arī izvēlēties rīkot Programmēšanas stundas aktivitātes, kas atbilst tradicionālajam nodarbību formātam, nevis ceļvedis-puzle/spēļu pieredzei.

Jūs varat sākt ar šo [veidni](https://docs.google.com/document/d/1DhaLNd6uim9rRPkHrglnOvVcgzGc8axKIzv8YI56JGA) savam stundu plānam.

**Pasākumi skolotājiem citos priekšmetos/jomās**

Mēs piedāvājam arī nodarbību plānus, kas paredzēti dažādām mācību jomām. Piemēram, vienas stundas stundu plāns koda mācīšanai ģeometrijas klasē. Vai mad-lib vingrinājums angļu klasē. Vai radoša viktorīnas radīšanas aktivitāte vēstures klasei. Tie var palīdzēt pieņemt darbā skolotājus citās jomās, lai vadītu programmēšanas stundas aktivitātes, kas ir unikālas viņu jomā, vienlaikus parādot, kā CS var ietekmēt un uzlabot daudzas dažādas tematiskās jomas.

Piemēri:

- Mirror Images (aktivitāte mākslas skolotājam)
- Arduino aktivitāte fizikas skolotājam
- Vēstures skolotāja tehnoloģiju aktivitātes vēsture
- Un skatiet [šo sarakstu](https://docs.google.com/document/d/19jQ2VhBUyIn30VFbfdHOVpmlymhNiB0AkEqRxHz3dr0/edit) , lai iegūtu vairāk ideju no pedagogiem (vai pievienojiet savu sarakstu, lai iedvesmotu citus)

**Studentiem ar īpašām vajadzībām**

Ja jūs izveidojat darbību vai pamācību, kas ir paredzēta studentiem ar īpašām vajadzībām, lūdzu, zvaniet, kas izklāstīti aprakstā. Jo īpaši redzes traucējumiem ir ļoti maz iespēju. Ja jūsu darbība ir paredzēta šai auditorijai, lūdzu, dariet mums zināmu.

[**Atgriezties sākumā**](#top)

<a id="guidelines"></a>

## Vispārējās vadlīnijas, lai izveidotu "Programmēšanas stundas" aktivitātes

Programmēšanas stundas mērķis ir dot iesācējiem pieejamu pirmo datorzinātņu vai programmēšanas garšu. Tonam jābūt šādam:

- Ikviens var apgūt datorzinātnes neatkarīgi no vecuma, dzimuma, rases vai spējas/invaliditātes.
- Datorzinātne ir saistīta ar visdažādākajām jomām un interesēm. Ikvienam vajadzētu to iemācīties!
- Mudiniet skolēnus radīt kaut ko unikālu, ko var dalīties ar vecākiem/draugiem vai tiešsaistē.

Darbībām vajadzētu mācīt datorzinātnes koncepciju, piemēram, cilpas, nosacījumi, šifrēšanu vai kā internets darbojas. Aktivitāte var arī mācīt par to, kā datorzinātne savienojas ar reālās pasaules profesijām, notikumiem vai vēsturi. Piemēram, UX noformējuma mācīšana, lai padarītu lietotnes, kas ir nozīmīgas auditorijai vai cēloņiem. Mēs atturam no darbībām, kas vērstas uz programmēšanas sintaksi, nevis uz jēdzieniem. Piemēram, mēs uzskaitīsim, bet ne izcelt darbības, kas māca HTML. Tāpat mēs atturam bloķēt programmēšanas nodarbības, kas koncentrējas uz konfigurācijas opciju iestatīšanu/mainīšanu, nevis mācīties, kā modelēt algoritmu vai procesu.

*Technical requirements:* Because of the wide variety of school and classroom technology setups, the best activities are Web-based or smartphone-friendly, or otherwise unplugged-style activities that teach computer science concepts without the use of a computer (see <http://csunplugged.com/>). Darbības, kurām nepieciešama programmu instalēšana, datora programma vai spēļu konsole, ir labi, bet ne ideāla. Mēs neuzskaitīsim darbības, kurām nepieciešama reģistrēšanās vai maksājums. (Robotikas darbībām var būt nepieciešama robotikas iegāde.)

*Studentu vadīja (Self-Vadoties) Formāts:* Sākotnējā kodēšanas stunda tika veidota galvenokārt uz panākumiem pašapkalpošanās konsultācijas vai nodarbības, pēc izvēles veicināja skolotājs. Ir daudz esošo iespēju, bet, ja vēlaties izveidot jaunu, šīs aktivitātes ir jāizstrādā tā, lai tās varētu būt jautri studentam, kas strādā vienatnē, vai klasē, kuras skolotājam ir minimāla sagatavošana vai CS fons. Viņiem būtu jānodrošina virzieni studentiem pretstatā beztermiņa stundu ilgam izaicinājumam. Ideālā gadījumā instrukcijas un konsultācijas tiek integrētas tieši programmēšanas platformā, lai izvairītos no cilņu vai logu pārslēgšanas starp pamācību un programmēšanas platformu.

Lai iegūtu izpratni par dažādiem apmācības veidiem un stundu plāniem, ko varat izveidot, apmeklējiet [kodēšanas stundu aktivitātes lapu](https://hourofcode.com/us/learn).

[**Atgriezties sākumā**](#top)

<a id="submit"></a>

## Kā iesniegt

Apmeklējiet [kodēšanas stundu™ darbību iesniegšanas lapu](https://docs.google.com/forms/d/e/1FAIpQLSdsdTKU6pD5qXlPUQWlgrjESRlm2L02wkTcZQuL8hz9OEvzNQ/viewform) un aizpildiet jautājumus, lai iesniegtu savu darbību.

Kas tev būs nepieciešams:

* Primārās kontaktpersonas vārds un e-pasta adrese, kas pārstāv iesniegto darbību
* Darbības nosaukums (nosaukumā nevar iekļaut “Kodēšanas stunda”)
* URL saite uz darbību
* Vai šī darbība ietver AI: Jā vai Nē
* Aktivitātes apraksts (maks. rakstzīmju skaits: 400) 
    * Lūdzu, norādiet vai apraksts ir pārsvarā skolēnu veidots vai skolotāju sekmēts. Pie tam, dažas skolas ir ieinteresētas noskaidrot vai "Programmēšanas stundas" aktivitātes atbilst mūsdienās noteiktiem tehnoloģiju apmācību standartiem. Ja aktivitāte atbilst noteiktiem standartiem, pieļaujiet iespēju iekļaut šo informāciju.
* Ieteicamais (-ie) pakāpes līmenis (-i) paredzētajiem lietotājiem. Jūs varat atsaukties uz \[Computer Skolotāju asociācijas K-12 standarti\] (https://k12cs.org/framework-statements-by-grade-band/) par pakāpei atbilstošu datorzinātņu koncepcijas. Piemēri pakāpju līmeņi ir šādi: 
    * Bērnudārzs - 1. pakāpe (vecums 4-6)
    * 2.-5. Klases (7-10 gadi)
    * 6.—8. pakāpe (11-13 gadu vecums)
    * Klases 9+ (vecums 14+)
* Programmēšanas valodu saraksts, ko māca jūsu darbība. Ti. C/C++, Java, JavaScript utt. vai neatkarīgi no valodas (stundu plāniem, kurus var mācīt vairākās valodās) izvēlēties valodu mākslu.
* Atbalstīto dabisko valodu saraksts. Piezīme: Valodas noteikšana ir atbildīga par darbību nodrošinātāju; mēs novirzīsim visus lietotājus uz norādīto vienoto URL. vai neatkarīgi no valodas (stundu plāniem, kurus var mācīt vairākās valodās)
* Kādam pieredzes līmenim pedagogam vajadzētu izmantot jūsu darbību? (Piemēram, iesācējs vai ērts.) Un kādam pieredzes līmenim vajadzētu būt studentiem?
* Kādam pieredzes līmenim pedagogam vajadzētu izmantot jūsu darbību? (Piemēram, iesācējs vai ērts.) Un kādam pieredzes līmenim vajadzētu būt studentiem? Ja vēlaties sagatavot pilnīgākas Programmēšanas stundas™ aktivitātes, lūdzu, norādiet savas darbības aprakstā nepieciešamās iepriekšējās zināšanas.
* Jūsu darbības ilgums 
    * Tikai 1 stunda
    * 1 stunda ar pēckursu
    * 2-6 stundas (var būt vairākas nodarbības)
* Pieejamības naktsmītņu saraksts, ja tās ir jūsu aktivitātēs. Tie ietver ekrāna lasītāja saderību, teksta-runas iespējas, augsta kontrasta krāsu izmantošanu vai jebkādas citas naktsmītnes, kas paredzētas mācību dalībniekiem ar īpašām vajadzībām.

#### Papildu lietas, kas jums būs nepieciešamas, iesniedzot stundu plānus

* Saite uz savu stundu plānu. Tas var būt tīmekļa lapa, dropbox saite, google disks vai līdzīgs pakalpojums.
* Kāda programmatūra un/vai aparatūra skolotājam būs nepieciešama, lai veiktu jūsu stundu plānu (Scratch? Roboti? Nekas?)

#### Papildu lietas, kas jums būs nepieciešamas, iesniedzot tiešsaistes darbības

* Organizācijas nosaukums un logotips
* Neatkarīgi no tā, vai tas atbilst COPPA prasībām
* URL saite uz skolotāja piezīmēm (neobligāti)
* Testētu un saderīgu platformu saraksts: 
    * Tīmeklī: kuras platformas esat pārbaudījis? 
        * OS - Mac, Win un citas versijas
        * Pārlūkprogrammas - IE11, Edge, Firefox, Chrome, Safari
        * iOS mobilais Safari (pielāgots mobilajām ierīcēm)
        * Android Chrome (pielāgots mobilajām ierīcēm)
    * Ne tīmeklī: norādiet platformu vietējam kodam (Mac, Win, iOS, Android, XBox, citi)
    * Atslēgtas
* Programmēšanas stundas aktivitātes ekrānuzņēmums vai mārketinga attēls. Lūdzu, sūtiet vismaz vienu attēlu ar 4:3 izmēriem. Tam jābūt vismaz 520px ar 390px. Šim attēlam uz tā jābūt NO teksta (izņemot jūsu logotipu), lai padarītu to pieejamāku tiem, kas nav angļu valodā runājošie. Ja nepieciešamais attēls netiks nodrošināts, mēs izmantosim savu ekrānuzņēmumu no jūsu apmācībām VAI mēs varam to neizmantot. Visi attēli ir jāiesniedz kā URL saite uz .jpg, .jpeg vai .png.
* Lai precīzāk izsekotu līdzdalību, trešo pušu apmācības partneriem ir jāiekļauj 1 pikseļa izsekošanas attēli savā Programēšanas stundas pamācību pirmajā lapā. Skatiet [izsekošanas pikseļu](#pixel) sadaļu zemāk, lai iegūtu sīkāku informāciju.
* Upon finishing your activity, users should be directed to [code.org/api/hour/finish](https://code.org/api/hour/finish) where they will be able to: 
    * Daļa sociālajos plašsaziņas līdzekļos, ka viņi pabeiguši Programmēšanas stunda
    * Saņemiet sertifikātu, ka viņi pabeiguši Programmēšanas stundu
    * Skatiet līderu padomus par to, kurās valstīs/pilsētās ir visaugstākie dalības rādītāji Programmēšanas stundas aktivitātēs
    * For users who spend an hour on your activity and don’t complete it, please include a button on your activity that says “I’m finished with my Hour of Code” which links back to [code.org/api/hour/finish](https://code.org/api/hour/finish) as well.
* (Papildiespēja) Mēs varam sekot ar tiešsaistes aptaujas/veidlapas saiti, lūdzot ziņojumu par šādiem darbības rādītājiem: 
    * Tiešsaistes darbībām (īpaši viedtālruņu/planšetdatoru lietotnēm): 
        * Lietotāju skaits
        * Cik daudz pabeidza uzdevumu
        * Vidējais uzdevuma veikšanas laiks
        * Kopējais kodu rindu skaits, kas uzrakstīti visiem lietotājiem
        * Cik daudz turpināja mācīties (mērot kā jebkuru lietotāju, kurš pabeidz uzdevumu un dodas uz papildu uzdevumiem jūsu vietnē)
    * Bezsaistes darbībām 
        * Lejupielāžu skaits darbības papīra versijā (ja piemērojams)

#### Papildu lietas, kas jums būs nepieciešamas, iesniedzot robotiku

* Ja jūs iesniedzat robotikas darbību, mums jāzina izmaksas uz vienu studentu robotikas jomā.
* Lai novērtētu robotikas aktivitātes iekļaušanai mājas lapā, mums būs nepieciešams, lai jūs nosūtītu paraugu komplektus recenzentiem.

[**Atgriezties sākumā**](#top)

<a id="inclusion"></a>

## Kā tiks novērtētas aktivitātes

Daudzveidīga datorzinātņu pedagogu komiteja ierindos iesniegumus, pamatojoties uz kvalitatīviem un kvantitatīviem kritērijiem. Tiks uzskaitītas visas darbības, kas atbilst pamatkritērijiem.

Tradicionālajām kodēšanas aktivitātēm daudzveidīga datorzinātņu pedagogu komiteja ierindos iesniegumus, pamatojoties uz kvalitatīviem un kvantitatīviem kritērijiem. Tiks uzskaitītas visas darbības, kas atbilst pamatkritērijiem. Skolotāji varēs filtrēt un kārtot, lai atrastu labākās aktivitātes savai klasei.

Ja pārskata komiteja novērtē darbību nulles līmenī ražošanas kvalitātē (sliktu kļūdu vai instrukciju dēļ, kas padara to ļoti grūti izmantot), veicinot mācīšanos nepietiekami pārstāvētās grupās (rasistu/seksisma materiālu dēļ), izglītības vērtībā (nemāca CS koncepcijas) vai jautri/iesaistoties (sakarā ar grūtību/atturēšanu skolēniem strādāt), darbība netiks uzskaitīta.

- Augsta produkcijas kvalitāte
- Veicināt mācīšanos visās demogrāfiskajās grupās (esp. skolēni, kas ir nepietiekami pārstāvēti CS, piemēram, jaunas sievietes, studenti no atstumtām rasu un etniskajām grupām un studenti ar invaliditāti)
- Izglītības (labi māca datorzinātnes koncepcijas)
- Jautri un iesaistoši
- Mudina skolēnus radīt kaut ko unikālu viņi var dalīties (Jaunākiem skolēniem: ar vecākiem un klasesbiedriem. Vecajiem studentiem: internetā)

Turklāt, lai tās tiktu uzskaitītas, visām darbībām:

Jaunajiem skolotājiem un skolēniem pašorientētām aktivitātēm pārskata komiteja meklēs, vai:

- Būtu piemērots publiskai skolas klasei (bez ieročiem, nav skaidra/nobrieduša satura, bez reliģiska satura utt.)
- Nepieciešama reģistrēšanās
- Nepieciešama nekāda samaksa (izņēmums robotikas darbībām, kuras var prasīt robota/komplekta iegādi)

Jaunajiem skolotājiem un skolēniem pašorientētām aktivitātēm pārskata komiteja meklēs, vai:

- Skolotāji var izmantot pamācību vai darbību bez fona datorzinātnēs
- Studenti var būt veiksmīgi bez fona datorzinātnēs
- Studenti var vadīt sevi caur pamācību bez vecāku vai skolotāju norādījumiem

Skolotāji un skolēni varēs meklēt un filtrēt mūsu darbību sarakstu, balstoties uz tādiem filtriem kā pakāpe, pieredzes līmenis, priekšmets, aparatūra utt. Pēc noklusējuma mēs vispirms parādīsim stundu plānus un aktivitātes:

- Saņemiet augstākos vērtējumus no izskatīšanas komitejas
- Ir viena stunda pašorientētas aktivitātes, kas paredzētas iesācējiem (skolēniem un skolotājiem)
- Apelācija plašam lietotāju lokam (dažādās platformās, valodās un vecumos)
- Nepieciešama instalēšana
- Ir jauni šogad

[**Atgriezties sākumā**](#top)

<a id="design"></a>

## Ieteikumi vienas stundas pašapkalpošanās konsultāciju projektēšanai

Darbībās nav jāiekļauj AI, lai gan mēs vēlamies piedāvāt vairāk ar mākslīgo intelektu saistītu darbību.

**Iekļaut skolotāju piezīmes.** Lielākajai daļai aktivitāšu jābūt studentiem, bet, ja darbību veicina vai vada skolotājs, lūdzu, iekļaujiet skaidrus un vienkāršus norādījumus skolotāja piezīmju veidā ar atsevišķu URL, kas iesniegts ar savu darbību. Ne tikai studenti iesācēji, daži no skolotājiem ir arī. Iekļaut informāciju, piemēram:

**Iekļaut atgriezenisko saiti darbības beigās.** (Piemēram, “Jūs pabeidzāt 10 līmeņus un uzzinājāt par cilpām! Lielisks darbs!”)

**Mudiniet skolēnus publicēt sociālajos medijos (ja nepieciešams), kad viņi ir pabeidzuši.** Piemēram, “Es esmu darījusi Programmēšanas stunda ar ________ Vai jums? #HourOfCode” vai “Es esmu darījusi #HourOfCode kā daļu no #CSEdWeek. Vai tu esi?

- Kādas platformas un pārlūkprogrammas vislabāk darbojas apmācība?
- Vai tas darbojas viedtālruņos? Tabletes?
- Vai jūs ieteiktu pāra programmēšanu?
- Apsvērumi izmantošanai klasē? Piemēram, ja ir videoklipi, ieteikt skolotājiem rādīt videoklipus projicētā ekrānā visai klasei, lai tos skatītu kopā

**Iekļaut atgriezenisko saiti darbības beigās.** (Piemēram, “Jūs pabeidzāt 10 līmeņus un uzzinājāt par cilpām! Lielisks darbs!”)

**Mudiniet skolēnus publicēt sociālajos medijos (ja nepieciešams), kad viņi ir pabeidzuši.** Piemēram, “Es esmu darījusi Programmēšanas stunda ar ________ Vai jums? #HourOfCode” vai “Es esmu darījusi #HourOfCode kā daļu no #CSEdWeek. Vai tu esi? @Scratch.” Izmantojiet hashtag **#HourOfCode** (ar lielajiem burtiem H, O, C)

**Izveidojiet savu darbību spāņu valodā vai citās valodās, izņemot angļu valodu.**

**Paskaidrojiet vai savienojiet darbību ar sociāli nozīmīgu kontekstu.** Datorprogrammēšana kļūst par lielvalsti, kad studenti redz, kā tā var mainīt pasauli uz labo pusi!

**Pārliecinieties, ka jūsu apmācība var izmantot [Pair Programmēšana](http://www.ncwit.org/resources/pair-programming-box-power-collaborative-learning) paradigma.** Tas ir īpaši noderīgi programmēšanas stundai, jo daudzās klasēs nav 1:1 aparatūras visiem studentiem.

[**Atgriezties sākumā**](#top)

<a id="tm"></a>

## Preču zīmes vadlīnijas

Kodēšanas stunda® un Hora del Código® ir reģistrētas preču zīmes Code.org. Daudzi no mūsu apmācības partneriem ir izmantojuši mūsu “Programmēšanas stundas” preču zīmes savās tīmekļa vietnēs saistībā ar programmēšanas stundas aktivitātēm. Mēs negribam, lai novērstu šo izmantošanu, bet mēs vēlamies, lai pārliecinātos, ka izmantošana ietilpst pāris ierobežojumi:

1. Izmantot “Programmēšanas stunda” tikai saistībā ar nekomerciālām CS izglītības darbībām Programmēšanas stundas kampaņas kontekstā un nekādiem citiem mērķiem.
2. Jebkura atsauce uz “Programmēšanas stundu” jāizmanto tādā veidā, kas neliecina, ka tas ir jūsu zīmola nosaukums, bet tas drīzāk atsaucas uz Programmēšanas stundu kā tautas kustību. Labs piemērs: “Piedalieties Programēšanas stundā® vietnē AcmeCorp.com”. Slikts piemērs: “Izmēģiniet kodēšanas stundu pēc ACME Corp”.
3. Izmantojiet “®” augšrakstu visredzamākajās vietās, kur pieminat “Programmēšanas stunda”, gan savā tīmekļa vietnē, gan lietotņu aprakstos.
4. Iekļaujiet valodu lapā (vai kājenē), tostarp saites uz Programmēšanas stunda, CSedWeek un Code.org tīmekļa vietnēm, kas atklāj abas šīs: a. Kodēšanas stunda® un Hora del Código® ir reģistrētas preču zīmes Code.org; un b. “Uz '[stunda koda](http://hourofcode.com/) ® ir valsts mēroga iniciatīva [Code.org](http://code.org/) ieviest miljoniem studentu uz vienu stundu datorzinātnes un datorprogrammēšanas.”
5. Neizmantojiet “Kodēšanas stunda” lietotņu nosaukumos.
6. Neizmantojiet “Programmēšanas stundu” saistībā ar jebkādiem komerciāliem mērķiem vai mērķiem (piemēram, novietojot savu Programmēšanas stundas darbību aiz maksas sienas, veicinot citu maksas pakalpojumu kā daļu no jūsu Programmēšanas stundas darbības; pārdodot “Programmēšanas stundas” preces).
7. Neizmantojiet “Kodēšanas stunda” saistībā ar jebkuru darbību, kurai nepieciešams pieteikšanās vai konta izveide.

[**Atgriezties sākumā**](#top)

<a id="pixel"></a>

## Enkura punkts

Ja jūsu apmācība ir apstiprināta un iekļauta pēdējā apmācības lapā, Code.org sniegs jums unikālu izsekošanas pikseli, lai jūs varētu integrēties savā apmācībā. Skatiet piemēru zemāk. Neiekļaujiet savā apmācībā nekādās pagaidu lapās.

Tas ļaus mums saskaitīt lietotājus, kuri veic jūsu kodēšanas stundas pamācību. Tas novedīs pie precīzākas līdzdalības skaita jūsu apmācībai.

Ja jūsu apmācība ir apstiprināta un iekļauta pēdējā apmācības lapā, Code.org sniegs jums unikālu izsekošanas pikseli, lai jūs varētu integrēties savā apmācībā. Skatiet piemēru zemāk.

PIEZĪME.: Tas nav svarīgi, lai veiktu instalējamas programmas (iOS vai Android lietojumprogrammas vai datora instalēšanas programmas)

Piemērs sekošanas pikseļi Dance Party:

IMG SRC = <http://code.org/api/hour/begin_dance.png>   


[**Atgriezties sākumā**](#top)

<a id="promote"></a>

## Jūsu aktivitāšu veicināšana, CSEDWeek un Programmēšanas stunda

Lūdzu, veiciniet savu darbību savā tīklā! Novirziet tos uz jūsu Programmēšanas stundas lapu. Jūsu lietotājiem ir daudz lielāka iespēja reaģēt uz pasta sūtīšanu no jums par jūsu darbību. Izmantojiet starptautisko Programmēšanas stundas kampaņu Datorzinātnes izglītības nedēļas laikā kā attaisnojumu, lai mudinātu lietotājus uzaicināt citus pievienoties un palīdzēt mums sasniegt vairāk studentu!

- Feature Programmēšanas stunda un CSedWeek savā mājas lapā. Piemērs: <http://www.tynker.com/hour-of-code>
- Veicināt Programmēšanas stundu, izmantojot sociālos medijus, tradicionālos medijus, adresātu sarakstus utt., izmantojot hashtag #HourOfCode (ar lielajiem burtiem H, O, C)
- Vadi vietējo notikumu vai palūdz saviem darbiniekiem to darīt tuvējās skolās vai komūnās.
- Lai iegūtu sīkāku informāciju, skatiet mūsu [resursu komplektu](https://hourofcode.com/us/promote) .

[**Atgriezties sākumā**](#top)

{{ signup_button }}