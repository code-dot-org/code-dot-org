...
**生徒の認定証はどうやったら印刷できますか？**

[Code.org tutorials](http://studio.code.org)を終えると、メールで認定証が届きます。 認定書をクリックすることで印刷ができます。 もしくは、[Certificates](http://<%= codeorg_url() %>/certificates)のページから、まとめて沢山の認定証を印刷することも可能です。

**Hour of Code を終えてしまったら？**

Hour of Code はテクノロジーの仕組みやソフトウェアアプリケーションの作り方に関する勉強の初めの1歩にすぎません。 <% if @country == 'uk' %> Hour of Code はテクノロジーがどのような仕組みになっているか、またソフトウエアアプリケーションをどのように製作するかについて学ぶ最初の入り口に過ぎません。 引き続き [オンラインで勉強しよう](http://uk.code.org/learn/beyond) にある課題に取り組んでみてください。 <% else %> これを継続させるための、教育者用のリソースは[こちら](http://<%= codeorg_url() %>/educate)です。 また、子供にも[オンライン](http://<%= codeorg_url() %>/learn/beyond)で学習を持続することを促してください。 <% end %>