...
The OTel Contrib Collector receives telemetry data (traces, metrics, logs) via OTLP and forwards it to a configurable APM backend. Supported backends are **Datadog**, **Sentry** (default), and **Splunk Observability Cloud**. The active backend is controlled by the `apm_backend` attribute.
...
| `node['cdo-otel-collector']['apm_backend']` | `'sentry'` | APM backend: `'datadog'`, `'sentry'`, or `'splunk'` |
