  ...
  def self.setup(environment, helpers, cloudfront=false)
    describe 'http proxy cache' do
      ...
      it 'Handles Accept-Language behaviors' do
        ...
        url = build_url 's/starwars/lesson/1/puzzle'
        text_en = 'Hello World!'
      ...
      it 'Strips cookies from the penultimate dance level' do
        assert strips_session_specific_cookies_from_request? '/s/dance/lesson/1/puzzle/12'
        assert strips_session_specific_cookies_from_request? '/s/dance-2019/lesson/1/puzzle/9'
      end

      it 'Does not strip cookies from the last dance level' do
        refute strips_session_specific_cookies_from_request? '/s/dance/lesson/1/puzzle/13'
        refute strips_session_specific_cookies_from_request? '/s/dance-2019/lesson/1/puzzle/10'
      end

      it 'Strips cookies from an aquatic level' do
        assert strips_session_specific_cookies_from_request? '/s/aquatic/lesson/1/puzzle/5'
      end
