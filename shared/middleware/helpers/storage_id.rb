...
def create_storage_id_for_user(user_id = nil, anon_user_id: nil)
  ...
  if Integer(user_id, exception: false).nil? && Integer(anon_user_id, exception: false).nil?
    CDO.log.info JSON.dump(
      namespace: 'project_storages',
      event: 'missing_anon_user_id',
      storage_id:,
      user_id: user_id.inspect,
      anon_user_id: anon_user_id.inspect,
      request_anon_user_id: defined?(request) ? request.anon_user_id.inspect : 'no_request',
