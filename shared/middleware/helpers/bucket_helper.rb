...
class BucketHelper
  ...
  def copy_files(src_channel, dest_channel, options={})
    ...
    result = s3.list_objects(bucket: @bucket, prefix: src_prefix).contents.map do |fileinfo|
      ...
      Honeybadger.context(
        {
          copy_source: CGI.escape(src),
          copy_dest_bucket: @bucket,
          copy_dest_key: dest
        }
      )
      response = s3.copy_object(bucket: @bucket, key: dest, copy_source: CGI.escape(src), metadata_directive: 'REPLACE')
  ...
  def restore_file_version(encrypted_channel_id, filename, version)
    ...
    s3.copy_object(bucket: @bucket, copy_source: CGI.escape("#{@bucket}/#{key}?versionId=#{version}"), key: key, metadata_directive: 'REPLACE')
  end
  ...
  def replace_abuse_score(encrypted_channel_id, filename, abuse_score)
    ...
    s3.copy_object(bucket: @bucket, copy_source: CGI.escape("#{@bucket}/#{key}"), key: key, metadata: {abuse_score: abuse_score.to_s}, metadata_directive: 'REPLACE')
  end
