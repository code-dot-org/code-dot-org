...
class AppsApi < Sinatra::Base
  ...
  #
  # GET /v3/(shared|user)-properties/<app-id>
  #
  # Returns all of the properties in the bag
  #
  get %r{/v3/(shared|user)-properties/([^/]+)$} do |endpoint, app_id|
    dont_cache
    content_type :json
  ...
  #
  # GET /v3/(shared|user)-properties/<app-id>/<property-name>
  #
  # Returns a single value by name.
  #
  get %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)$} do |endpoint, app_id, name|
    dont_cache
    content_type :json
  ...
  #
  # DELETE /v3/(shared|user)-properties/<app-id>/<property-name>
  #
  # Deletes a value by name.
  #
  delete %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)$} do |endpoint, app_id, name|
    dont_cache
  ...
  #
  # POST /v3/(shared|user)-properties/<app-id>/<property-name>/delete
  #
  # This mapping exists for older browsers that don't support the DELETE verb.
  #
  post %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)/delete$} do |endpoint, app_id, name|
    call(env.merge('REQUEST_METHOD'=>'DELETE', 'PATH_INFO'=>File.dirname(request.path_info)))
  end

  #
  # POST /v3/(shared|user)-properties/<app-id>/<property-name>
  #
  # Set a value by name.
  #
  post %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)$} do |endpoint, app_id, name|
    unsupported_media_type unless request.content_type.to_s.split(';').first == 'application/json'
    unsupported_media_type unless request.content_charset.to_s.downcase == 'utf-8'
  ...
  #
  patch %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)$} do |endpoint, app_id, name|
    call(env.merge('REQUEST_METHOD'=>'POST'))
  end
  put %r{/v3/(shared|user)-properties/([^/]+)/([^/]+)$} do |endpoint, app_id, name|
    call(env.merge('REQUEST_METHOD'=>'POST'))
  end
  ...
  #
  # GET /v3/(shared|user)-tables/<app-id>/<table-name>
  #
  # Returns all of the rows in the table.
  #
  get %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)$} do |endpoint, app_id, table_name|
    dont_cache
    content_type :json
  ...
  #
  # GET /v3/(shared|user)-tables/<app-id>/<table-name>/<row-id>
  #
  # Returns a single row by id.
  #
  get %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)$} do |endpoint, app_id, table_name, id|
    dont_cache
    content_type :json
  ...
  #
  # DELETE /v3/(shared|user)-tables/<app-id>/<table-name>/<row-id>
  #
  # Deletes a row by id.
  #
  delete %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)$} do |endpoint, app_id, table_name, id|
    dont_cache
  ...
  #
  # POST /v3/(shared|user)-tables/<app-id>/<table-name>/<row-id>/delete
  #
  # This mapping exists for older browsers that don't support the DELETE verb.
  #
  post %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)/delete$} do |endpoint, app_id, table_name, id|
    call(env.merge('REQUEST_METHOD'=>'DELETE', 'PATH_INFO'=>File.dirname(request.path_info)))
  end

  #
  # POST /v3/(shared|user)-tables/<app-id>/<table-name>
  #
  # Insert a new row.
  #
  post %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)$} do |endpoint, app_id, table_name|
    unsupported_media_type unless request.content_type.to_s.split(';').first == 'application/json'
    unsupported_media_type unless request.content_charset.to_s.downcase == 'utf-8'
    ...
    redirect "/v3/#{endpoint}-tables/#{app_id}/#{table_name}/#{value[:id]}", 301
  end

  #
  # PATCH (PUT, POST) /v3/(shared|user)-tables/<app-id>/<table-name>/<row-id>
  #
  # Update an existing row.
  #
  post %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)$} do |endpoint, app_id, table_name, id|
    unsupported_media_type unless request.content_type.to_s.split(';').first == 'application/json'
    unsupported_media_type unless request.content_charset.to_s.downcase == 'utf-8'
  ...
  patch %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)$} do |endpoint, app_id, table_name, id|
    call(env.merge('REQUEST_METHOD'=>'POST'))
  end
  put %r{/v3/(shared|user)-tables/([^/]+)/([^/]+)/(\d+)$} do |endpoint, app_id, table_name, id|
    call(env.merge('REQUEST_METHOD'=>'POST'))
  end

  #
  # POST /v3/import-(shared|user)-tables/<app-id>/<table-name>
  #
  # Imports a csv form post into a table, erasing previous contents.
  #
  post %r{/v3/import-(shared|user)-tables/([^/]+)/([^/]+)$} do |endpoint, app_id, table_name|
