...
class AwsS3IntegrationTest < Minitest::Test
  ...
  def test_aws_s3
    ...
    upload_key = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, test_key, test_value, no_random: true)
    assert_equal test_value, Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, test_key)
    ...
    Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, video_key, all_bytes, no_random: true, content_type: 'video/mp4')
    value = Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, video_key)
    ...
    randomized_key = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, key, test_value2)
    assert randomized_key.end_with? key
    assert randomized_key.length > key.length
    assert_equal test_value2, Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, randomized_key)

    test_value3 = random.rand.to_s
    randomized_key2 = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, key, test_value3)
    assert randomized_key2 != randomized_key
    assert_equal test_value3, Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, randomized_key2)

    # Test that it throws a NoSuchKey exception for a non-existent key
    assert_raises(Cdo::AwsWrapper::S3::NoSuchKey) do
      Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, 'nonexistent_key')
    end

    # Test connect_v2!
    client = Cdo::AwsWrapper::S3.connect_v2!
  ...
  def test_s3_presigned_url_upload
    ...
    presigned_url = Cdo::AwsWrapper::S3.presigned_upload_url(TEST_BUCKET, key)
    ...
    downloaded = Cdo::AwsWrapper::S3.download_from_bucket(TEST_BUCKET, key)
  ...
  def test_s3_presigned_url_delete
    ...
    VCR.use_cassette('awss3integration/s3_presigned_url_delete-create') do
      Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, key, test_value, no_random: true)
    end
    ...
    presigned_url = Cdo::AwsWrapper::S3.presigned_delete_url(TEST_BUCKET, key)
    parsed_url = URI.parse(presigned_url)
    Net::HTTP.start(parsed_url.host) do |http|
    ...
    VCR.use_cassette('awss3integration/s3_presigned_url_delete-get_deleted') do
      refute Cdo::AwsWrapper::S3.exists_in_bucket(TEST_BUCKET, key)
    end
  end
  ...
  def test_aws_s3_acl_options
    ...
    public_key = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, 'public_key', 'hello', acl: 'public-read')
    ...
    private_key = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, 'private_key', 'hello')
  ...
  def test_public_url
    assert_equal "https://cdo-temp.s3.amazonaws.com/a/filename.pdf",  Cdo::AwsWrapper::S3.public_url(TEST_BUCKET, 'a/filename.pdf')
  end
  ...
  def test_find_objects_with_ext
    VCR.use_cassette('awss3integration/s3_find_objects_with_ext') do
      test_key_1 = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, 'find_objects/1.test', 'hello', no_random: true)
      test_key_2 = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, 'find_objects/2.test', 'world', no_random: true)
      test_key_3 = Cdo::AwsWrapper::S3.upload_to_bucket(TEST_BUCKET, 'find_objects/3.bad', 'third value', no_random: true)
      results = Cdo::AwsWrapper::S3.find_objects_with_ext(TEST_BUCKET, 'test', 'find_objects')
