...
class NetSimApiTest < Minitest::Unit::TestCase
  ...
  def create_record(record)
    @net_sim_api.post "/v3/netsim/#{@table_name}", record.to_json, 'CONTENT_TYPE' => 'application/json;charset=utf-8'
    @net_sim_api.last_response.location.split('/').last
  end

  def read_records
    @net_sim_api.get "/v3/netsim/#{@table_name}"
    JSON.parse(@net_sim_api.last_response.body)
  end

  def update_record(id, record)
    @net_sim_api.put "/v3/netsim/#{@table_name}/#{id}", record.to_json, 'CONTENT_TYPE' => 'application/json;charset=utf-8'
    JSON.parse(@net_sim_api.last_response.body)
  end

  def delete_record(id)
    @net_sim_api.delete "/v3/netsim/#{@table_name}/#{id}"
  end
