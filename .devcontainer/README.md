...
This is a container you can develop this repo in. It runs Rails, the apps
webpack dev server, the frontend Vite server, the test suites, and the
linters. It is one configuration, usable in a codespace or on your own
machine.
...
| `db` | [cdo-devdb](../.github/workflows/cdo-devdb-image.yml): `mysql:8.0`, carrying both dashboard databases pre-seeded, development with curriculum, test prepared and seeded. You skip `dashboard:setup_db`, and the test seed, on first use. |
| `redis` | `redis:7`, stock. |
...
Create a codespace on your branch. A prebuild speeds up the start. Without
one, the first start also pulls the images and runs both `yarn install`s.
It then downloads the prebuilt apps package and applies the migrations
that are newer than the database image.
...
The apps dev server, Rails, MySQL, and a test worker all run at once.

## On your own machine

This uses the same `devcontainer.json` and the same compose file. Give the
container a clone of its own. Use either VS Code's "Dev Containers: Clone
Repository in Container Volume", or a directory you use for nothing else.
Then open it:
...
Or use "Reopen in Container" from VS Code instead. Plain compose works too.
Reach for it when you want the stack without the devcontainer tooling:
...
Plain compose starts the containers and nothing else. It has no lifecycle
hooks to run the two halves for you. `dev-bootstrap start` is the one
command you owe it, on every start. Run `dev-bootstrap create` too, the
first time, if you want the git hooks and the apps package.
...
.devcontainer/preseed-db.sh                          # seeds a database
docker build -f .devcontainer/Dockerfile.db -t cdo-devdb:local .devcontainer
```

```
# .devcontainer/.env
# compose reads this file automatically. It is gitignored.
CDO_DEV_IMAGE=cdo-dev:local
CDO_DEV_DB_IMAGE=cdo-devdb:local
```

This lists everything compose reads, in one place. All of it is optional,
and all of it comes from `.devcontainer/.env`:
...
have, not from GitHub. Git hardlinks the objects, so the clone avoids a
full fetch:
...
Pointing the container at a checkout you also build natively works. The two
will then fight over the build outputs they share. See [Sharing a checkout
...
one, naming a database this container cannot reach. The container does not
...
loopback network. The name says so, so nobody has to decide whether a
value that looks like a credential is one. These values differ from
`docker/developers/`. That is deliberate. This stack talks only to itself.
...
work, and their ports are forwarded. Run `yarn storybook` in `apps/` for
port 9001, and in `frontend/apps/design-system-storybook/` for port 6006.

`bin/mysql-client-dashboard-{reader,writer}` work as they do natively. The
mounted config gives them the `db` service and its credentials. The mysql
client comes from the base image. They take SQL as a positional argument,
not behind `-e`.
...
The test database arrives prepared and seeded. Schema is loaded, fixtures
are in, and `seed:test` has already run. A test command therefore costs a
Spring boot and nothing else.

Three things to know before the first run:

- Dashboard controller tests need precompiled test assets. Spring caches
  the manifest it saw at boot. Do both steps below, in this order:
  ...
  asset pipeline", against a manifest that is no longer on disk. This step
  is not automated in `dev-bootstrap`. It only matters if you run
  dashboard controller tests. Assets are workspace files, not database
  rows. Unlike the test database, no image can carry them.
...
- `pre-commit` lints **staged files only**. With an empty index it does
  nothing and exits 0, which is not a passing lint run. Run `git add`
  first.

The first `spring` command in a fresh container sometimes dies with a
`mutex_m` default-gem conflict. It can wall `bin/rails` too.
`bundle exec spring stop` sometimes clears it. `DISABLE_SPRING=1` always
does. That is the escape hatch to reach for when a Spring-backed command
will not start at all.

### Developing in apps/

The container starts with `use_my_apps: false`. This serves the prebuilt
package `dev-bootstrap create` downloaded. Level pages render, but nothing
you edit under `apps/src` reaches the browser. To change that, on the
host, take your own config and flip the flag:
...
serve to Rails. It reloads on save.


## Paths less traveled

These routines work here, but they are not set up for you. Each has one
caveat, because each has exactly one thing that bites.
...
your locals copy and run `bin/restart-active-job-workers`. Its pid files
live in `dashboard/tmp/pids` on the mount, so they outlive the container.
After a restart the stale-pid check aborts. Deleting them is the fix.

**The in-browser error console.** `better_errors` and `web-console` only
answer requests from 127.0.0.1. Requests reach this container from the
compose bridge instead. The REPL on the error page therefore does not
render, silently. The stack trace still does. Widen the permitted IPs in a
local initializer if you need the console.
...
**Playwright firefox and webkit** are not in the image. Only chromium is,
which is what the suite runs by default. `yarn exec playwright install
--with-deps firefox webkit` installs them in a running container (sudo is
available) when you are chasing an engine-specific failure. They do not
survive a rebuild.
...
| pegasus serving and tests | This is deprecated. It also needs `pdftk` and `enscript`, neither of which is in the image. |
| Selenium UI tests, Karma | Out of scope for this container. |

## Sharing a checkout with a native setup

This applies only if you took the at-your-own-risk option above. A
codespace, a clone-in-volume, or any clone the container has to itself has
none of this.

The container and the host are then the same directory. The build outputs
...
| `locals.yml` | Not shared. That is deliberate. See Config. |
| `dashboard/public/assets` | Shared. A native precompile's manifest does not match the container's. Symptom: controller tests erroring with an asset "is not present in the asset pipeline". Fix by precompiling in the container, `RAILS_ENV=test bundle exec rake assets:precompile`. That then makes the host's stale in the same way. |
| `dashboard/public/apps-package` | `dev-bootstrap create` downloads the prebuilt package over it whenever `blockly` does not resolve. It logs the commit_hash it replaced and the one it installed. The host's build is gone either way. |
...
pre-seeded datadir. `dev-bootstrap start` replays the missing migrations.
That costs some replay time, not a full reseed. Pre-seed again when the
replay stops being cheap.

### Caches

yarn's global state is a named volume. It is the one cache that lives
outside the checkout. Without it, the frontend workspaces refetch
everything every time the container is recreated. `apps/` keeps its cache
in the workspace (`enableGlobalCache: false`), and turbo keeps its own
under `frontend/.turbo`. Those two persist with the checkout and need
nothing.

The volume is named by compose, which prefixes it with the project. That
gives one cache per clone, not one per machine. This is deliberate. A
machine-global name would mean `docker compose down -v` in any unrelated
project deletes the cache this one just filled. Plain `down` keeps it.
`down -v` here deletes it along with the database and the emulated S3.
...
`CDO_APPS_STORYBOOK_PORT`, `CDO_DS_STORYBOOK_PORT`, `CDO_DB_PORT`. Use these
if you also run any of these services natively. A port already bound on
the host stops the container from starting at all, not just that one
service.
...
- A dev apps build emits `en_us` only, so anything driving a lab page in
  another language fails locally. Watch for `tests/foundations/i18n.spec.ts`.
...
- The apps package `dev-bootstrap create` downloads is keyed on the content of
  ...
  package to download. Level pages then need a local `yarn build`, or the
