...
class I18n::Resources::Apps::Animations::SyncOutTest < Minitest::Test
  ...
  def test_execution
    PegasusLanguages.stub(:get_crowdin_name_and_locale, [{crowdin_name_s: 'Not English', locale_s: 'not-EN'}]) do
      ...
      expected_crowdin_sprintlab_file_path = CDO.dir('i18n/locales/Not English/animations/spritelab_animation_library.json')
      expected_i18n_sprintlab_file_path = CDO.dir('i18n/locales/not-EN/animations/spritelab_animation_library.json')

      File.expects(:exist?).with(expected_crowdin_sprintlab_file_path).in_sequence(exec_seq).returns(true)

      FileUtils.expects(:mv).with(expected_crowdin_sprintlab_file_path, expected_i18n_sprintlab_file_path, force: true).in_sequence(exec_seq).returns(true)
      ...
      JSON.expects(:load_file).with(expected_i18n_sprintlab_file_path).in_sequence(exec_seq).returns('expected_translations')
  ...
  def test_execution_for_us_english_locale
    PegasusLanguages.stub(:get_crowdin_name_and_locale, [{crowdin_name_s: 'English', locale_s: 'en-US'}]) do
      ...
      expected_crowdin_sprintlab_file_path = CDO.dir('i18n/locales/English/animations/spritelab_animation_library.json')
      expected_i18n_sprintlab_file_path = CDO.dir('i18n/locales/en-US/animations/spritelab_animation_library.json')
      ...
      JSON.expects(:load_file).with(expected_i18n_sprintlab_file_path).never.returns('expected_translations')
      expected_manifest_builder = stub
      ManifestBuilder.expects(:new).with({spritelab: true, upload_to_s3: true, quiet: true}).never.returns(expected_manifest_builder)
  ...
  def test_execution_with_non_existing_crowdin_sprintlab_file
    PegasusLanguages.stub(:get_crowdin_name_and_locale, [{crowdin_name_s: 'Not English', locale_s: 'not-EN'}]) do
      exec_seq = sequence('execution')
      ...
      File.expects(:exist?).with(expected_crowdin_sprintlab_file_path).in_sequence(exec_seq).returns(false)

      FileUtils.expects(:mv).with(expected_crowdin_sprintlab_file_path, expected_i18n_sprintlab_file_path, force: true).never.returns(true)
