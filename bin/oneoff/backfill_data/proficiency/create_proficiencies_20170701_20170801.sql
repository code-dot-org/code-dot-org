/**
 * This query creates a temporary table containing user proficiency data for
 * students working in courses C-F or the express course (A, B, and pre-express
 * are K-1 scripts and therefore not included), limited to a given date range.
 */


CREATE TABLE user_proficiencies_20170701_20170801 AS
SELECT
  user_levels.user_id,
...
WHERE user_levels.best_result = 100
  AND user_levels.script_id IN (238, 239, 240, 241, 258)
  AND user_levels.id > 1055000000
  AND user_levels.id < 1065000000
  AND user_levels.created_at > "2017-07-01"
  AND user_levels.created_at < "2017-08-01"
