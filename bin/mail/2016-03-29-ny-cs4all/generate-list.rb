...
def query_k5_pd_teachers
  section_ids = Set.new
  DB[:forms].where(kind: 'ProfessionalDevelopmentWorkshop').map(:data).each do |form_data|
    section_id = JSON.parse(form_data)['section_id_s']
    section_ids.add(section_id.to_i) unless section_id.blank?
  end
...
all_results = k5_pd_teachers.merge(non_pd_teachers).merge(hoc_organizers_no_code_studio)
