...
def main(options)
  ...
  options.unit_names.each do |unit_name|
    ...
    lessons_without_lesson_plan_to_update = script.lessons.reject(&:has_lesson_plan?).reject {|l| paired_lesson_ids.include?(l.id)}
...
def get_validated_lesson_pairs(script, cb_unit)
  ...
  lessons_with_lesson_plans = script.lessons.select(&:has_lesson_plan?)
