...
def fetch_teacher_evals(unit_name:, level_id:, output_dir:)
  if Rails.env.production?
    filename = 'export_teacher_evals.sql.erb'
    pathname = File.expand_path(filename, __dir__)
    query_template = File.read(pathname)
    params = {
      unit_name: unit_name,
    ...
    puts "Querying redshift using #{filename} with #{params}..."
    execute_redshift_query(client, query)
    puts "Redshift teacher_evals query executed in: #{(Time.now - start_time).round(2)} seconds"
  end
end
