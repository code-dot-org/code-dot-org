...
# @param [Hash] projects Mapping of crowdin project names to their configuration files.
# @param [Boolean] upload_manifests If true, uploads translated sprite animation metadata to S3.
def sync_out(projects, upload_manifests=false)
  puts "Sync out starting"
  rename_from_crowdin_name_to_locale
  restore_redacted_files(projects)
  distribute_translations(upload_manifests, projects)
...
#
# @param [String] locale the locale code to check. This can be either the
...
#
# @param [String] file the path to the file to check. Note that this should be
...
#
# @param [Hash] projects Mapping of crowdin project names to their configuration files.
def file_changed?(locale, file, projects)
  @change_datas ||= projects.keys.map do |crowdin_project|
...
# @param [Hash] projects Mapping of crowdin project names to their configuration files.
def restore_redacted_files(projects)
  ...
  Parallel.each(locales, in_processes: (Parallel.processor_count / 2)) do |prop|
    ...
    original_files.each do |original_path|
      ...
      next unless file_changed?(locale, relative_path, projects)
...
def distribute_course_content(locale, projects)
  locale_strings = {}
...
# @param [Boolean] upload_manifests If true, uploads translated sprite animation metadata to S3.
# @param [Hash] projects Mapping of crowdin project names to their configuration files.
def distribute_translations(upload_manifests, projects)
  locales = Languages.get_locale
  ...
  Parallel.each(locales, in_processes: (Parallel.processor_count / 2)) do |prop|
    ...
    ### Course Content
    distribute_course_content(locale, projects)
    ...
    if file_changed?(locale, spritelab_animation_translation_path, projects)
...
sync_out(CROWDIN_PROJECTS) if __FILE__ == $0
