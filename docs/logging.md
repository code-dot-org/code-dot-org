...
- CloudFront [logs](#webcdn) the request/response; if the WebACL blocks it, the WAF log [records](../aws/cloudformation/data.yml.erb#L662-L715) the decision.
...
- Browser‑side interaction events are batched and [written](#cloudwatch-logs) to the `<env>-browser-events` CloudWatch log group; the page may also report New Relic page actions/errors.
- Client analytics may log to Statsig (back‑end via server SDK; front‑end via app code) and historically to Firehose (deprecated).
