...
- On the instance, NGINX [writes](#application-servers-ec2) an access line and proxies to Puma; Rails (via Lograge in prod/staging) [emits](./log-formats.md#rails-application-logs-lograge-cee-json) a condensed JSON line to syslog/CloudWatch for the controller action rendering the level page.
- Database queries executed to render the page (ActiveRecord) are included in the Rails Lograge timings; notable DB activity may also [emit](#cloudwatch-logs) to Aurora’s CloudWatch export groups (slow/error/general/audit).
- Any server‑side exceptions during page render [notify](../lib/cdo/honeybadger.rb#L27-L74) Honeybadger and are reflected in Rails Lograge JSON and syslog.

Secondly, while the user is on the page (in addition to the above):

- Browser‑side interaction events are batched and [written](#cloudwatch-logs) to the `<env>-browser-events` CloudWatch log group; the page may also [report](../apps/src/logToCloud.js#L31-L41) New Relic page actions/errors.
- Client analytics may log to [Statsig](../dashboard/lib/metrics/events.rb) (back‑end via server SDK; front‑end via app code) and historically to [Firehose](../lib/cdo/firehose.rb#L71-L91) (deprecated).
- Subsequent XHR/API calls generated by the page result in additional Rails Lograge request lines; any exceptions [notify](../lib/cdo/honeybadger.rb#L27-L74) Honeybadger.
- Background jobs (e.g., ActiveJob) kicked off from user actions log via the same Rails logger and appear in syslog/CloudWatch.
