 ...
 * level. A lab fixture's `appOptions` is served ahead of this by the dispatcher;
 * this fall-through lets a lab load (via `loadLab` in LabHost) without Rails
 * when no fixture is registered — without it, LabHost's `useAppOptions`
 * (`throwOnError`) fails against no backend and the lab never mounts.
 */
