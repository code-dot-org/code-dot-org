...
export default class Neighborhood {
  ...
  processSignals() {
    ...
    if (this.signals.length > this.nextSignalIndex) {
      ...
      if (signal.value === NeighborhoodSignalTypes.DONE) {
      ...
      if (signal.value === ConsoleSignalTypes.CONSOLE_LOG) {
        this.onOutputMessage(signal.detail);
      } else if (signal.value === ConsoleSignalTypes.PARTIAL_LOG) {
  ...
  mazeCommand(signal: NeighborhoodSignal, timeForSignal: number) {
    switch (signal.value) {
      case NeighborhoodSignalTypes.MOVE: {
      ...
      case NeighborhoodSignalTypes.INITIALIZE_PAINTER: {
      ...
      case NeighborhoodSignalTypes.TAKE_PAINT: {
        const {id} = signal.detail!;
        this.controller!.subtype.takePaint(id);
        break;
      }
      case NeighborhoodSignalTypes.PAINT: {
        const {id, color} = signal.detail!;
        this.controller!.subtype.addPaint(id, color);
        break;
      }
      case NeighborhoodSignalTypes.REMOVE_PAINT: {
        const {id} = signal.detail!;
        this.controller!.subtype.removePaint(id);
        break;
      }
      case NeighborhoodSignalTypes.TURN_LEFT: {
        const {id} = signal.detail!;
        this.controller!.subtype.turnLeft(id);
        break;
      }
      case NeighborhoodSignalTypes.SHOW_PAINTER: {
        const {id} = signal.detail!;
        this.controller!.showPegman(id);
        break;
      }
      case NeighborhoodSignalTypes.HIDE_PAINTER: {
        const {id} = signal.detail!;
        this.controller!.hidePegman(id);
        break;
      }
      case NeighborhoodSignalTypes.SHOW_BUCKETS: {
        this.controller!.subtype.setBucketVisibility(true);
        break;
      }
      case NeighborhoodSignalTypes.HIDE_BUCKETS: {
  ...
  onClose() {
    ...
    this.signals.push({value: NeighborhoodSignalTypes.DONE});
  }
