...
describe('ParentGuardianEmail', () => {
  ...
  // The section still hands the address on file to the modal — not to prefill
  // it, but so the modal can reject retyping the same address.
  it('gives the modal the address on file, which rejects retyping it', async () => {
    renderSection();
    fireEvent.click(
      screen.getByRole('button', {name: 'Update parent/guardian email'}),
    );
    const dialog = await screen.findByRole('dialog');
    const address = within(dialog).getByRole('textbox', {
      name: /^Parent\/guardian email address/,
    });
    expect(address).toHaveValue('');

    fireEvent.change(address, {target: {value: 'parent@example.com'}});
    expect(
      within(dialog).getByText(
        'New email address must not match old email address.',
      ),
    ).toBeInTheDocument();
  });
  ...
  it('blocks submit and flags a mismatch when the two emails differ', async () => {
    ...
    // Update is disabled while the pair differs, so the click is inert and the
    // mismatch is already on screen.
    const submit = within(dialog).getByRole('button', {name: 'Update'});
    expect(submit).toBeDisabled();
    fireEvent.click(submit);

    expect(
      await within(dialog).findByText('Email addresses must match.'),
    ).toBeInTheDocument();
