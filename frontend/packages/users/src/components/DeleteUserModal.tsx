  ...
  return (
    ...
    >
      ...
      {dependedUponForLogin && (
        <DialogContentText>
          Deleting your account will permanently erase all personal information,
          coursework, projects, and professional learning information connected
          to this account after 28 days.{' '}
          <strong>
      ...
      {dependedUponForLogin ? (
        <>
          {ACK_IDS.map(id => (
            <FormControlLabel
              key={id}
              // mx:0 — FormDialog stretches content to width:100%, and
              // MuiFormControlLabel's default -11px/+16px margins then overflow
              // the content box (x-scroll).
              sx={{mx: 0}}
              control={
                <Checkbox
                  checked={acks[id]}
