...
const VersionHistoryPanel: FunctionComponent<VersionHistoryPanelProps> = ({
  ...
  const previewViewAsUserId = useRef<number | undefined>(viewAsUserId);
  ...
  const restoreSelectedVersion = useCallback(() => {
    ...
    if (selectedVersion === INITIAL_VERSION_ID) {
      sendLabAnalyticsEvent('Version restored on lab2', {
  ...
  const onVersionChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (!isLatest) {
        sendLabAnalyticsEvent('Version viewed on lab2', {
  ...
  const handleSaveVersionSuccess = useCallback(() => {
    sendLabAnalyticsEvent('Version committed on lab2', {
