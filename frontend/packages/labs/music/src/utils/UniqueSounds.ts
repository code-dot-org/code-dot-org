...
class UniqueSounds {
  private previousUniqueSounds?: string[];
   ...
   * the same row.
   */
  getUniqueSounds(songDataEvents: PlaybackEvent[]): string[] {
    // First, generate a list of all current unique sounds.
    const currentUniqueSounds: string[] = [];
    ...
    if (this.previousUniqueSounds) {
      ...
      for (const currentUniqueSound of currentUniqueSounds) {
        const previousIndex =
