...
class Driver extends (EventEmitter as unknown as new () => TypedEmitter<DriverEvents>) {
  ...
  onBlockEvent(e: Blockly.Events.Abstract) {
    ...
    if (e.type === Blockly.Events.FINISHED_LOADING) {
      ...
      // Adjust the position of any overlapping blocks, including immovable top
      // blocks. Use positionBlocksOnWorkspace rather than Blockly's cleanUp():
      // cleanUp() columns every top block at the top-left corner, clobbering the
      // start-block corner offset, whereas this only nudges edge/overlapping
      // blocks and preserves the offset.
      serialization.positionBlocksOnWorkspace(this.workspace);
    }
    ...
    return this.player.preloadSounds(events, (loadTimeMs, soundsLoaded) => {
      ...
      if (soundsLoaded > 0) {
        LabRegistry.metricsReporter.reportLoadTime(
          ...
          loadTimeMs,
          {
            loadType: this.hasLoadedInitialSounds ? 'Subsequent' : 'Initial',
          },
        );
      }
