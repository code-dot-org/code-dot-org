...
export interface PlayMultiMutatorState {
  extraSoundInputCount: number;
}

export const playMultiMutator = defineMutator(PLAY_MULTI_MUTATOR, {
  ...
  loadExtraState: function (state: PlayMultiMutatorState) {
  ...
  addSound_: function () {
    ...
    this.appendValueInput(EXTRA_SOUND_INPUT_PREFIX + this.extraSoundInputCount_)
      .setShadowDom(Blockly.utils.xml.textToDom(shadowBlockXml))
  ...
  showHidePlus_: function (shouldShow: boolean) {
    ...
    if (shouldShow) {
      ...
      (this.getField('PLUS') as Blockly.FieldImage).setOnClickHandler(
        fieldImage => (fieldImage?.getSourceBlock() as typeof this)?.plus(),
      );
    }
  },
  showHideMinus_: function (shouldShow: boolean) {
    ...
    if (shouldShow) {
      ...
      (this.getField('MINUS') as Blockly.FieldImage).setOnClickHandler(
        fieldImage => (fieldImage.getSourceBlock() as typeof this)?.minus(),
      );
    }
  },
  plus: function () {
