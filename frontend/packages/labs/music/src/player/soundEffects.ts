...
class SoundEffects {
  ...
  /** The time to delay the effect */
  private delayTimeSeconds: number;
  /** Busses */
  private busses: {
    [key: string]: AudioNode;
  };

  constructor(audioContext: AudioContext, delayTimeSeconds: number) {
  ...
  generateKeyString(effects: Effects) {
    const keyStrings = [];
  ...
  generateBus(effects: Effects): {
    ...
    if (effects.filter && ['low', 'medium'].includes(effects.filter)) {
    ...
    if (effects.delay && ['low', 'medium'].includes(effects.delay)) {
  ...
  insertEffects(effects: Effects, lastNode?: AudioNode) {
    // Volume is inserted per sound, so possibly add it here.
    if (effects.volume && ['low', 'medium'].includes(effects.volume)) {
