...
import {LabRegistry, LabMetricsReporter} from '@code-dot-org/lab-classic';
 ...
 * outside of Music Lab, check {@link packages/labs/base/src/LabMetricsReporter}.
 */
export default class LabMusicMetricsReporter extends LabMetricsReporter {
  ...
  async startSession() {
    ...
    this.incrementCounter('Music Session Start');
  ...
  onPackSelected(packId: string) {
    ...
    this.incrementCounter('Music Pack Selected', {value: packId});
  }
  ...
  private trackUIEvent(eventType: string, payload: object = {}) {
    ...
    this.incrementCounter(eventType, sessionPayloadToMetricsPayload(payload));
  }
  ...
  endSession() {
    ...
    this.incrementCounter(
      'Session end',
      sessionPayloadToMetricsPayload(payload),
    );
