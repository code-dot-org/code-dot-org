...
import type {
  ChordEventValue,
  PlayStyle,
} from '../../player/interfaces/ChordEvent';
...
const ChordPanel: React.FunctionComponent<ChordPanelProps> = ({
  ...
  const [musicLibrary, setMusicLibrary] = useState<MusicLibrary | undefined>(
    MusicLibrary.getInstance(),
  );

  useEffect(() => {
    (async () => {
      setMusicLibrary(await MusicLibrary.libraryLoader);
    })();
  }, [setMusicLibrary]);

  const instruments: [string, string][] = useMemo(
    () =>
      musicLibrary?.instruments.map(folder => [folder.name, folder.id]) || [],
    [musicLibrary],
  );
  ...
  useEffect(() => {
    if (musicLibrary && !MusicRegistry.player.isInstrumentLoaded(instrument)) {
      setIsLoading(true);
  ...
  }, [setIsLoading, musicLibrary, instrument]);
