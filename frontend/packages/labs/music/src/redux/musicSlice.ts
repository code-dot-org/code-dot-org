...
export const InstructionsPosition = {
  LEFT: 'LEFT',
  RIGHT: 'RIGHT',
} as const;
...
export interface MusicState {
  ...
  instructionsPosition: keyof typeof InstructionsPosition;
...
const musicSlice = createSlice({
  ...
  reducers: {
    ...
    setInstructionsPosition: (
      state,
      action: PayloadAction<keyof typeof InstructionsPosition>,
    ) => {
...
export const getBlockMode = (state: RootState): ValueOf<typeof BlockMode> => {
  ...
  return (
    ...
    (levelProperties as MusicLevelProperties | undefined)?.levelData
      ?.blockMode ||
    BlockMode.SIMPLE2
  );
};
