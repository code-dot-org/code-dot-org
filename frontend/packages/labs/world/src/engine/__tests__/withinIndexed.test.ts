...
//
// The neighbourhood query measured every actor against every centre. Written
...
// neighbourhood is a set, which the header argues at length.
...
/** Who is within `reach` of `centres`, worked out the long way. */
const byHand = (world: World, centres: string[], reach: number) => {
  const at = (id: string) =>
    [...world.actors].find(a => a.id === id)!.get(PositionProperty);
  return [...world.actors]
    .filter(
      actor =>
        !centres.includes(actor.id) &&
        centres.some(centre => {
          const c = at(centre);
...
describe('the neighbourhood of an actor', () => {
  it('is the same set the long way round, over and over', () => {
    const world = scatter(150);
    const actors = [...world.actors];

    for (const centre of actors.slice(0, 40)) {
      for (const reach of [0, 30, 80, 400]) {
        expect(
          idsOf(within(world.actors, centre, reach)),
          `${centre.id} within ${reach}`,
        ).toEqual(byHand(world, [centre.id], reach));
      }
    }
  });

  it('means near ANY of several, and each of them only once', () => {
    // Two centres whose neighbourhoods overlap: an actor near both is in the
    // answer once, and neither centre is in it at all.
    const world = scatter(80);
    const actors = [...world.actors];
    const centres = [actors[0], actors[1], actors[2]];

    expect(idsOf(within(world.actors, centres, 200))).toEqual(
      byHand(
        world,
        centres.map(c => c.id),
        200,
  ...
  it('finds nothing for a reach that is not a distance', () => {
    ...
    const centre = [...world.actors][0];

    expect([...within(world.actors, centre, -1)]).toEqual([]);
    expect([...within(world.actors, centre, Number.NaN)]).toEqual([]);
  });
  ...
  it('answers about the SOURCE when the source is not the world', () => {
    ...
    const actors = [...world.actors];
    const centre = actors[0];
    const few = actors.slice(1, 10);

    const answer = idsOf(within(few, centre, 500));
    ...
    expect(answer).toEqual(
      few
        .filter(
          a =>
            Math.hypot(
              a.get(PositionProperty).x - centre.get(PositionProperty).x,
              a.get(PositionProperty).y - centre.get(PositionProperty).y,
            ) <= 500,
        )
        .map(a => a.id)
        .sort(),
  ...
  it('stops early, so `take ⟨3⟩ of` does not measure the world', () => {
    ...
    const iterator = within(world.actors, centre, 500)[Symbol.iterator]();
