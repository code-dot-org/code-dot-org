// Colours in, shader colours out.
//
// Two conventions meet here and neither is going to move. Every colour picker a
...
// produces a colour can feed an effect parameter, not just the one we shipped.
//
...
/** A colour as a shader wants it: three floats, 0–1. */
...
/** What a colour socket can hand over: a hex string, or floats already. */
export type ColourValue = string | readonly number[];

/**
 * Parse a colour into four 0–1 floats, alpha last.
 *
 * Accepts `#rgb`, `#rrggbb`, `#rrggbbaa` and a float array. Hex without an
 * alpha means opaque — a picker has no way to say otherwise, and a colour a
 * learner chose from a swatch is a colour they expect to see.
 *
 ...
 */
function channels(value: ColourValue): Rgba {
...
/** A colour as three 0–1 floats. Any alpha the value carries is dropped. */
export function rgb(value: ColourValue): Rgb {
  const [r, g, b] = channels(value);
  return [r, g, b];
}

/** The same with alpha, which defaults to opaque when the value omits one. */
export function rgba(value: ColourValue): Rgba {
 ...
 *
 * Needed because an effect declares its default colour the shader way, and the
 * picker standing in for it on the block needs a hex swatch to show.
 */
export function toHex(colour: readonly number[]): string {
  const channel = (index: number): string =>
    Math.round(clamp01(Number(colour?.[index] ?? 0)) * 255)
      .toString(16)
      .padStart(2, '0');
