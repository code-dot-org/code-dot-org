...
describe('Health Bar', () => {
  // The assembly, and the only thing worth checking about it: that three rules
  // written without knowing about each other add up to a bar that follows
  // somebody's health. Health has the number, Progress has the fraction, and
  // Attachment has the actor — this is the sentence saying they are one thing.

  /** A subject with health, and a bar pointed at it. */
  const bar = (full = 3, left = 3) => {
    const world = new WorldBuilder({id: 'w', name: 'W'})
      .useRules([
        rule('rules/collisions'),
        rule('rules/health'),
        rule('rules/progress'),
        rule('rules/attachment'),
        rule('rules/healthBar'),
      ])
      .instantiate();
    const subject = new ActorBuilder({id: 'subject', name: 'subject'})
      .useTraits([of('rules/health', 'HasHealthTrait')])
      .set(of('rules/health', 'MostHealthProperty'), full)
      .set(of('rules/health', 'HealthProperty'), left)
      .set(PositionProperty, at(100, 100))
      .instantiate('subject');
    world.addActor(subject);
    const meter = new ActorBuilder({id: 'meter', name: 'meter'})
      .useTraits([
        of('rules/progress', 'ShowsProgressTrait'),
        of('rules/attachment', 'AttachedTrait'),
        of('rules/healthBar', 'ShowsHealthTrait'),
      ])
      .set(PositionProperty, at(0, 0))
      .instantiate('meter');
    world.addActor(meter);
    return {world, subject, meter};
  };
  const shown = (meter: unknown) =>
    (meter as {get(p: unknown): number}).get(
      of('rules/progress', 'FractionProperty'),
    );

  it('is one line to point, and does both jobs', () => {
    // `attached to` is the whole of the wiring: it says where the bar sits AND
    // whose health it shows. Two properties for one intention would be two
    // chances to disagree — a bar over one thing reporting another.
    const {world, subject, meter} = bar(4, 3);
    meter.set(of('rules/attachment', 'AttachedToProperty'), subject as never);

    run(world, 0.2);

    expect(shown(meter)).toBe(0.75);
    expect((meter as {get(p: unknown): Vector}).get(PositionProperty).y).toBe(
      100 - 24,
    );
  });

  it('follows the health down as it is taken', () => {
    const {world, subject, meter} = bar();
    meter.set(of('rules/attachment', 'AttachedToProperty'), subject as never);
    run(world, 0.1);
    const before = shown(meter);

    subject.act(of('rules/health', 'TakeDamageAction'), 1);
    run(world, 0.1);

    expect(before).toBe(1);
    expect(shown(meter)).toBeCloseTo(2 / 3, 5);
  });

  it('leaves its fraction alone while it is pointed at nobody', () => {
    // A bar nothing has attached shows what it was given, like any other
    // progress bar — rather than reading an empty property and going to zero,
    // which looks like the rule working and reads as it failing.
    const {world, meter} = bar();

    run(world, 0.2);

    expect(shown(meter)).toBe(1);
  });

  it('shows empty rather than infinity when full is nothing', () => {
    // `most health` of zero is a thing a learner can write, and a bar showing
    // Infinity is worse than one showing empty.
    const {world, subject, meter} = bar(0, 0);
    meter.set(of('rules/attachment', 'AttachedToProperty'), subject as never);

    run(world, 0.2);

    expect(shown(meter)).toBe(0);
  });
});
