...
export function WorldRuntimeProvider({children}: {children: ReactNode}) {
  ...
  const ownProperties = useMemo(
    () => projectOwnMetas(projectFiles(currentSources.source)),
    [currentSources],
  );
  ...
  const getActorInfo = async (
    ...
    const pair = managers.current;
    // No guard on `actorPaths` being empty, which this used to have: a project
    // with no actor files has nothing to introspect and the manifest says so.
