...
// `define action` / `define query` carry an imperative `do` body — unlike the
// declarative blocks above, that body IS real code (the standard set/get/if/
// for-each/return blocks), generated by the headless generator, not read
// statically. So these blocks still declare no generator (the metadata + the
// body extraction handle them); their bodies' blocks carry their own generators.
...
const worldRuleAction = defineBlock({
  ...
  tooltip:
    ...
    'or an actor (inside a "define trait"). Its steps go in "do".',
  generator: noGenerator,
});

// A query REPORTS a value; `TYPE` is what it reports (the reporter's output
// socket), mirroring `define property`'s type dropdown. Its `do` is a full
// statement body ending in a `return`.
const worldRuleQuery = defineBlock({
  type: 'world_rule_query',
  message0: 'define %1 query %2',
  args0: [
    {type: 'field_dropdown', name: 'TYPE', options: QUERY_RETURN_TYPE_OPTIONS},
    {type: 'field_input', name: 'NAME', text: 'some value'},
  ],
  message1: 'do %1',
  args1: [{type: 'input_statement', name: 'DO'}],
  previousStatement: true,
