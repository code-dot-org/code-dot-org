...
| Tile                 | The lesson                                                                                                 | Unlocks                     | Check                                                                                        |
| -------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------- |
| `platformer/jump`    | Up. Then the two things that make a jump feel right: a moment of grace after the ledge, and a second jump. | Jumping rule                | `[o]` a press from the ground raises y; a press in mid-air does not, until double jump is on |
| `platformer/ground`  | Floors, walls, and a platform that moves under your feet.                                                  | Patrol rule                 | `[o]` the player rides the moving platform                                                   |
| `platformer/hazards` | Something that can hurt you, and a rule that does not know who it hurts.                                   | Health rule; `Deals Damage` | `[t]` health fell once per touch, not once per frame                                         |
| `platformer/pickups` | Coins, and the count of them.                                                                              | Collection rule; Coin actor | `[o]` picking up three coins leaves three counted and none in the world                      |
| `platformer/level`   | A level with a start, a route and an end. Camera, map, hazards, a goal.                                    | the platformer template     | `[o]` the scripted run reaches the goal and the win state is entered                         |
...
| `making/change`   | Arcade     | Change a number in it, and meet the fact that catches everybody: your project has its OWN copy.      | `each frame` in a `.rule`   | `[o]` the behaviour differs from stock in the way the edit predicts |
| `making/property` | Puzzle     | Add a property to a rule and watch two blocks appear in its category.                                | `define event`              | `[o]` the property is on the built actor and its blocks are used    |
...
| `making/trait`    | Simulation | One rule, two traits, and the actors that elect one each.                                            | `define trait`, `use trait` | `[o]` two kinds carry different traits from one rule                |
