...
7. **Real completion.** The assertion channel is **done**; the checks are
   ...
   **Every authored lesson has a check** — fifty-two of the sixty-seven tiles
...
trait` opens the rule behind the trait; the rule count on `define world`
   ...
   **Three lessons keep the browser, and each earns it.**
...
sprite` the way there is on `use trait`, so with no list there is no way in.
   ...
   `making/property` is the third and needs no argument: a Making lesson is
   spent inside a `.rule`, and a rule is a file.
