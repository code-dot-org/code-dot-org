...
// two sides and a game names them separately. A player, a crate and a boss
// elect "Has Health"; a spike, an energy ball and a patrolling crawler elect
// "Deals Damage".
// Neither has to know about the other, which is what makes "these things are
// dangerous" sayable at all.
//
// This rule exists because two others already led nowhere. Zapping could send
// an energy ball and nothing could be damaged by it; Collisions could say what
// was touching what and the only thing in the library that reacted was
// Collection. "Thing meets thing, thing loses, thing runs out" is the loop
// under most action games, and it was the one thing every project had to write
// by hand.
//
// IT DOES NOT REMOVE ANYTHING. Running out raises \`runs out of health\` and
// stops there — what that means is the game's business, and games disagree: remove
...
// a crawler for half a second is sixty frames, and taking sixty damage is not
// what anybody means. So a damaged actor cannot be damaged AGAIN until its
// mercy time has passed — which gives all three cases the one mechanism
// handles: an energy ball lands once and is gone, a crawler you are pressed
// against takes a readable amount, and standing in lava keeps costing for as
...
/** How much more damage this can take. At zero it has run out. */
...
/**
 * Seconds of not being damaged again after being damaged.
 *
...
/**
 * The world time this can be damaged again at.
 *
 ...
 * so an actor can be damaged from the first frame without a sentinel.
 */
const safeUntil = hurtable.number('safe until', 0, {readonly: true});

/** How much this takes off, when it touches something that has health. */
const damage = dangerous.number('damage', 1);

/** `when ⟨Player⟩ is damaged` — raised however the damage arrived. */
const isDamaged = hurtable.event(['is damaged']);

/**
 * `when ⟨Player⟩ runs out of health` — raised once, when the last of it goes.
 *
 * SAID THAT WAY ROUND, rather than `dies`. What running out MEANS is the
 * game's to decide — a life lost, a level ended, a crate broken open — and an
 * event that named one of those would be deciding for it. It also keeps the
 * library's words plain: this is a number reaching zero.
 */
const runsOut = hurtable.event(['runs out of health']);

/** `when ⟨Spike⟩ damages ⟨Player⟩` — the dangerous thing's side. */
 ...
 *
 * One place, so that health cannot go negative, `runs out of health` cannot be
 * raised twice, and a fall or a trap costs by exactly the same route a spike
 * does. The step below calls this too.
 *
...
const takeDamage = hurtable.block({
  returns: 'none',
  description:
    'Take damage. Raises “is damaged”, and “runs out of health” if that was the last of it.',
  say: ['take', param('amount', 'number'), 'damage'],
  body: ({amount}) => [
    note('Already empty? Nothing to take, and the event has been said once.'),
    when([
      [
        ...
        [
          ...
          isDamaged({}, thisActor()),
          when([
            [atMost(health.of(thisActor()), n(0)), [runsOut({}, thisActor())]],
          ]),
        ],
      ],
    ]),
...
hurtable.step('take contact damage', 'react', [
  ...
  forEach(source, {
    from: filter(source, {
      ...
      where: hasTrait(source.get(), DealsDamage),
    }),
    body: [
      when([
        [
          ...
          [
            ...
            damages({other: thisActor()}, source.get()),
          ],
        ],
      ]),
    ],
