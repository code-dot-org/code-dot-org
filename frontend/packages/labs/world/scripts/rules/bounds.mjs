 ...
 *
 * Zero means "not measured" rather than "no size" — an actor is drawn before
 * anything has said how big its picture is, and one drawn from outside the
 * project may never be measured at all. Falling through to a 32 by 32 square is
 * what `collision size of` does with the same gap.
 */
