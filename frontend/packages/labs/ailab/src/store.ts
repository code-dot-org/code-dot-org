...
export const store = configureStore({
  ...
  middleware: getDefaultMiddleware =>
    getDefaultMiddleware({
      // state.trainedModel is a live model instance from the selected
      // trainer's library, used for predictions, so it (and the action that
      // sets it) are intentionally non-serializable.
