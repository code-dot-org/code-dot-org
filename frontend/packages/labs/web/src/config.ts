 ...
 */
// Exactly the extensions legacy's `SafeAndSupportedImageTypes` yields
// (image/gif, image/jpeg, image/png, image/webp). "Safe" is the operative word:
// SVG is markup and executes script, so it is a stored-XSS vector in a lab that
// serves student uploads back to the browser, and the assets bucket rejects both
// it and .ico anyway (dashboard asset_bucket.rb allows jpg/jpeg/gif/png/webp
// plus audio and documents). Do not widen this without checking that list.
