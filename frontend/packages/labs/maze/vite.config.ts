import fs from 'fs';
import path from 'path';
import type {OutputOptions} from 'rollup';
import {defineConfig} from 'vite';
import type {Plugin} from 'vite';
import react from '@vitejs/plugin-react';
import {externalizeDeps} from 'vite-plugin-externalize-deps';
import dts from 'vite-plugin-dts';
import cssInjectedByJsPlugin from 'vite-plugin-css-injected-by-js';

/**
 * Get Rollup output configuration.
 * @param format es or cjs
 * @returns Rollup output configuration
 */
function getRollupOutputConfig(format: 'es' | 'cjs'): OutputOptions {
  return {
    format,
    exports: 'named',
    entryFileNames: format === 'es' ? '[name].mjs' : '[name].cjs',
    preserveModules: true,
    preserveModulesRoot: 'src',
  };
}

/**
 * Vite plugin that emits static assets as separate files in library mode.
 *
 * In library mode, Vite inlines all static asset imports as base64 regardless
 * of assetsInlineLimit. This plugin intercepts imports matching the given
 * pattern, emits them via Rollup's emitFile API, and returns a reference URL
 * so the built JS points to the emitted file.
 */
function emitStaticAssets(pattern: RegExp): Plugin {
  return {
    name: 'emit-static-assets',
    enforce: 'pre',
    load(id) {
      if (!pattern.test(id)) {
        return null;
      }

      const source = fs.readFileSync(id);
      const basename = path.basename(id);
      const referenceId = this.emitFile({
        type: 'asset',
        name: basename,
        source,
      });

      return `export default import.meta.ROLLUP_FILE_URL_${referenceId};`;
    },
  };
}

// https://vite.dev/config/
export default defineConfig({
  plugins: [
    // Emit skin assets as separate files instead of inlining as base64
    emitStaticAssets(/src\/assets\/skins\/.+\.(png|gif|mp3|ogg)$/),
    // Enable React support
    react(),
    // Generate Typescript declaration files using the Vite default tsconfig
    dts({tsconfigPath: './tsconfig.app.json'}),
    // Inject CSS directly into JS bundle as inline styles so CSS loads
    // automatically when a module is imported. `relativeCSSInjection: true`
    // is required for this preserveModules multi-entry build — see
    // music-lab's vite.config.ts for why the default global mode drops
    // styles from every entry but the last.
    cssInjectedByJsPlugin({relativeCSSInjection: true}),
    // Ensure dependencies are externalized for library build
    // Libraries such as react, react-dom, lodash, etc. should not be bundled by the library.
    // Instead, they are expected to be provided by the host application.
    externalizeDeps({
      useFile: path.resolve(__dirname, 'package.json'),
    }),
  ],
  resolve: {
    // Dedupe React and Redux packages to ensure only one instance is used across all packages.
    // This prevents "older version of React" errors when workspace packages
    // have different React versions in their devDependencies.
    // Also dedupe Redux packages to ensure the store singleton is shared across all packages.
    dedupe: [
      'blockly',
      'react',
      'react-dom',
      'react/jsx-runtime',
      'react/jsx-dev-runtime',
      'react-redux',
      '@reduxjs/toolkit',
    ],
    // Force workspace packages with CSS to use dist files so CSS imports are resolved correctly.
    // Source files import './index.css' which doesn't exist - CSS is only generated during build.
    alias: {
      // Allow direct SCSS imports from source (must come before the general @code-dot-org/lab-classic alias)
      '@code-dot-org/lab-classic/styles/variables.scss': path.resolve(
        __dirname,
        '../classic/src/components/layout/variables.scss',
      ),
    },
  },
  optimizeDeps: {
    // Force Vite to pre-bundle React from the root to ensure a single instance
    include: ['react', 'react-dom'],
  },
  css: {
    // Enable CSS modules for .module.scss files
    modules: {
      localsConvention: 'camelCase',
    },
  },
  worker: {
    format: 'es',
    rollupOptions: {
      output: {
        inlineDynamicImports: true,
        exports: 'auto',
      },
    },
  },
  server: {
    allowedHosts: ['localhost', 'localhost-studio.code.org'],
  },
  build: {
    sourcemap: true,
    cssCodeSplit: true,
    lib: {
      entry: ['src/App.tsx'],
      name: 'maze-lab',
      formats: ['es', 'cjs'],
    },
    rollupOptions: {
      output: [getRollupOutputConfig('es'), getRollupOutputConfig('cjs')].map(
        output => ({
          ...output,
          assetFileNames: 'assets/[name]-[hash][extname]',
        }),
      ),
    },
  },
});
