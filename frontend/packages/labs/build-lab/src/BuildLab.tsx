...
import {useBuildLabPersistence} from './projectPersistence';
...
export default function BuildlabView({
  ...
  const [elements, setElements] = useState(DEFAULT_PROJECT.elements);
  const [selectedElementId, setSelectedElementId] = useState('button1');
  const [screens, setScreens] = useState(DEFAULT_PROJECT.screens);
  const [mlModels, setMlModels] = useState<ImportedMlModel[]>(
    DEFAULT_PROJECT.mlModels ?? [],
  );
  ...
  const [workspaceState, setWorkspaceState] = useState<BuildlabWorkspaceState>(
    DEFAULT_PROJECT.workspaceState,
  );
  ...
  const [assets, setAssets] = useState(DEFAULT_PROJECT.assets);
  ...
  const [dataTables, setDataTables] = useState(DEFAULT_PROJECT.dataTables);
  ...
  const applyLoadedProject = (loadedProject: BuildLabProject) => {
    const loadedScreens = loadedProject.screens.map((screen, index) => ({
      ...screen,
      isDefault: screen.isDefault ?? index === 0,
    }));
    setAssets(loadedProject.assets);
    setDataTables(loadedProject.dataTables);
    setElements(loadedProject.elements);
    setKeyValuePairs(loadedProject.keyValuePairs);
    setMlModels(loadedProject.mlModels ?? []);
    setScreens(loadedScreens);
    setWorkspaceState(loadedProject.workspaceState);
    setActiveScreenId(defaultScreenId(loadedScreens));
    setEventScreenId(defaultScreenId(loadedScreens));
    setSelectedElementId(loadedProject.elements[0]?.id ?? '');
    const firstAsset = loadedProject.assets[0];
    setActiveAssetType(firstAsset?.assetType ?? 'costume');
    setSelectedAssetId(firstAsset?.id ?? '');
    setSelectedDataTableId(loadedProject.dataTables[0]?.id ?? '');
  };
  const saveStatus = useBuildLabPersistence({
    channelId,
    onLoad: applyLoadedProject,
    project,
    readOnly,
  });
  ...
  const toggleRun = () => {
    ...
    const initialScreenId = defaultScreenId(screens, activeScreenId);
    ...
    applyRuntimeState(initialRuntimeState);
    setIsRunning(true);
  };
    ...
    const nextRuntimeState = workspaceState.blocks.blocks
      ...
      .reduce<RuntimeState>(
          ...
          ),
        {
          ...
          keyboardMovements: runtimeKeyboardMovements,
  ...
  useEffect(() => {
    ...
    const moveSprites = () => {
      ...
      setRuntimeElements(currentElements => {
        const nextState = moveWithArrowKeys(
          {
    ...
    <main className={styles.root}>
      ...
      <div className={styles.modeToolbar}>
        <SegmentedButtons
          ...
          size="xs"
        />
        <Typography className={styles.modeLabel} variant="overline">
          Workspace
        </Typography>
        <Button
      ...
      >
        {showWorkspace && (
          <section
            ...
            className={`${styles.workspacePanel} ${readOnly ? styles.readOnly : ''}`}
