...
declare global {
  interface Window {
    McpApp?: {
      ...
      reportSize: () => void;
      updateModelContext: (payload: { structuredContent: Record<string, unknown> }) => void;
    };
  }
}
...
function App() {
  ...
  useEffect(() => {
    if (done && !completedFired.current) {
      ...
      window.McpApp?.updateModelContext({ structuredContent: { event: 'completed' } });
  ...
  function pick(item: ScenarioItem, choice: ToolOption['id']) {
    ...
    window.McpApp?.updateModelContext({
      structuredContent: {
        event: 'answered', itemId: item.id, choice, correctAnswer: item.answer,
        correct, index, total: ITEMS.length,
      },
    });
  ...
  return (
    <div className="pyb-wrap">
      ...
      <div className="pyb-progress-row" role="img" aria-label={`Progress: ${Object.keys(answers).length} of ${ITEMS.length} answered`}>
        {ITEMS.map((it) => (
          <div key={it.id} className={'pyb-dot' + (answers[it.id] ? ' done' : '')} />
        ))}
      </div>

      {!done ? (
        <div className="pyb-card">
          <div className="pyb-emoji" aria-hidden="true">{item.emoji}</div>
          <p className="pyb-prompt">{item.text}</p>
          <div className="pyb-choices" role="group" aria-label="Pick a programming tool" ref={answered ? undefined : choicesRef}>
            {TOOLS.map((tool) => (
              <GenericButton
                key={tool.id}
                text={`${tool.icon} ${tool.label}`}
                type={answered && answered.choice === tool.id ? 'primary' : 'secondary'}
          ...
          {answered && (
            <>
              <div className="pyb-feedback-row">
                ...
                <Alert text={answered.correct ? item.feedbackCorrect : item.feedbackIncorrect} />
              </div>
        ...
        <div className="pyb-card">
          <div className="pyb-takeaway-emoji" aria-hidden="true">🧰</div>
          <p className="pyb-takeaway-title">Nice picking!</p>
          <p className="pyb-takeaway-text">{TAKEAWAY}</p>
        </div>
      )}

      <div aria-live="polite" className="pyb-sr-only">{liveMessage}</div>
    </div>
  );
}
