 ...
 *
 * The ONE esbuild configuration for a widget package: the authoring service
 * (rebuilding a session widget live) and this package's own `buildCatalog()`
 * (rebuilding a graduated widget for CI/release) both call this same
 * function, so a document built at authoring time and a document built in
 * CI can never disagree about how the bundle was produced — see this
 * package's README for why that matters for `docHash` reproducibility.
 *
