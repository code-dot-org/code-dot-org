...
export const TOOLTIP_OVERRIDES: Components<Theme>['MuiTooltip'] = {
  // Tail on, and the text describes the trigger (so the trigger needs its
  // own name). Placement stays MUI's centered `bottom`; pass `-start`/`-end`
  // when the bubble should line up with an edge.
  defaultProps: {
    arrow: true,
    describeChild: true,
  },
