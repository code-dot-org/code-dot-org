...
brandCodeAiNext.css          CADS primitives + semantic tokens, scoped to
                             ...
                             by scripts/generateBrandCodeAiNext.mjs)
...
audit brand survives that deletion unchanged. A follow-up change collapses
this bridge: the CADS files move to `:root` and the brand-scoped layers are
deleted (see the `brandOverrides.css` header for the plan).
...
today the only valid default) unless `brand-router-enabled` is on, in
which case a `?brand=codeai-audit` URL param or the `brand` cookie can
override it per request. The result is written to `data-brand` on
`<html>` by `application.html.haml`.
