...
The theme defaults to `placement="bottom-start"` so the bubble's start edge
lines up with the trigger (CADS), not MUI's centered `bottom`. Pass
`placement="bottom"` (or `top` / `left` / `right`) to center instead. A call
site moved over from the legacy tooltip that never set a direction still needs
an explicit `placement="top"` if it used to sit above the trigger.
