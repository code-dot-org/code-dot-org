...
| `button`              | **Migrated**   | `Button`, `IconButton` | `styleOverrides/button.tsx`             | Deprecated. All `/apps` consumers migrated. All internal DSCO consumers migrated (PR #71642). Ready for step 4 (delete DSCO source). Use `Button`, `IconButton` from `@mui/material`. Codemod: `yarn codemod:buttons`. See `src/button/BUTTON_MIGRATION_TO_MUI.md`. |
...
| `snackbar`            | **MUI-native** | `Snackbar`             | —                                       | No DSCO wrapper exported. Use `Snackbar` from `@mui/material` directly, or the `toast` wrapper for a transient status message with an accessible announcer.                                                                                                         |
| `toast`               | **MUI-native** | `Snackbar`             | —                                       | Composed wrapper: top-center MUI `Snackbar` + DSCO `Alert` + a persistent polite live region. Exports `ToastProvider`/`useToast` (imperative), `Toast` (controlled), and `ToastAnnouncer`.                                                                          |
...
| `form`                | **N/A**        | —                      | —                                       | Composed form kit (`FormProvider`/`useField`/`useFormOptions`, `SaveBar`, `Field`, `FormError`, save-state machine, save-error mapping). Standalone components with no MUI equivalent — no migration needed; kept as-is. `FormError` renders the DSCO `Alert`.      |
