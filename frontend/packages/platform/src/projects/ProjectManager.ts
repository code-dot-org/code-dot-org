...
export default class ProjectManager {
  ...
  async saveThumbnail() {
    if (this.thumbnailUrl && this.thumbnailPngBlob) {
      try {
        ...
        logger.warn('Failed to save thumbnail.');
  ...
  private onSaveFail(errorMessage: string, error: Error) {
    ...
    if (error.message.includes('409') || error.message.includes('401')) {
      ...
      logger.warn(`${error.message}. Reloading page.`);
      ...
      logger.warn('Project too large to save');
    } else {
      // Otherwise, capture the error with the friendly message as context.
      recordError(error, {message: errorMessage, errorMessage: error.message});
    }
  }
  ...
  private logAndThrowError(errorMessage: string) {
    ...
    recordError(error, {message: errorMessage});
    throw error;
  }
