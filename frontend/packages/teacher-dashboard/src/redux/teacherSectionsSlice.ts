...
import {OAuthSectionTypes} from '@code-dot-org/users';
import type {OAuthSectionType} from '@code-dot-org/users';
...
export const assignToSection = (
  ...
  return (dispatch, getState) => {
    ...
    if (
      ...
      metrics.count('Section Curriculum Assigned', 1, {
...
export const unassignSection =
  (sectionId: number, _location: string): SectionThunkAction =>
  (dispatch, _getState) => {
    dispatch(beginEditingSection(sectionId, true));
    dispatch(
      editSectionProperties({
