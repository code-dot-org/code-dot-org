...
const FILE_MAP: Record<string, string> = {
  // Generated by dashboard and gitignored, so a fresh checkout has no copy;
  // the shim vendors the three constants the feature reads.
  '@cdo/generated-scripts/sharedConstants': stub('sharedConstants.ts'),
...
const appsRoot = path.resolve(repoRoot, 'apps') + path.sep;
const pkgEntry = path.resolve(pkgRoot, 'index.html');
...
export function cdoResolverPlugin(): Plugin {
  return {
    ...
    resolveId(source, importer, options) {
      const mapped = FILE_MAP[source];
      if (mapped) return mapped;
      // Bare npm imports inside apps-served files (the sketchlab canvas
      // closure) would otherwise resolve by walking up from apps/, which
      // only works in a checkout where apps/node_modules is installed. The
      // package declares every runtime dep the closure uses, so re-resolve
      // them as if imported from the package root instead.
      if (
        importer?.startsWith(appsRoot) &&
        !source.startsWith('.') &&
        !source.startsWith('/')
      ) {
        return this.resolve(source, pkgEntry, {...options, skipSelf: true});
      }
      return null;
    },
  };
