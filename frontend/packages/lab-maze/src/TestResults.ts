...
class TestResults {
  ...
  constructor(workspace: Blockly.Workspace, options: TestResultsOptions) {
    ...
    this.status_ = Status.NO_TESTS_RUN;
  ...
  runTests(options: TestResultsOptions): Status {
    ...
    if (options.shouldCheckForEmptyBlocks) {
      ...
      if (emptyBlockFailure !== Status.ALL_PASS) {
        return emptyBlockFailure;
      }
    }

    if (!options.allowTopBlocks && this.hasExtraTopBlocks()) {
      return Status.EXTRA_TOP_BLOCKS_FAIL;
    }

    if (this.hasUnusedParam()) {
      return Status.UNUSED_PARAM;
    }

    if (this.hasUnusedFunction()) {
      return Status.UNUSED_FUNCTION;
    }

    if (this.hasParamInputUnattached()) {
      return Status.PARAM_INPUT_UNATTACHED;
    }

    if (this.hasIncompleteBlockInFunction()) {
      return Status.INCOMPLETE_BLOCK_IN_FUNCTION;
    }

    if (this.hasQuestionMarksInNumberField()) {
      return Status.QUESTION_MARKS_IN_NUMBER_FIELD;
    }

    // Run block tests
    if (!this.hasAllBlocks(options.requiredBlocks || [])) {
      return levelComplete
        ? Status.MISSING_BLOCK_FINISHED
        : Status.MISSING_BLOCK_UNFINISHED;
    }

    if (!this.hasAllBlocks(options.recommendedBlocks || [])) {
      return levelComplete
        ? Status.MISSING_RECOMMENDED_BLOCK_FINISHED
        : Status.MISSING_RECOMMENDED_BLOCK_UNFINISHED;
    }
    ...
    if (!levelComplete) {
      ...
      return Status.LEVEL_INCOMPLETE_FAIL;
    }

    // Check for exceeded block limits
    if (options.blockLimitMap?.anyOver()) {
      return Status.BLOCK_LIMIT_FAIL;
    }
    ...
    if (
      ...
      return Status.BETTER_THAN_IDEAL;
    }
