...
export default defineConfig({
  plugins: [
    ...
    // Libraries such as react, react-dom, @modelcontextprotocol/sdk, and zod
    // should not be bundled by the library. Instead, they are expected to be
  ...
  build: {
    sourcemap: true,
    lib: {
      entry: ['src/index.ts', 'src/chrome.ts'],
      name: 'widgetRuntime',
    },
    rollupOptions: {
