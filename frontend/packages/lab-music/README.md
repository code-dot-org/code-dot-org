...
This is the client-side code for **Music Lab**, as instantiated by a `music` level using [Lab2](../lab-base/).
...
**Music Lab** is also the first lab to use the new [Lab2](../lab-base/) framework, and showcases some features and components unique to that framework, when used in a level progression:
...
- [MusicView](./components/MusicView.jsx) is the entrypoint into Music Lab, and is the top-level container view that is rendered by the Lab2 framework. MusicView handles orchestrating a lot of the overarching app logic, such as connecting UI events to player APIs and loading the music library.
- MusicView renders [MusicLabView](./components/MusicLabView.tsx) which actually renders most of the UI. MusicLabView was created primarly to reduce the size of the MusicView file and convert more of the UI code into functional React/TypeScript (as MusicView is currently class-based and JavaScript). Most other view components are rendered by MusicLabView.
- There are a few view components rendered by custom Blockly fields. These include:
  - [SoundsPanel](./components/soundsPanel/SoundsPanel.tsx) which represents the sound picker UI.
  - [ChordPanel](./components/chordPanel/ChordPanel.tsx) which represents the "play notes" keyboard/chord selection UI.
  - [PatternPanel](./components/patternPanel/PatternPanel.tsx) which represents the "play drums" grid drum editor.
