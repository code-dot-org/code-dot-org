...
class AudioSystem {
  ...
  constructor() {
    try {
      ...
      if (e instanceof Error) {
        LabRegistry.metricsReporter.logError('Web Audio API is not supported in this browser', e);
      }
      throw e;
    }

    AudioSystem.soundEffects = new SoundEffects(AudioSystem.audioContext, DEFAULT_DELAY_TIME);
  }
  ...
  updateConfiguration(options: {
    ...
    if (AudioSystem.audioContext) {
      ...
      AudioSystem.soundEffects = new SoundEffects(AudioSystem.audioContext, delayTimeSeconds);
  ...
  PlaySoundByBuffer(audioBuffer: AudioBuffer, id: number, when: number, loop: boolean, effects: Effects | undefined, callback: (id: number) => void, duration?: number): AudioBufferSourceNode | undefined {
    if (!AudioSystem.audioContext) {
      return;
    }

    const source: AudioBufferSourceNode = AudioSystem.audioContext.createBufferSource(); // creates a sound source
