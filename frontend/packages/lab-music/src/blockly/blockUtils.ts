...
import {BLOCK_TYPES, JavascriptBlockGenerator, BlocklySerialization, ProcedureBlock} from '@code-dot-org/blockly-workspace';
 ...
 */
export function getCodeForSingleBlock(block: Blockly.Block | null, javascriptGenerator: JavascriptGenerator) {
  if (!block) {
    return '';
  }
...
export const isBlockInsideWhenRun: (block: Blockly.Block) => boolean = block => {
  return [BlockTypes.WHEN_RUN, BlockTypes.WHEN_RUN_SIMPLE2].includes(
    block.getRootBlock().type as BlockTypes
  );
};

// Override default function block implementation for the current block mode.
export function installFunctionBlocks(blockMode: BlockMode, javascriptGenerator: JavascriptGenerator) {
...
export function backupFunctionDefinitons(javascriptGenerator: JavascriptGenerator) {
  ...
  backupBlockDefinitionTypes.forEach(type => {
    ...
    BlockCache.blockGenerators[type] = javascriptGenerator.forBlock[type as string];
  });
}
...
function restoreBlockDefinitions(javascriptGenerator: JavascriptGenerator) {
  ...
  Object.keys(BlockCache.blockDefinitions).forEach(type => {
    blockDefinitions[type as keyof BlockDefinitions] = BlockCache.blockDefinitions[type as keyof BlockDefinitions];
  });
...
function simple2FunctionCallGenerator(functionName: string, functionCallBlockId: string) {
  return `
...
export function applyBlockIdOverrides(workspaceJson: BlocklySerialization, overrides: Record<string, string>) {
