...
const copyCraftAssets = async (subpath = 'public') => {
  const source = path.resolve(__dirname, '../../../apps/static/craft');
  const target = path.resolve(__dirname, `${subpath}/craft`);
  ...
  try {
    await fs.cp(source, target, {recursive: true, force: true});
    console.log('[copy.mjs] Copied Craft skin assets.');
  } catch (err) {
    console.warn('[copy.mjs] Failed to copy Craft skin assets:', err);
  }
};
copyCraftAssets(); // Copy to 'public'
copyCraftAssets('dist'); // Copy to 'dist'
