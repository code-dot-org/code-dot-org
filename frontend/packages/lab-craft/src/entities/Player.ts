...
class Player extends BaseEntity {
  ...
  constructor(
    ...
    super(scene, type, 'Player', x, y, facing);
    ...
    if (scene.getIsDirectPlayerControl()) {
      this.moveDelayMin = 0;
      this.moveDelayMax = 0;
    } else {
    ...
    if (scene.levelData.ocean) {
      this.offset = [-26, -26];
    }

    if (scene.levelData.boat) {
      this.offset = [-27, -30];
    }
  ...
  prepareSprite() {
    ...
    //this.sprite.animationRig = this.scene.add.sprite(0, 0, `player${this.playerName}`, 'Player_121');
    const actionGroup = this.scene.levelView.actionGroup;
    actionGroup.add(this.sprite);
    //this.sprite.addChild(this.sprite.animationRig);
    ...
    const atlas = this.scene.textures.exists('playerSteveAquatic')
      ? 'playerSteveAquatic'
      : this.scene.textures.exists('playerSteveEvents')
        ? 'playerSteveEvents'
        : 'playerSteve';
   ...
   */
  walkableCheck(block: LevelBlock) {
  ...
  canUpdateMovement(): boolean {
    ...
    const queueHasOne =
      !!this.queue.currentCommand && this.queue.getLength() === 0;
    const timeElapsed = (new Date()).getTime() - this.lastMovement;
    const movementAlmostFinished = timeElapsed > 300;
    if (isMoving && timeElapsed > 800) {
    ...
    return (
      !this.onTracks &&
      (queueIsEmpty || (queueHasOne && movementAlmostFinished)) &&
      isMoving
    );
  }

  doMove(commandQueueItem: BaseCommand, action: (entity?: BaseEntity) => void) {
    ...
    levelView.playMoveForwardAnimation(
      this,
      prevPosition,
      jumpOff,
      groundType,
      () => this.afterMove(commandQueueItem),
    );
  ...
  doMoveForward(commandQueueItem: BaseCommand) {
    this.doMove(commandQueueItem, this.scene.levelModel.moveForward);
  }

  doMoveBackward(commandQueueItem: BaseCommand) {
    this.doMove(commandQueueItem, this.scene.levelModel.moveBackward);
  }

  afterMove(commandQueueItem: BaseCommand) {
    const levelModel = this.scene.levelModel;
    const levelView = this.scene.levelView;

    if (levelModel.isUnderwater()) {
      this.play('underwater-walk');
    } else {
      this.play('idle');
    }

    if (levelModel.isPlayerStandingInWater() && !levelModel.isInBoat()) {
      levelView.playDrownFailureAnimation(
        () => {
      ...
      levelView.playBurnInLavaAnimation(
        () => {
          this.scene.handleEndState(false);
        },
        this
  ...
  collectItems(targetPosition?: Position) {
    targetPosition ||= this.position;

    // collectible check
    const collectibles = this.scene.levelView.collectibleItems;
    for (let i = 0; i < collectibles.length; i++) {
      ...
      if (!sprite) {
        ...
        const collectiblePosition = this.scene.levelModel.spritePositionToIndex(
          offset,
          new Position(sprite.x, sprite.y),
        );
        if (
          Position.absoluteDistanceSquare(targetPosition, collectiblePosition) <
          collectibleDistance
        ) {
          this.scene.levelView.playItemAcquireAnimation(() => {}, blockType, sprite, this);
          collectibles.splice(i, 1);
        }
      }
    }
