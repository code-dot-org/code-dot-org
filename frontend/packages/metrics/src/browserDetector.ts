...
function isUnsupportedIE() {
  return isIE() && parseInt(IEVersion()) < 11;
}
...
function isUnsupportedChrome() {
  return isChrome() && parseInt(chromeVersion()) < 33;
}
...
function isUnsupportedSafari() {
  return isSafari() && parseInt(safariVersion()) < 7;
}
...
function isUnsupportedFirefox() {
  return isFirefox() && parseInt(firefoxVersion()) < 25;
}
...
export function isIE11() {
  return !!(typeof window.MSInputMethodContext !== 'undefined') && !!(typeof document.documentMode !== 'undefined');
}
...
export function isStorageAvailable(type: 'localStorage' | 'sessionStorage') {
  try {
    ...
    // Ignore errors so we return false at the end.
  }
  return false;
}
