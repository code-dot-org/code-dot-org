...
1. The observability package SHALL export an `ObservabilityClient` interface with at minimum the following operations: `init`, `recordError`, `setConsented`, and `shutdown`.
...
5. The `ObservabilityClient` SHALL record only error-level data and session metadata; it SHALL NOT record user interaction replays, DOM snapshots, or network request payloads unless explicitly enabled by a future requirement.
...
**User Story:** As a platform operator, I want to control which users' log events are forwarded to the observability provider so that I can investigate issues for specific users without incurring the cost of full-volume log ingestion.
...
3. The `ObservabilityClient` SHALL support session ID-based log sampling: the adapter SHALL use an observability-owned session ID — a UUID generated by the observability package itself and persisted in `sessionStorage` under `__cdo_observability_session_id__` — as a stable sampling key so that all log events for a given session are consistently included or excluded. If no session ID exists in `sessionStorage`, the adapter SHALL generate one and persist it before executing any sampling decision. If `sessionStorage` throws at any point, the adapter SHALL log a single `console.warn`, set an internal `sessionStorageUnavailable` flag, and short-circuit all subsequent sampling decisions to "not sampled" without retrying `sessionStorage`. This approach supports anonymous users — no user ID or consent is required for sampling decisions. Consent is not required because the session ID is used solely as a local sampling key to decide whether to emit events; the session ID itself is never transmitted to the provider, and the log events that are emitted contain no personally identifiable information.
...
**User Story:** As a platform operator, I want to control which users' metric events are forwarded to the observability provider so that I can manage costs while retaining the ability to investigate specific users' metric data.
...
3. The `ObservabilityClient` SHALL support session ID-based metrics sampling: the adapter SHALL use an observability-owned session ID — a UUID generated by the observability package itself and persisted in `sessionStorage` under `__cdo_observability_session_id__` — as a stable sampling key so that all metric events for a given session are consistently included or excluded. If no session ID exists in `sessionStorage`, the adapter SHALL generate one and persist it before executing any sampling decision. If `sessionStorage` throws at any point, the adapter SHALL log a single `console.warn`, set an internal `sessionStorageUnavailable` flag, and short-circuit all subsequent sampling decisions to "not sampled" without retrying `sessionStorage`. This approach supports anonymous users — no user ID or consent is required for sampling decisions. Consent is not required because the session ID is used solely as a local sampling key to decide whether to emit events; the session ID itself is never transmitted to the provider, and the metric events that are emitted contain no personally identifiable information.
...
### Requirement 11: Distributed Tracing — Frontend/Backend Trace Context Propagation
...
5. When `tracePropagationTargets` is not set, the `ObservabilityClient` SHALL use the environment-derived default described in 11.4, preventing accidental trace header leakage to third-party services.
