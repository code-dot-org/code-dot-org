...
type SeatAction =
  ...
  | {type: 'MARK_JOURNEY_NODE_COMPLETE'; seatId: SeatId; updatedSeat: Seat};
...
function seatReducer(state: SeatState, action: SeatAction): SeatState {
  switch (action.type) {
    ...
    case 'MARK_JOURNEY_NODE_COMPLETE':
      return {
        ...state,
        seats: state.seats.map(s =>
          s.id === action.seatId ? action.updatedSeat : s,
        ),
      };
...
export interface UseActiveSeatResult {
  ...
  /** Marks a notebook journey node complete. No-op if already complete or no active seat. */
  markJourneyNodeComplete: (nodeIndex: number) => Promise<void>;
}
...
export function useActiveSeat(): UseActiveSeatResult {
  ...
  const markJourneyNodeComplete = useCallback(
    async (nodeIndex: number) => {
      ...
      const existing = await readNotebookProgress(seatId);
      const nodes: boolean[] = existing?.nodes ?? [false, false, false, false];
      if (nodes[nodeIndex]) return; // already complete, no-op

      const updatedNodes = [...nodes];
      updatedNodes[nodeIndex] = true;
      const graduated = updatedNodes.every(Boolean);
      ...
      await writeNotebookProgress(seatId, progress);

      const updatedSeat: Seat = {
        ...seat,
        journeys: {...seat.journeys, notebook: progress},
      };
      dispatch({type: 'MARK_JOURNEY_NODE_COMPLETE', seatId, updatedSeat});
    },
    [state.activeSeatId, state.seats],
  );
