...
class Salmon extends BaseEntity {
  ...
  prepareSprite() {
    ...
    const frameBase = this.controller.levelModel.isUnderwater()
      ? 'Salmon'
      : 'Salmon_Surface';

    let frameListPerDirection = [
      [13, 25], // up
      [0, 12], // right
      [13, 25], // down
      [13, 25],
    ...
    for (let i = 0; i < 4; i++) {
      const facingName = this.controller.levelView.getDirectionName(i);
      const frameList = Phaser.Animation.generateFrameNames(
        frameBase,
        frameListPerDirection[i][0],
        frameListPerDirection[i][1],
        '.png',
        2,
      );
      this.sprite.animations
        .add('idle' + facingName, frameList, frameRate, false)
        .onComplete.add(() => {
