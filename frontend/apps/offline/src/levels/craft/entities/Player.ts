...
class Player extends BaseEntity {
  ...
  doMoveForward(commandQueueItem: BaseCommand) {
    ...
    const jumpOff = wasOnBlock && wasOnBlock !== player.isOnBlock;
    const groundType =
      player.isOnBlock || jumpOff
        ? levelModel.actionPlane.getBlockAt(player.position).blockType
        : levelModel.groundPlane.getBlockAt(player.position).blockType;
  ...
  doMoveBackward(commandQueueItem: BaseCommand) {
    ...
    levelView.playMoveBackwardAnimation(
      this,
      prevPosition,
      this.facing,
      jumpOff,
  ...
  bump(commandQueueItem: BaseCommand) {
    const levelView = this.scene.levelView,
      levelModel = this.scene.levelModel;
    ...
    for (let i = 0; i < collectibles.length; i++) {
      ...
      if (sprite === null) {
        ...
        if (
          ...
          this.scene.levelView.playItemAcquireAnimation(
            this.position,
            this.facing,
            sprite,
            () => {},
