...
class Sheep extends BaseEntity {
  ...
  use(commandQueueItem: BaseCommand, userEntity: BaseEntity) {
    ...
    this.scene.events.forEach(e =>
      e({
        eventType: EventType.WhenUsed,
  ...
  bump(commandQueueItem: BaseCommand) {
    ...
    if (forwardEntity !== null) {
      ...
      this.scene.events.forEach(e =>
        e({
          eventType: EventType.WhenTouched,
          targetType: this.type,
          targetIdentifier: this.identifier,
  ...
  attack(commandQueueItem: BaseCommand) {
    ...
    this.sprite.on(
      `animationcomplete-${this.getNakedSuffix()}attack-${facingName}`,
      () => {
        ...
        if (frontEntity !== null) {
          this.sprite.on(
            `animationcomplete-${this.getNakedSuffix()}hurt-${facingName}`,
            () => {
              this.scene.events.forEach(e =>
                e({
                  eventType: EventType.WhenAttacked,
                  targetType: this.type,
  ...
  takeDamage(callbackCommand: CallbackCommand) {
    if (this.healthPoint > 1) {
      ...
      this.play('die');
      setTimeout(() => {
        const tween = this.scene.levelView.addResettableTween(this.sprite).to(
          {
            alpha: 0,
          },
          500,
