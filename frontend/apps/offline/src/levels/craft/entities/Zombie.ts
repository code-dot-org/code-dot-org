...
class Zombie extends BaseEntity {
  ...
  playMoveForwardAnimation(
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < 2; i++) {
        const tween = this.controller.levelView
          .addResettableTween(this.burningSprite[i])
          .to(
            {
              x:
                this.offset[0] + this.burningSpriteOffset[0] + 40 * position[0],
              y:
                this.offset[1] + this.burningSpriteOffset[1] + 40 * position[1],
            },
            300,
  ...
  takeDamage(callbackCommand: CallbackCommand) {
    ...
    if (this.healthPoint > 1) {
      ...
      setTimeout(() => {
        let tween = this.scene.levelView.addResettableTween(this.sprite).to(
          {
            alpha: 0,
          },
          500,
