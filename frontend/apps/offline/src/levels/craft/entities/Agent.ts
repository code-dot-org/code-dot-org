...
import BaseEntity, {CanPlace} from '../BaseEntity';
...
class Agent extends BaseEntity {
  ...
  updateMovement() {
    if (
      !this.controller.attemptRunning ||
      !this.controller.getIsDirectPlayerControl()
    ) {
      return;
    }
  ...
  doMove(commandQueueItem, movement) {
    ...
    levelView[`playMove${movement}Animation`](
      this,
      prevPosition,
      this.facing,
      jumpOff,
      ...
      () => {
        ...
        this.controller.delayPlayerMoveBy(
          this.moveDelayMin,
          this.moveDelayMax,
          () => {
  ...
  bump(commandQueueItem: BaseCommand) {
    ...
    const frontEntity = this.controller.levelEntity.getEntityAt(
      levelModel.getMoveForwardPosition(this),
    );
    if (frontEntity !== null) {
      const isFriendlyEntity = this.controller.levelEntity.isFriendlyEntity(
        frontEntity.type,
      );
  ...
  takeDamage(callbackCommand: CallbackCommand) {
    const facingName = this.controller.levelView.getDirectionName(this.facing);
    this.healthPoint--;
    // still alive
    if (this.healthPoint > 0) {
      this.controller.levelView.playScaledSpeed(
      ...
      this.controller.levelView.playFailureAnimation(
        this.position,
        this.facing,
        this.isOnBlock,
        () => {
