...
class LevelPlane {
  ...
  getMinecartTrack(
    position: Position,
    facing: Direction,
  ): [string, Position, Direction, number] {
  ...
  determineRailType(position: Position, updateTouching: boolean = false) {
    ...
    const variant =
      connectionName(block.connectionA) + connectionName(block.connectionB);
    ...
    if (this.getBlockAt(armPosition).getIsPistonArm()) {
      if (this.getBlockAt(pistonPosition).getIsStickyPiston()) {
        ...
        if (
          ...
          this.getBlockAt(stuckBlockPosition).isStickable
        ) {
   ...
   */
  pushBlocks(blocksPositions: Position[], offset: [number, number] = [0, 0]) {
    let pistonType = '';
    let redo = false;
    if (offset[0] === 1) {
   ...
   */
  powerCheck(position: Position, canReadCharge: boolean = false): Position[] {
  ...
  getConduitRingPositions(position: Position, ringSize: number) {
