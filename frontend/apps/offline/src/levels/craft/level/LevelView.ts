...
import type GameController, {AudioPlayer} from '../GameController';
...
class UnderwaterEffectPipeline extends Phaser.Renderer.WebGL.Pipelines
  .PostFXPipeline {
  ...
  onBoot() {
    ...
    this.targetTexture = source.glTexture;
...
class LevelView {
  ...
  protected baseShading?: string;
  protected prismarinePhase: number;
  protected uniforms: Uniforms;
  protected waveShader: Phaser.GameObjects.Shader;
  protected player?: Player;
  protected agent?: Agent;
  protected selectionIndicator?: string;
  protected groundGroup?: Phaser.Group;
  protected shadingGroup?: Phaser.Group;
  protected hintGroup?: Phaser.Group;
  protected actionGroup?: Phaser.Group;
  protected fluffGroup?: Phaser.Group;
  protected fowGroup?: Phaser.Group;
  protected airGroup?: Phaser.Group;
  protected collectibleItems: BaseEntity[] = [];
  //{sprite : sprite, type : blockType, position : [x,y]}
  protected trees: BaseEntity[] = [];
  protected miniBlocks: {
    [key: string]: string;
  };
  protected blocks: {
  ...
  protected actionPlaneBlocks: Map<number, Phaser.GameObjects.Sprite>;
  /** Maintains a mapping from block index to animation effect for a block */
  protected blockEffects: Map<number, Phaser.GameObjects.Sprite>;
  protected toDestroy: LevelBlock[] = [];
  protected resettableTweens: Phaser.Tweens.Tween[] = [];
  protected treeFluffTypes: {
  ...
  blockReceivesCornerShadow(x: number, y: number): boolean {
    const southBlock = this.scene.levelModel.actionPlane.getBlockAt([x, y + 1]);
    if (!southBlock || (southBlock.blockType && !southBlock.isWalkable)) {
      return false;
    }

    const southWestBlock = this.scene.levelModel.actionPlane.getBlockAt([
      x - 1,
      y + 1,
    ]);
    return (
  ...
  createBlock(
    ...
    const buildTree = (frame: number[]) => {
      ...
      sprite.onBlockDestroy = (_logSprite: Phaser.GameObjects.Sprite) => {
    ...
    const buildDoor = (type: string): Phaser.GameObjects.Sprite => {
      ...
      const sprite = this.scene.add.sprite(
        xOffset + MIDX + 40 * x,
        yOffset + MIDY + group.yOffset + 40 * y,
        atlas,
        frame,
      );
      group.add(sprite);
      ...
      default:
        {
          ...
          if (
            ...
            if (!this.blockEffects.has(index)) {
      ...
      onComplete: () => {
        if (overrides.onComplete) {
          ...
          const distanceBetween = function (position, position2) {
            return Math.sqrt(
              Math.pow(position.x - position2.x, 2) +
          ...
          if (
            distanceBetween(this.player.position, collectiblePosition) <
            collectibleDistance
          ) {
  ...
  refreshGroundGroup() {
    ...
    for (let y = 0; y < this.scene.levelModel.planeHeight; y++) {
      for (let x = 0; x < this.scene.levelModel.planeWidth; x++) {
        ...
        if (groundBlock.blockType === 'torch') {
          console.log('CREATE TORCH!! GB ', groundBlock.blockType);
        }
  ...
  resetEntity(entity: BaseEntity) {
    if (this.scene.followingPlayer() && entity === this.player) {
  ...
  updateShadingGroup(shadingData) {
    let index, shadowItem, sx, sy, atlas;
  ...
  create(levelModel: LevelModel) {
    ...
    if (levelModel.isUnderwater()) {
      this.scene.sys.renderer.pipelines.addPostPipeline(
        'underwater',
        UnderwaterEffectPipeline,
      );

      this.waveShader =
        this.scene.sys.renderer.pipelines.getPostPipeline('underwater');
  ...
  getDirectionName(facing: Direction): string {
    return FacingDirection.directionToRelative(facing).toLowerCase();
  }
  ...
  playOpenConduitAnimation(position: Position) {
    ...
    if (this.actionPlaneBlocks.has(index)) {
      ...
      sprite?.on(
        `${Phaser.Animations.Events.ANIMATION_COMPLETE_KEY}${animationName}`,
        () => {
          this.scene.levelModel.actionPlane.getBlockAt(position).isEmissive =
            true;
