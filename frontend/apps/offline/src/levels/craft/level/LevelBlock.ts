...
/**
 * Represents a block in the level.
 */
class LevelBlock {
  protected blockType: string;
  protected isEntity: boolean;
  protected isWalkable: boolean;
  protected isPlacable: boolean;
  protected isDestroyable: boolean;
  protected isUsable: boolean;
  protected isEmpty: boolean;
  protected isEmissive: boolean;
  protected isTransparent: boolean;
  protected isRedstone: boolean;
  protected isPowered: boolean;
  /** Whether or not this block can connect to nearby redstone wire */
  protected isConnectedToRedstone: boolean;
  protected isRedstoneBattery: boolean;
  protected isOpen: boolean;
  protected isRail: boolean;
  protected isSolid: boolean;
  protected isWeaklyPowerable: boolean;
  protected isStickable: boolean;
  protected isActivatedConduit: boolean;
  protected connectionA?: LevelBlock;
  protected connectionB?: LevelBlock;
  ...
  constructor(blockType: string) {
    ...
    if (blockType.substring(0, 5) === 'rails') {
      this.isWeaklyPowerable =
        blockType === 'railsRedstoneTorch' ? true : false;
      ...
      this.isConnectedToRedstone =
        /^rails(RedstoneTorch|Unpowered|Powered)/.test(blockType);
    ...
    if (blockType.startsWith('piston')) {
      ...
      if (
        blockType.substring(blockType.length - 2, blockType.length) === 'On' ||
        blockType.startsWith('pistonArm') ||
        blockType.substring(blockType.length - 8, blockType.length) ===
          'OnSticky'
      ) {
        this.isEntity = true;
      }
  ...
  notValidOnGroundPlane(): boolean {
    return (
      this.blockType.startsWith('rails') ||
      this.blockType.startsWith('redstone')
    );
  }
  ...
  getIsStickyPiston(): boolean {
    return (
      this.blockType.substring(
        this.blockType.length - 6,
        this.blockType.length,
      ) === 'Sticky'
    );
  }
  ...
  isDestroyableUponPush(): boolean {
    return (
      this.blockType.startsWith('redstone') ||
      this.blockType.startsWith('door') ||
      this.blockType.startsWith('railsRedstone') ||
      this.blockType.startsWith('pressure')
    );
  }
  ...
  needToRefreshRedstone(): boolean {
    if (
      this.isRedstone ||
      this.blockType === '' ||
      (this.isConnectedToRedstone && !this.blockType.startsWith('piston'))
    ) {
      return true;
    } else {
      return false;
  ...
  getPistonDirection(): Direction {
    if (this.blockType.startsWith('piston')) {
      const direction = this.blockType.substring(6, 7);
      switch (direction) {
        case 'D':
   ...
   * TODO remove this method once all calling methods have been updated
  ...
  static skipsDestructionOverlay(blockType: string): boolean {
    return new LevelBlock(blockType).skipsDestructionOverlay();
  }

  static notValidOnGroundPlane(blockType: string): boolean {
   ...
   * frame
   */
  static getMiniblockFrame(blockType: string): string | undefined {
