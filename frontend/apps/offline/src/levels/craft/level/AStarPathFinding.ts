...
class AStarPathFinding {
  ...
  createGrid(): Node[] {
    return this.levelModel.actionPlane
      .getAllPositions()
  ...
  findPath(startPosition: Position, endPosition: Position): Node[] {
    // Ensure we are in a starting state.
    this.reset();

    const startIndex = this.levelModel.coordinatesToIndex(
    ...
    while (openList.length > 0) {
      ...
      for (let i = 0; i < neighbors.length; i++) {
        ...
        if (!neighbor.visited) {
          ...
          neighbor.h = Position.manhattanDistance(
            neighbor.position,
            endNode.position,
          );
