...
class BaseEntity {
  ...
  getWalkAnimation(): string {
    this.getAnimation('walk');
  }

  getIdleAnimation(): string {
    this.getAnimation('idle');
  }
  ...
  playMoveForwardAnimation(
    ...
    // play walk animation
    levelView.playScaledSpeed(
      this.getAnimationManager(),
      this.getWalkAnimation(),
    );
    setTimeout(() => {
      tween = this.scene.levelView.addResettableTween(this.sprite).to(
        {
  ...
  updateHidingTree() {
    ...
    if (!this.underTree.state) {
      ...
      const isOtherEntityUnderTree = function (
        currentEntity,
        entities,
        currentTreeIndex,
      ) {
  ...
  updateHidingBlock(prevPosition: Position) {
    ...
    const frontBlockCheck = (entity, position) => {
      ...
      if (frontBlock && !frontBlock.isTransparent) {
        const sprite =
          levelView.actionPlaneBlocks[
            levelView.coordinatesToIndex(frontPosition)
          ];
        if (sprite) {
    ...
    const prevBlockCheck = (entity, position) => {
      const frontPosition = Position.south(position);
  ...
  bump(commandQueueItem: BaseCommand) {
    ...
    this.scene.levelView.playScaledSpeed(
      this.getAnimationManager(),
      animName + facingName,
    );
  ...
  moveAway(commandQueueItem: BaseCommand, moveAwayFrom: BaseEntity) {
    ...
    let bestPosition = [];
    const comparePositions = function (moveAwayPosition, position1, position2) {
      return Position.absoluteDistanceSquare(position1.y, moveAwayPosition) <
    ...
    if (
      ...
      currentDistance >=
        Position.absoluteDistanceSquare(moveAwayPosition, bestPosition.y)
    ) {
      commandQueueItem.succeeded();
    } else {
  ...
  moveToward(commandQueueItem: BaseCommand, moveTowardTo: BaseEntity) {
    ...
    let bestPosition = [];
    const comparePositions = function (
      moveTowardPosition,
      position1,
      position2,
    ) {
  ...
  use(commandQueueItem: BaseCommand, userEntity: BaseEntity) {
    ...
    const animationName =
      'lookAtCam' + this.scene.levelView.getDirectionName(this.facing);
    this.scene.levelView.playScaledSpeed(
      this.getAnimationManager(),
      animationName,
    );
  ...
  playRandomIdle(facing: Direction) {
    let animationName = '';
    const facingName = this.scene.levelView.getDirectionName(facing);
  ...
  handleMoveOffPressurePlate(moveOffset: Position) {
    ...
    const isMovingOffOf =
      this.scene.levelModel.actionPlane.getBlockAt(previousPosition)
        .blockType === 'pressurePlateDown';
    ...
    this.scene.levelEntity.entityMap.forEach(workingEntity => {
      if (
        ...
        this.scene.positionEquivalence(workingEntity.position, previousPosition)
      ) {
        remainOn = true;
      }
    });
    ...
    if (isMovingOffOf && !remainOn) {
      ...
      this.scene.levelModel.actionPlane.setBlockAt(
        previousPosition,
        block,
        moveOffset.x,
        moveOffset.y,
  ...
  handleMoveOnPressurePlate(moveOffset: Position): boolean {
    ...
    const isMovingOnToPlate =
      this.scene.levelModel.actionPlane.getBlockAt(targetPosition).blockType ===
      'pressurePlateUp';
  ...
  handleMoveOffIronDoor(moveOffset: Position) {
    ...
    const wasOnDoor =
      this.scene.levelModel.actionPlane.getBlockAt(formerPosition).blockType ===
      'doorIron';
    const isOnDoor =
      this.scene.levelModel.actionPlane.getBlockAt(this.position).blockType ===
      'doorIron';
  ...
  registerAnimations(animations: Animations, frameRate: number, atlas: string) {
    ...
    for (const [action, info] of Object.entries(animations)) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const offset: [number, number] | undefined = info.offsets?.[facingName];
        ...
        const delayFrame = info.delayFrame
          ? {
              key: info.atlas || atlas,
              frame: `${info.prefix}${info.delayFrame[facingName].toString().padStart(info.zeroPad || 3, '0')}${info.suffix || ''}`,
            }
        ...
        if (!this.scene.anims.exists(animationName)) {
          this.scene.anims.create({
            key: animationName,
            frames: frameList,
            frameRate: this.scene.originalFpsToScaled(info.fps || frameRate),
        ...
        if (info.sound) {
          this.sprite.on(
            `${Phaser.Animations.Events.ANIMATION_START_KEY}${animationName}`,
            () => {
              this.levelView.audioPlayer.play(info.sound);
            },
          );
        }
      }
