...
On the GH action side, Helm CI only builds and publishes the immutable `git-<full-commit-sha>` image tag, and it writes no package artifact and no Git Freight record.
...
Helm-shaped plans keep envType values under `apps/codeai/envTypes/` and
deployment metadata under `apps/codeai/deployments/`. `deploy/` is reserved
for rendered output on stage branches instead of committed source on `main`.
...
- Keep the Helm render path aligned with the promoted source commit
...
- exact checkout plus sparse paths
