...
6. Copy the full checked-in `code-dot-org/k8s/kustomize/` tree literally, including `overlays/` and `bin/`. Do not trim the Kustomize snapshot to only `base/` and `components/`.
7. Copy `k8s/kustomize/` to `warehouses/codeai/freight/git-<full-commit-sha>/kustomize/`
