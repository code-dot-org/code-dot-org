...
contract for this plan family. Implementors should start from that canonical
...
- keeps the rendering contract entirely within Kustomize
...
- Introduce `kustomize-build` promotion steps.
...
The kustomize flow is shown explicitly below.
...
Kustomize CI stays the same on the CI side; the workflow only emits image tags and build-lock metadata, and rendering happens later at promotion time through `packaging.kind` and `sourcePath`.
...
the promoted release will later pin a Kustomize entrypoint,
...
those hints to decide whether to render source into a branch,
...
- `packaging.kind` tells the stage that it should later run Kustomize
...
- whether the render engine uses Kustomize
...
plus sparse paths. That means narrowing the sparse checkout to `k8s/kustomize`. The
...
- `vars.sourceCommit` comes from the build-lock helper or from Git Freight
  itself
- for Kustomize phases, sparse checkout can be narrowed to just `k8s/kustomize`
