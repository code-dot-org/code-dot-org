# argo-trace-old stress harness

This directory holds the live stress harness for `bin/argo-trace-old`.
...
The runner is `run.rb`. It writes each captured `argo-trace-old` output under:

- `logs/argo-trace-old-stress-test/<timestamp>/`
...
```sh
ruby /Users/seth/src/code-dot-org/k8s/tofu/codeai-k8s/cluster-infra-argocd/test/argo-trace-old-stress-test/run.rb
```
...
- capture one-shot `bin/argo-trace-old` output at key checkpoints
...
`bin/argo-trace-old` on this shared dev cluster. Rootless tracing is supposed to
