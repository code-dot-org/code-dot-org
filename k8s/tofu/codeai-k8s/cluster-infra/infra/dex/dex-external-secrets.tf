...
# 3) An IRSA-annotated service account consumed by the static cluster-infra-argocd Dex chart

locals {
  dex_secret_prefix                         = "k8s/tofu/${var.cluster_name}"
  dex_eso_oidc_host                         = replace(var.cluster_oidc_issuer_url, "https://", "")
...
data "aws_iam_policy_document" "dex_external_secrets" {
  statement {
    ...
    resources = [
      "arn:aws:secretsmanager:${var.cluster_region}:${data.aws_caller_identity.current.account_id}:secret:${local.dex_google_client_secret_aws_secret_name}*",
      "arn:aws:secretsmanager:${var.cluster_region}:${data.aws_caller_identity.current.account_id}:secret:${local.dex_google_service_account_key_aws_name}*",
    ]
  }
}
