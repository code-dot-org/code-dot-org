...
1. The AWS IAM role for that service account is scoped to be able to access only AWS Secrets matching: ([`eso-per-env/iam.tf`](../../tofu/codeai-k8s/eks-cluster-addons/modules/eso-per-env/iam.tf))
   - `staging/cdo/*`
   - `CfnStack/staging/*`
1. Each dashboard helm deployment provisions an `ExternalSecret` which configures the External Secrets Operator to periodically sync AWS secrets matching `staging/cdo/*` into Kubernetes Secret `cdo-secrets` (see: [`aws-secrets-manager.yaml`](../templates/aws-secrets-manager.yaml))
...
1. When a dashboard pod launches, its manifest specifies that Kubernetes secret `cdo-secrets`'s sub-keys, including `db_writer`, will be mapped onto its environment variables, each with a `CDO_` prefix. (see: [`_dashboard.yaml`](../templates/dashboard/_dashboard.yaml))
1. Therefore, the `db_writer` key from `cdo-secrets` will be available as the `CDO_db_writer` env var inside the pod.
...
- `cdo-secrets-local` is a Kubernetes `Secret` object with many sub-secrets (see: [`cdo-secrets-local.yaml`](../templates/cdo-secrets-local.yaml))
