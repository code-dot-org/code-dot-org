...
COPY k8s/mimic/docker-mimic/docker/mimic-1.txt mimic-1.txt
RUN echo "rebuilding mimic-1: `date`"
COPY k8s/mimic/docker-mimic/docker/package.json package.json
RUN npm install

FROM mimic-base
COPY k8s/mimic/docker-mimic/docker/mimic.txt mimic.txt
