...
SHALL NOT write the Rails session cookie (`_learn_session*`). The legacy
storage-id cookie MAY be written for cookieless anonymous requesters —
`get_file` resolves `get_storage_id`, which creates one; that is parity, and
the path's CDN behavior strips cookies either way.
