...
helper (`test_helper.rb` disables forgery protection globally). Netsim
`<table>@<min_id>` reads: capture one segment with `format: false` and parse
the `@` suffix in the controller. Codeprojects: Sinatra `pass` semantics
port as routes `constraints` lambdas — put both the host check and
`valid_encrypted_channel_id` in the constraint so non-channel paths fall
through to other routes instead of 404ing in the controller.
