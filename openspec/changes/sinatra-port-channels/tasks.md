...
helper — `test_helper.rb` disables forgery protection globally, so a naive
test passes without testing anything. Channel-id route segments need
`format: false` (encrypted ids can contain dots Rails would strip as a
format).
