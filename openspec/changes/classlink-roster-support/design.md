...
- **`enabledUser` is not acted on either** (declined at review, 2026-09; raised as "disabled users are still imported"). The `tobedeleted` reasoning transfers whole: roster membership is the one departure channel we trust, and a second flag with unverified per-district semantics risks excluding students a district considers active. Two more reasons specific to this flag: neither Clever nor Google Classroom applies an equivalent client-side filter, and the failure asymmetry favors importing — a disabled ClassLink user cannot SSO in, so a wrongly imported account sits inert, while a wrongly excluded one is a missing student in a teacher's section. `enabledUser` is deliberately absent from the client's extracted fields (task 3.1c); task 3.1a's mention of it is a string-boolean comparison hazard note, not a filtering requirement. Do not add the filter without reconciling this entry and the `tobedeleted` one above — filtering one flag while ignoring its sibling is indefensible.
