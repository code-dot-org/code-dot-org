...
- [x] 3.1 Create `dashboard/lib/clients/classlink_one_roster.rb` with methods (all collection methods paginate via the 3.2 helper):
  ...
  _Implemented. One value could not be sourced from the repo or the design doc: the proxy host, set to `https://oneroster-proxy.classlink.io` per ClassLink's partner documentation. Nothing in-repo can verify it (no non-production key), so confirm it against the captures' request logs before the production deploy_
...
- [x] 6.6 Add the i18n strings for the failure states (see the spec requirement "Each rostering failure state has specific user-facing copy"):
  ...
  _Implemented: the four strings live in `dashboard/config/locales/base/en.yml` under `classlink_rostering` and are sent as the error JSON's message; the RosterDialog classlink case renders the server message and falls back to an apps-i18n copy of the generic string when none arrives. The no-students state is a 400 that refuses the operation: first import creates no empty section, and a sync to zero leaves the roster untouched. That second half is in tension with the retired roster-shrink guard ("a class that drops to zero is applied; ClassLink is the source of truth") — flagged for review rather than silently resolved either way_
