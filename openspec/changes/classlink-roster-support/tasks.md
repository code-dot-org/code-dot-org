...
- [x] 3.5 Add `CDO.classlink_roster_api_key` to config. **Production only** declares it as `!Secret`, and the secret must be provisioned **before** it is declared — `CDO` is frozen at boot and raises on undefined keys, and `config.ru` eager-loads declared secrets outside development, so declaring ahead of provisioning fails boot. Staging, test, and adhoc get a literal empty string (decided 2026-08-10), which has no secret to provision and so no boot-order risk. _Implemented as a `''` default in the root `config.yml.erb` (every property needs a default there, and the default covers development/staging/test/adhoc in one line) with the `!Secret` override in `config/production.yml.erb` (decided 2026-09-08 to include it in this PR rather than a follow-up). **Merge gate: provision the secret in production Secrets Manager first** — declared secrets are eager-loaded at boot outside development, so deploying the declaration against an unprovisioned secret fails production boot_
...
- [x] 6.9 Gate the entry point behind a DCDO flag (added 2026-09-08): `DCDO.get('classlink-rostering-enabled', false)`, read inline in `teacher_dashboard/show.html.haml` beside the other DCDO reads there. Off hides `classlink` from the providers payload, so PR 2 can merge and deploy dark; enable per environment via DCDO when ready. Deliberately a single inline read rather than a policy object — the flag is temporary, and removal at launch should be a one-line deletion. Scoped to the entry point only: the endpoints stay ungated, since they already require a v2 credential and One Roster class membership, and a teacher who cannot see the button cannot reach them through the product. Test covers the payload with the flag disabled
- [x] 6.10 Post-import section-management parity (raised at review, 2026-09-09): `OwnedSectionsTable.loginInfoFormatter` shows "ClassLink" instead of the raw `CL-…` code, and `SectionActionDropdown` hides "Print login cards" and offers "Sync students from ClassLink" for ClassLink sections, both matching Clever/Google
- [ ] 6.11 **Before flipping the DCDO flag:** add the sync screenshot to the ClassLink login-info page. The page shipped 2026-09-09 with owner-approved sign-in copy (sign in at studio.code.org via 'Continue with ClassLink', or launch from ClassLink) and a text-only sync section — the screenshot `img` in `SectionLoginInfo` renders only when an asset exists. Capture the "Sync students from ClassLink" button on a ClassLink section's Manage Students tab (parity with `syncClever.png`); a local section created via console suffices, no ClassLink access needed
