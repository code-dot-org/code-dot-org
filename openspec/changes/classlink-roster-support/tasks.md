...
- [ ] 4.2 Add a helper to parse `class_sourced_id` from `section.code` (format `CL-<TenantId>|<classSourcedId>`): strip the `CL-` prefix, then take everything after the first `|`. The tenant component is parsed only for display or diagnostics — never used to resolve credentials, which come from `current_user`'s auth option per invariant I1
