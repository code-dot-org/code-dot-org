...
#### Scenario: A v2 signup gains its v1 record at its next sign-in
...
#### Scenario: An account holding its v1 record survives losing its SourcedId
...
#### Scenario: An account holding its v1 record survives a changed SourcedId
...
#### Scenario: Writing the v1 record is idempotent
- **WHEN** a user who already holds both a v1 and a v2 ClassLink auth option signs in
- **THEN** no additional auth option is created

#### Scenario: A connect attempt does not write the credential holder a v1 record
- **WHEN** a user attempts to connect a ClassLink credential held by a different, v2-only account
- **THEN** no v1 auth option is created on the holder's account, because that request may go on to refuse the connect or to take over and destroy the holder — the record is written on the holder's own sign-in instead

#### Scenario: A failed v1-record write does not block sign-in
