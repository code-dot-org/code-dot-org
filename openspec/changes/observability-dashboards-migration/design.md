...
**D1 — Location: two new `frontend/packages/` workspace packages — `@code-dot-org/telemetry-catalog` and `@code-dot-org/grafana-dashboards`.**
`frontend/` gives turborepo, lint-config, typecheck, vitest, and CI for free, and — decisively — `apps/` can only import workspace code via `portal:` links into `frontend/packages/*`, which the catalog requires. The convention objection ("frontend package observing Rails") is already bent by precedent: `frontend/packages/e2e-tests` exercises the full Rails stack. Two packages rather than one because emit sites must import metric names without dragging `@grafana/grafana-foundation-sdk` into the `apps/` dependency graph: `telemetry-catalog` is zero-dependency (types + data only); `grafana-dashboards` depends on `telemetry-catalog` + the SDK. Neither name collides with `core/plugins/observability` or the Rails `observability` engine. Alternatives considered: one combined package (rejected — SDK leaks into apps' node_modules); top-level `observability/` dir mirroring the infra repo (rejected — a third JS workspace with bespoke tooling and no `portal:` reachability).

**D2 — The signal catalog is a typed registry, not just shared string constants.**
Entries carry `{name, kind (counter|gauge|timing), unit, dimensions, owner}` for metrics, plus log-attribute and app-slug tables (the Lab2 attribute set currently duplicated in `lab2.ts`). Emit sites import names from the catalog; dashboard builders accept only catalog entries. Enforcement mechanism: the metric-target helpers in the dashboards package (`sentryMetricsTarget` and the panel kits built on it) change their `metric` parameter from an inline `{name, type, unit}` object to the exported `CatalogMetric` entry type, and the raw-string/inline forms are removed from the public API — so an unregistered signal cannot typecheck. A registry (vs bare constants) is what makes agent-authored dashboards checkable — the builder can typecheck that a panel charts a real signal with the right unit. Catalog also ships in the artifact as `catalog.json` so non-JS consumers (Ruby, future alerting) can read it. Adoption at emit sites is mechanical and incremental; new signals must go through the catalog from day one.

**D3 — Binding via Grafana datasource variables first, placeholders second.**
Dashboards declare `type: datasource` template variables for Prometheus/Sentry/Infinity/CloudWatch refs — the Grafana-native portability mechanism, zero rebinding needed. The variable names are the datasource contract, fixed here: `DS_PROMETHEUS`, `DS_SENTRY`, `DS_INFINITY_SENTRY`, `DS_CLOUDWATCH`. The residue that variables cannot express — Sentry numeric project IDs inside Infinity query bodies, the absolute `runbook_url` host for alert annotations — uses `%%NAME%%` placeholder tokens (e.g. `%%SENTRY_ORG_SLUG%%`, `%%SENTRY_PROJECT_ID_APPS%%`, `%%GRAFANA_BASE_URL%%`) resolved by the consumer at plan time via chained Tofu `replace()` calls. Deliberately NOT `${...}` syntax and NOT `templatefile()`: Grafana JSON legitimately contains `${DS_PROMETHEUS}`, `${__data.fields.id}`, `${environment}` etc., which `templatefile()` would attempt to interpolate and fail on; `%%…%%` cannot collide and `replace()` is inert to everything else. The infra repo's `config.ts` values become Tofu variables. Alternative rejected: build-time `build(config)` injection — it forces the consumer to run node and means no generic artifact ever exists.

**D4 — Distribution: public OCI artifact on `ghcr.io/code-dot-org`, never committed `dist/`.**
Committing generated JSON was rejected as a smell (generated artifacts are releases, not source). Publishing publicly from the OSS repo is a release, same act as our public npm packages — not a private binding. Artifact layout: `dashboards/*.json`, `alerts/*.json`, `catalog.json`, `runbooks/**`, `manifest.json` (schema version, source SHA, placeholder inventory, SDK version), packed as a single `bundle.tar.gz` layer pushed with `oras push`, annotated with `org.opencontainers.image.source` and `org.opencontainers.image.revision`. Tags: `:main` on every merge (staging channel), semver on release (prod channel), digest-addressable. Signed with GitHub artifact attestations (`actions/attest-build-provenance`) — the GHA-native cosign equivalent; consumers verify with `gh attestation verify --repo code-dot-org/code-dot-org`. Alternatives rejected: npm package consumed by infra (drags node into infra CI); Tofu `git::` module source (go-getter clones the multi-GB monorepo per init); committed-dist vendoring (the smell).
