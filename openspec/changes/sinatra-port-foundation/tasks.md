...
- [ ] 3.1 Create a forgery-protection test helper (e.g.
      `dashboard/test/testing/with_forgery_protection.rb`): a block helper
      that sets `ActionController::Base.allow_forgery_protection = true` and
      restores the prior value in `ensure`. Required because
      `dashboard/test/test_helper.rb:45` disables forgery protection
      globally — without this helper, every CSRF-rejection test in this
      series silently tests nothing. Include a self-test proving a tokenless
      POST is rejected inside the block and accepted outside it.
- [ ] 3.2 Write `dashboard/test/controllers/api/v1/projects_api_base_controller_test.rb`
      with a test route/controller inheriting the base: anonymous GET yields
      no `Set-Cookie`; CAP-locked signed-in user (reuse CAP lockout test
      setup from existing `handle_cap_lockout` coverage) is not redirected;
      mid-LTI-linking user (stub `Policies::Lti.account_linking?` true) is
      not redirected; non-GET without CSRF token rejected (inside the 3.1
      helper); the `Api::V1::JSONApiController` 403-on-RecordNotFound rescue
      is absent (a raised `ActiveRecord::RecordNotFound` is not rescued to
      403).
- [ ] 3.3 Implement
      ...
      :handle_cap_lockout, :assert_lms_landing_policy`. Green.
