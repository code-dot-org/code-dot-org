...
skip the `initialize_statsig_stable_id` and `handle_cap_lockout` before
actions and SHALL NOT skip `protect_from_forgery`. It SHALL NOT inherit from
`Api::V1::JSONApiController` (whose `ActiveRecord::RecordNotFound → 403`
rescue conflicts with legacy 404 semantics).
...
- **THEN** the response contains no `Set-Cookie` header and no session is
