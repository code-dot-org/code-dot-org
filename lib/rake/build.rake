        ...
        unless rack_env?(:production)
          ...
          if GitUtils.file_changed_from_git?(schema_cache_file)
            ...
            if rack_env?(:staging)
              ...
              RakeUtils.git_push
              # We pulled changes to make it likely that we can push the new
              # schema_cache.dump file - that probably invalidates this build.
              # Abandon the build now so we can start a new one.
              raise 'Abandoning build after committing schema_cache.dump change'
