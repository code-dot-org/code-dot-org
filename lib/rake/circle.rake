...
namespace :circle do
  ...
  task :run_ui_tests do
    ...
    Dir.chdir(Dir.glob('sc-*-linux')[0]) do
      ...
      RakeUtils.exec_in_background 'for i in 1 2; do ./bin/sc -vv -l $CIRCLE_ARTIFACTS/sc.log -u $SAUCE_USERNAME -k $SAUCE_ACCESS_KEY -i CIRCLE-BUILD-$CIRCLE_BUILD_NUM --tunnel-domains localhost-studio.code.org,localhost.code.org && break; done'
    end
