...
class ShareFilteringTest < Minitest::Test
  ...
  def test_extract_text_blockly_handles_deeply_nested_json
    # Build a deeply nested program JSON string that is <= 999 levels deep without using to_json.
    deep = '"test"'
    995.times do
      deep = %{{"key":#{deep}}}
    end
    ...
    # Should not raise a nesting error, and should simply return [] (no flagged text found).
    assert_nothing_raised do
      result = ShareFiltering.extract_text_blockly(program_json)
      assert_equal [], result
    end
  end

  def test_extract_text_blockly_deeply_nested_json_exception
    # Build a program JSON string that is > 999 levels deep without using to_json.
    deep = '"test"'
    996.times do
      deep = %{{"wrapper":#{deep}}}
    end
    ...
    # Raises a JSON Nesting Error.
    assert_raises(JSON::NestingError) do
      ShareFiltering.extract_text_blockly(program_json)
    end
  end
end
