...
class ImageModerationTest < Minitest::Test
  def test_returns_everyone_when_missing_api_key
    ...
    assert_equal :everyone, ImageModeration.rate_image(StringIO.new('fake-image-body'), 'image/png')
  end
  ...
  def test_uses_azure_when_api_key_present
    ...
    assert_equal :adult, ImageModeration.rate_image(StringIO.new('fake-image-body'), 'image/png')
  end
end
