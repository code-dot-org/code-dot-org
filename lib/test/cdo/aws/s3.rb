...
class CdoAwsS3Test < Minitest::Test
  ...
  def test_cached_exists_in_bucket_caches_list
    ...
    # test has set it.
    Cdo::AwsWrapper::S3.instance_variable_set(:@cached_bucket_contents, nil)

    # we expect the list operation to be called only once per bucket
    Cdo::AwsWrapper::S3.s3.stubs(:list_objects_v2).returns(
      Aws::S3::Types::ListObjectsV2Output.new(
        contents: [Aws::S3::Types::Object.new(key: "test-object")]
      )
    ).once

    refute Cdo::AwsWrapper::S3.instance_variable_get(:@cached_bucket_contents)
    assert Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-object')
    refute Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-nonexistent')
    assert Cdo::AwsWrapper::S3.instance_variable_get(:@cached_bucket_contents)

    Cdo::AwsWrapper::S3.s3.unstub(:list_objects_v2)
  end

  def test_upload_updates_cache
    Cdo::AwsWrapper::S3.instance_variable_set(:@cached_bucket_contents, {BUCKET => Set.new})
    Cdo::AwsWrapper::S3.s3.stubs(:put_object)

    refute Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-nonexistent')
    Cdo::AwsWrapper::S3.upload_to_bucket(BUCKET, 'test-nonexistent', '', {no_random: true})
    assert Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-nonexistent')

    Cdo::AwsWrapper::S3.s3.unstub(:put_object)
  end

  def test_delete_updates_cache
    Cdo::AwsWrapper::S3.instance_variable_set(:@cached_bucket_contents, {BUCKET => Set['test-object']})
    mock = MiniTest::Mock.new
    def mock.delete_marker
    end
    Cdo::AwsWrapper::S3.s3.stubs(:delete_object).returns(mock)

    assert Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-object')
    Cdo::AwsWrapper::S3.delete_from_bucket(BUCKET, 'test-object')
    refute Cdo::AwsWrapper::S3.cached_exists_in_bucket?(BUCKET, 'test-object')

    Cdo::AwsWrapper::S3.s3.unstub(:delete_object)
  end

  def test_get_console_link_from_presigned_rejects_unexpected_input
    assert_raises(ArgumentError) {Cdo::AwsWrapper::S3.get_console_link_from_presigned("")}
    assert_raises(ArgumentError) {Cdo::AwsWrapper::S3.get_console_link_from_presigned("https://example.com//test/20230222T200104%2B0000")}
    assert_raises(ArgumentError) {Cdo::AwsWrapper::S3.get_console_link_from_presigned("https://some-bucket.s3.amazonaws.com")}
  end
  ...
  def test_get_console_link_from_presigned_returns_transformed_url
    ...
    assert_equal(expected_url, Cdo::AwsWrapper::S3.get_console_link_from_presigned(presigned_url))
  end
end
