...
class TestEC2Autoscaling < Minitest::Test
  ...
  def test_refresh_instances_raises_when_no_groups_found
    ...
    assert_raises(RuntimeError) {AWS::EC2Autoscaling.refresh_instances('nonexistent-group')}
  end

  def test_refresh_instances_raises_when_too_many_groups_found
    ...
    assert_raises(RuntimeError) {AWS::EC2Autoscaling.refresh_instances('overloaded-group')}
  end

  def test_refresh_instances_triggers_instance_refresh_with_default_parameters
    ...
    AWS::EC2Autoscaling.refresh_instances('test-group')
  end

  def test_refresh_instances_does_not_wait_for_instance_refresh_to_complete_by_default
    describe_response = Aws::AutoScaling::Types::AutoScalingGroupsType.new(auto_scaling_groups: mock_autoscaling_groups)
  ...
  def test_refresh_instances_waits_until_instance_refresh_is_complete_if_flag_passed
    describe_response = Aws::AutoScaling::Types::AutoScalingGroupsType.new(auto_scaling_groups: mock_autoscaling_groups)
    Aws::AutoScaling::Client.any_instance.stubs(:describe_auto_scaling_groups).returns(describe_response)
    Aws::AutoScaling::Client.any_instance.expects(:start_instance_refresh).with(auto_scaling_group_name: 'test-group')

    Aws::AutoScaling::Client.any_instance.expects(:wait_until).with(:instances_healthy, auto_scaling_group_names: ['test-group'])
    AWS::EC2Autoscaling.refresh_instances('test-group', wait: true)
  end
end
