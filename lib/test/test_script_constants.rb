...
class ScriptConstantsTest < Minitest::Test
  ...
  def test_assignable_info
    ...
    assert_equal 2, ScriptConstants.assignable_info({name: 'aquatic'})[:position]
    assert_equal 3, ScriptConstants.assignable_info({name: 'hero'})[:position]
    assert_equal 4, ScriptConstants.assignable_info({name: 'mc'})[:position]
    assert_equal 5, ScriptConstants.assignable_info({name: 'minecraft'})[:position]
  end
