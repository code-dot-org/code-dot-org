...
module ImageModeration
  ...
  def self.rate_image(image_data, content_type, image_url = nil)
    puts "rate_image #{image_url} with content_type: #{content_type}" if image_url
    return :everyone unless CDO.azure_content_moderation_key
    rating = AzureContentModerator.new(
    ...
    puts "Image moderation rating: #{rating} for image_url: #{image_url}" if image_url
    return rating
