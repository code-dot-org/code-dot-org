    ...
    def call(env)
      ...
      if TARGET_HOSTNAMES.include?(request.hostname)
        if PATH_PATTERN.match?(request.path_info)
          ...
          redirect_uri = URI(::File.join(request.script_name, ROOT_PATH, ge_region, request.path_info))
          redirect_uri.query = URI.encode_www_form(request.params) unless request.params.empty?

          response = Response.new
          response.redirect(redirect_uri.to_s, 302)
