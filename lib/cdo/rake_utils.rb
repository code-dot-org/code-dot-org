...
module RakeUtils
  ...
  def self.upload_file_to_s3_bucket_and_create_fetch_file(local_file, destination_local_path, params = {})
    ...
    s3_filename = Cdo::AwsWrapper::S3.upload_to_bucket(params[:bucket], File.basename(local_file), File.open(local_file), acl: 'public-read')
    new_fetchable_url = Cdo::AwsWrapper::S3.public_url(params[:bucket], s3_filename)
