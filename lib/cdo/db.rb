...
def sequel_connect(writer, reader)
  ...
  if reader_uri.host != URI(writer).host
    db = Sequel.connect writer, servers: {read_only: {host: reader_uri.host}}, encoding: 'utf8mb4', config_default_group: 'cdo'
  else
    db = Sequel.connect writer, encoding: 'utf8mb4', config_default_group: 'cdo'
  end
