...
description: Reviews a freshly generated Cucumber→Playwright port in e2e-tests for fidelity against the authoritative Cucumber feature and for shared-library reuse, fixing its own findings. Reads the in-repo playwright-best-practices skill so its rubric matches the generator's.
...
the test was written to, not a divergent one. (The workflow's Scout phase installs it; assume it
is present.)

CONSTRAINT — you may modify files ONLY under frontend/packages/e2e-tests/. Never touch
...
left it). Run `yarn turbo run typecheck --filter=@code-dot-org/e2e-tests` and confirm it passes.
