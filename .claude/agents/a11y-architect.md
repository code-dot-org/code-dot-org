...
- **Semantics come from components, not bespoke code.** Reach for the design system's
  ...
  behavior out of the box. Use a raw semantic element only where no component fits; never
...
- **Lint + types:** `./tools/hooks/pre-commit` (changed files); `yarn typecheck` in `apps/` for
  TS. Fix `jsx-a11y` violations — don't disable them; a justified disable is one line with a why.
