export const meta = {
  ...
  description: 'Port one Cucumber feature file to Playwright (TypeScript) in apps-e2e-tests',
  phases: [
    {title: 'Scout'},
    {title: 'Dry Run'},
    {title: 'Generate'},
...
const SCOUT_SCHEMA = {
  ...
  properties: {
    ...
    targetSpec: {
      type: 'string',
      description:
        'Repo-root-relative spec path: frontend/packages/apps-e2e-tests/tests/' +
    ...
    workingTreeFoundationPresent: {
      type: 'boolean',
      description:
        'true if frontend/packages/apps-e2e-tests/tests/shared/ exists in the working ' +
...
─── STEP 4 — Fix target paths ─────────────────────────────────────────────────
  Place all files under frontend/packages/apps-e2e-tests/tests/{featureGroup-kebab}/
...
─── STEP 5 — Step resolution (RECOMMENDATIONS only) ───────────────────────────
  ...
  the working tree lacks the foundation):
    ...
    - Reference branch (TypeScript SHAPE only):
      git show ${POC_BRANCH}:frontend/packages/apps-e2e-tests/tests/shared/auth.ts  (etc.)
...
─── STEP 1 — Write a throwaway probe ──────────────────────────────────────────
  ...
  Playwright availability: prefer the apps-e2e-tests install; else \`npx playwright\`
...
apps-e2e-tests). Self-typecheck and self-lint before returning, and fix all issues.
...
GENERATED FILES: under frontend/packages/apps-e2e-tests/tests/${plan.featureGroup} (spec: ${plan.targetSpec}).
...
find inside apps-e2e-tests only, then confirm typecheck passes. Do NOT run the test suite —
...
const specRel = plan.targetSpec.replace('frontend/packages/apps-e2e-tests/', '')
  ...
  await agent(
    `Stabilize the spec at ${plan.targetSpec}; it FLAKED under the 5x/all-browser gate ` +
      ...
      result.failures
        ...
        .join('\n') +
      `\nDiagnose the ROOT CAUSE from the traces and fix it within apps-e2e-tests only, per ` +
...
const body =
  healStatus === 'green'
    ? `Port ${plan.scenarios.length} ${scenarioWord} from ${featureFile}.\n` +
      `Green under the 5x/all-browser stress gate; original Cucumber feature removed.\n\n` +
      `Source: ${featureFile}`
    : `Port ${plan.scenarios.length} ${scenarioWord} from ${featureFile}.\n` +
...
MSG
   ...
   (inside apps-e2e-tests only), re-stage, and commit again — do not bypass the hook.
4. Print the result: git log -1 --oneline --stat

Do NOT push. Do NOT open a pull request. Stay within apps-e2e-tests (plus the feature
deletion on green).`,
