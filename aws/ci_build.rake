...
task :regular_ui_tests => [UI_TEST_SYMLINK] do
  Dir.chdir(dashboard_dir('test/ui')) do
    ...
    failed_browser_count = RakeUtils.system_with_hipchat_logging 'RAILS_ENV=test', 'bundle', 'exec', './runner.rb', '-d', 'test-studio.code.org', '--parallel', '70', '--magic_retry', '--html', '--fail_fast'
...
task :eyes_ui_tests => [UI_TEST_SYMLINK] do
  Dir.chdir(dashboard_dir('test/ui')) do
    ...
    failed_browser_count = RakeUtils.system_with_hipchat_logging 'RAILS_ENV=test', 'bundle', 'exec', './runner.rb', '-c', 'ChromeLatestWin7,iPhone', '-d', 'test-studio.code.org', '--eyes', '--html', '-f', eyes_features.join(","), '--parallel', (eyes_features.count * 2).to_s
    if failed_browser_count == 0
