...
task :dashboard_eyes_ui_tests do
  Dir.chdir(dashboard_dir) do
    Dir.chdir('test/ui') do
      ...
      failed_browser_count = RakeUtils.system_ 'bundle', 'exec', './runner.rb', '-c', 'ChromeLatestWin7', '-d', 'test-studio.code.org', '--eyes', '--html', '--auto_retry', '-f', 'features/applab.feature,features/contractEditor.feature,features/eyes.feature', '-p', '3'
      if failed_browser_count == 0
