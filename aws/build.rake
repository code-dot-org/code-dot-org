...
task :dashboard_browserstack_ui_tests do
  Dir.chdir(dashboard_dir) do
    Dir.chdir('test/ui') do
      ...
      failed_browser_count = RakeUtils.system_ 'bundle', 'exec', './runner.rb', '-d', 'test-studio.code.org', '-p', '10', '--auto-retry', '--html'
...
task :dashboard_eyes_ui_tests do
  Dir.chdir(dashboard_dir) do
    Dir.chdir('test/ui') do
      ...
      failed_browser_count = RakeUtils.system_ 'bundle', 'exec', './runner.rb', '-c', 'Chrome33Win7', '-d', 'test-studio.code.org', '--eyes', '--html', '--auto-retry'
      if failed_browser_count == 0
