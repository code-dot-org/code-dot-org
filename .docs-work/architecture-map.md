...
Coarse-grained developer subsystems, sized so an orchestrator can carve
developer-doc domains. Not a design document: each entry names the subsystem,
what it is for, where it lives, where to start reading, what already documents
it, what it depends on outside the repo, and what this pass could not resolve.

The eight developer-doc areas in `taxonomy.md` map onto these subsystems as
follows.

| Area | Subsystems |
|---|---|
| dev-rails-platform | Rails app layout, non-Rails Ruby, Sinatra middleware, background jobs, project storage, email |
| dev-frontend-platform | apps/ webpack bundle, frontend/ Turborepo, design system, labs runtime contract, Blockly |
| dev-auth-and-authz | Auth, authorization plumbing |
| dev-curriculum-pipeline | Curriculum data model and its file round trip |
| dev-availability-and-config | DCDO, Gatekeeper, experiments, pilots, Rails environments (detailed in availability-gates.md) |
| dev-testing-and-verification | Test suites, Cucumber UI, Playwright e2e, test-only APIs, seeds (detailed in verification-infra.md) |
| dev-deploy-and-infra | Deploy and CI, caching and CDN, observability |
| dev-i18n | i18n and Global Edition |

## The one-paragraph version

`dashboard/` is a Rails monolith with 164 controllers, 350 models, 940
migrations and a 3133-line schema. Six legacy Sinatra apps are mounted as
ordered Rack middleware in front of it and still serve the project and asset
APIs. Repo-root `lib/` and `shared/` hold Ruby and assets that predate the
split between this app and pegasus, loaded through `deployment.rb` before
Rails boots. Curriculum is authored in a separate `levelbuilder` Rails
environment, serialized to roughly 39,000 files under `dashboard/config`, and
seeded one-way into every other environment including production. The
client is two things at once: one large webpack bundle under `apps/` that
every level page boots through a `window.appOptions` blob, and a growing
Turborepo of Vite packages under `frontend/` mounted at `/frontend-studio`.
Production runs on Chef-provisioned EC2 with CloudFront in front; a
k8s and Kargo path exists, is actively developed, and is not yet the deploy
path of record.

## Cross-cutting facts worth stating once

- The `stages` database table is the `Lesson` model. Nothing in the repo warns
  a newcomer about that.
- `Services::`, `Policies::` and `Queries::` live under `dashboard/lib/`, not
  `dashboard/app/`. There is no `dashboard/app/services`.
- The session cookie is `_learn_session` with `domain: :all`, which is what
  makes sign-in shared across `.code.org`, studio and pegasus alike.
- `Rails.cache` is a per-server file store, not shared between app servers.
  Anything that assumes a shared cache is wrong.
- Three asset pipelines coexist: forked Sprockets, the `apps/` webpack bundle
  reached through the `dashboard/public/blockly` symlink, and Vite for
  frontend-studio.
- There is no `dashboard/README.md`. A developer entering the Rails app has no
  local orientation document.
...
Purpose: One large webpack bundle holding most of the client. Every level page and most teacher tools are served from it. Reached at runtime through the `dashboard/public/blockly` symlink created by `rake package:apps:symlink`, which is why `use_my_apps: true` plus that symlink is the local-dev prerequisite.
Dirs: `apps/src` has 131 top-level entries. By role: 25 lab directories (aichatLab, ailab, applab, bounce, bubbleChoice, craft, dance, fish, flappy, flowlab, javalab, jigsaw, maker, maze, music, netsim, p5lab, panels, publicKeyCryptography, pythonlab, sketchlab, standaloneVideo, turtle, weblab, weblab2); 6 teacher-tool directories (aiDifferentiation, aiEvaluation, aiTeacherDrawer, courseExplorer, levelbuilder, schoolInfo); 35 shared or infrastructure directories, of which `templates/` (188 entries) and `code-studio/` are the two largest; and 4 others (generated, regionalPartnerMiniContact, sites, third-party).
Entry points: `apps/src/appMain.js`, `apps/src/StudioApp.js`, `apps/src/code-studio/appOptions.js`, `apps/webpack.config.js`.
Docs: `apps/README.md`, `apps/docs/build.md`, `STYLEGUIDE.md`, `apps/.storybook/README.md`.
External deps: webpack, with rspack available opt-in via `--rspack`; Blockly as a vendored fork.
Uncertainties: `apps/src/templates` is a 188-entry grab bag mixing teacher tools, shared components and page shells. It is the single least-structured directory in the repo and the hardest thing to draw a doc boundary around.

### frontend/ Turborepo

Purpose: The newer client. Standalone Vite and TypeScript packages, published or consumed independently, mounted into studio at `/frontend-studio` and gated by the `frontend_studio_enabled` DCDO flag. This is where new work is meant to go.
Dirs: `frontend/packages` holds 19 real packages: ai-tutor, audio, blockly, blockly-workspace, changelogs, component-library, component-library-styles, core, e2e-tests, fonts, labs, lesson-deep-dive, lint-config, markdown, platform, playwright-support, progress, teacher-dashboard, users. `frontend/apps` holds studio (the host app), design-system-storybook, and mobile (an untracked Capacitor experiment). `frontend/packages/labs` contains ailab, base (the Lab2 host shell), music and oceans.
Entry points: `frontend/turbo.json`, `frontend/apps/studio`, `dashboard/app/controllers/frontend_studio_controller.rb`.
Docs: `frontend/AGENTS.md`, `frontend/README.md`, and per-package READMEs.
External deps: Vite, Turborepo, MUI alongside the in-house DSCO component library.
Uncertainties: several directories under `frontend/packages` contain only a `dist/` with no `package.json` and are untracked local cruft rather than real packages. Two features exist twice, once in `apps/src` and once in `frontend/packages`: music lab and localization. Which serves production is not recorded.

### Design system

Purpose: The shared component library and its styles, plus the DSCO-to-MUI migration.
Dirs: `frontend/packages/component-library`, `frontend/packages/component-library-styles`, `frontend/packages/fonts`.
Entry points: `frontend/apps/design-system-storybook`.
Docs: many per-component READMEs, several frozen at "Status: Ready for Dev".
External deps: MUI.
Uncertainties: which components have been migrated to MUI is not recorded in one place. This is the only part of the repo with a dedicated CODEOWNERS review group (`component-library-engineering-reviewers`).
