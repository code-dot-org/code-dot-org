...
How each role is represented in code and where it is enforced. The three
end-user roles named in the product are Student, Teacher and District
administrator. Only the first two exist in the data model; the third does not,
and section 5 gives the evidence. Claims are tagged STRONGLY SUPPORTED
(code+test) / OBSERVED (db or rails runner) / INFERRED / AMBIGUOUS.

## Summary for the impatient

- `users.user_type` has exactly two values, `student` and `teacher`, and doubles
  as the Rails STI inheritance column.
- Everything finer-grained is a `UserPermission` row, and `permission?` returns
  false for any student regardless of what rows exist.
- `users.admin` is a separate plain boolean column meaning Code.org staff
  superuser. It is not a UserPermission and is not gated on `teacher?`.
- There is no district administrator role. Clever's `district_admin` and
  `school_admin` sign-ins are collapsed to plain `teacher`, and `educator_role`
  is self-reported profile text that no authorization code reads.
- Classroom authority is per-object, not global: `Section#user_id` is the one
  owning teacher, `section_instructors` are additional co-teachers, and
  `followers` are the students.
- Who may teach and who may take a given course is decided by two further
  axes, `instructor_audience`/`participant_audience` on the course and
  `participant_type` on the section. See section 6.
