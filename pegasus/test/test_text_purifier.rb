...
class TextPurifierTest < Minitest::Unit::TestCase
  ...
  def test_address_violations
    assert_nil(RegexpUtils.find_potential_street_address('this is just some text'))
    assert(RegexpUtils.find_potential_street_address('1 Embarcadero Blvd SF CA').is_a? StreetAddressViolation)
    assert_equal('1 Embarcadero Blvd SF CA', RegexpUtils.find_potential_street_address('1 Embarcadero Blvd SF CA'))
    assert_equal('123 Post Road Westport CT', RegexpUtils.find_potential_street_address('Hi I live at 123 Post Road Westport CT'))
    assert_equal('123, Post Road, Westport, CT', RegexpUtils.find_potential_street_address('Hi I live at 123, Post Road, Westport, CT'))
    assert_equal('123, Post Road, Westport, CT and other stuff', RegexpUtils.find_potential_street_address('Hi I live at 123, Post Road, Westport, CT and other stuff'))
    assert_nil(RegexpUtils.find_potential_street_address('I am the luckiest 1st 2 and 3rd person in California'))
    assert_nil(RegexpUtils.find_potential_street_address('Hi I am dog I live at 3 Rover Road in Christmasville MA 12346'))
  end
