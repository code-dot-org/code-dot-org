...
remoteStorage.setItem = function(keyWord, theValue){
    remoteStorage.setItem(keyWord, theValue, console.debug);
}

remoteStorage.setItem = function(keyWord, theValue, callback){
...
remoteStorage.getAllStartsWith = function(startsWith, callback){

  Parse.initialize("IuyeuvogPwCIzHohhXwldAjHfFa7FprF2wKjPSCj", "0Tgr28yVJGg8p4AFfSRmT40DiYrPS6sPKw7C2Juo");
  var RemoteObj = Parse.Object.extend("RemoteStorage");
  var query = new Parse.Query(RemoteObj);
  query.startsWith("key",startsWith);

  query.find({
    success: function(obj){

        getResult = obj;
       // console.log("Object found. 'getResult' set to "+obj);
        var resultHash = new Array();

        for(var i=0; i<obj.length; i++){
            resultHash[obj[i].get("key")]=obj[i].get("value");
        }
        getResult = resultHash;
  ...
  query.find({
    success: function(obj){
      ...
      if(obj.length>0 && obj[0].get("value")!=null){
        ...
        var list = [];
        for(var i=0; i<obj.length; i++){
            list[i] = obj[i].get("value");
        }

        getResult = list;
        //console.debug("Object found. 'getResult' set to "+list);
...
remoteStorage.removeItem = function(keyWord){
  ...
  var RemoteObj = Parse.Object.extend("RemoteStorage");
  var query = new Parse.Query(RemoteObj);
  console.log("Attempting to remove item with key="+keyWord);
  query.equalTo("key",keyWord);
...
remoteStorage.updateOrCreate = function(keyWord, newValue){
  remoteStorage.updateOrCreate(keyWord,newValue, parseTaskFinished);
}

remoteStorage.updateOrCreate = function(keyWord, newValue, callback){
  ...
  query.find({
    success: function(obj){
      ...
      if(obj.length>0 && obj[0].get("value")!=null){
        ...
        item.save();
        console.log("UPDATED: "+keyWord+" to "+newValue);
        //getResult = obj[0].get("value");
      } 
      else{
        console.debug("UpdateOrCreate keyword "+keyWord+" not found");
        remoteStorage.setItem(keyWord, newValue);
        console.debug("Made New Entry: "+keyWord+"->"+newValue);
...
remoteStorage.updateItem = function(keyWord, newValue){
  Parse.initialize("IuyeuvogPwCIzHohhXwldAjHfFa7FprF2wKjPSCj", "0Tgr28yVJGg8p4AFfSRmT40DiYrPS6sPKw7C2Juo");
  var RemoteObj = Parse.Object.extend("RemoteStorage");
  var query = new Parse.Query(RemoteObj);
  query.equalTo("key",keyWord);
