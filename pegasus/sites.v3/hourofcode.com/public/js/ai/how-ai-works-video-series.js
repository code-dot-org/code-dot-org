...
function reorganizeAndWrapSlides(carouselId, cardsList) {
  ...
  const actionBlocks = Array.from(
    carouselContainer.querySelectorAll(
      ".video-responsive.video-wrapper.video-wrapper--one-col"
    )
  );
...
$(document).ready(function () {
  const howAiWorksVideos = "#how_ai_works_videos";
  handleScreenSize("how_ai_works_videos");
  $(howAiWorksVideos + " .slides").carouFredSel({
    auto: false,
    pagination: "#how_ai_works_videos-pagination",
    prev: "#prev_1",
    next: "#next_1",
    responsive: true,
    height: "490",
