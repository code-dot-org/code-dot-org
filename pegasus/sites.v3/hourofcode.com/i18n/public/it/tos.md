...
Leggi attentamente questi Termini di Servizio ("**Termini**") prima di accedere o di utilizzare i Servizi. Accedendo o usando di qualsiasi parte dei Servizi o cliccando su un pulsante "Sono d'Accordo" o "Registrati" disponibile all'utente significa che l'utente accetta di essere vincolato da questi Termini senza alcuna modifica.

Questi termini sono un contratto legale tra te e Code.org riguardante il tuo uso dei servizi. Si possono utilizzare i servizi solo se viene stipulato un contratto vincolante con Code.org e se si è una persona ai cui è vietato ricevere servizi ai sensi delle leggi degli Stati Uniti o in un altra giurisdizione applicabile. Se si accettano queste condizioni e vengono utilizzati i servizi a nome di una compagnia, di un'organizzazione, di un governo, o di un'altra entità legale, si dichiara e garantisce di essere autorizzati a farlo. È possibile utilizzare i servizi solo in conformità con questi termini e tutte le leggi locali, statali, nazionali e internazionali applicabili, norme e regolamenti.

Queste condizioni sono soggette a modifiche. Se si continuano a utilizzare i servizi dopo che vengono modificati i termini si accetta di essere vincolati da tali modifiche, si prega quindi di controllare periodicamente le modifiche a queste condizioni.
...
Code.org ti permette di registrare un account per i Servizi tramite l'utilizzo di servizi di terze parti, tipo Facebook Connect e Google (“**Servizio di Autenticazione**”). Registrandoti ai Servizi utilizzando un Servizio di Autenticazione, acconsenti all'accesso di Code.org alle informazioni del tuo account tramite il Servizio di Autenticazione, e acconsenti a ciascuno dei termini di utilizzo del Servizio di Autenticazione riguardo l'uso del Servizio tramite il Servizio di Autenticazione. Tu sei l'esclusivo responsabile delle tue interazioni con il Servizio di Autenticazione.
