...
### Få alle skoler til at deltage, alle elever skal lære!
  ...
  1. **Få lokale skoler og kommunen til at deltage.** Brug [ denne e-mail](<%= hoc_uri('/resources#sample-emails') %>) eller [ denne korte tekst](<%= hoc_uri('/resources/stats') %>) som udgangspunkt og tage et kig på denne [guide](<%= hoc_uri('/resources/how-to') %>) til skoler og kommunen. Let them know **every** participating educator will receive a [thank you gift](<%= hoc_uri('/resources/how-to') %>) and in **each state**, one participating school will win **$10,000 in technology**!
