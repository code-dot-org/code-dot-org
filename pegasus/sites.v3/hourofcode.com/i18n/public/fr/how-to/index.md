...
Rejoignez le mouvement et initiez un groupe d'élèves à leur première heure d'informatique grâce à ces étapes. L'Heure de Code est facile à exécuter - même pour les débutants! If you'd like an extra set of hands to help out, you can find a [local volunteer](<%= codeorg_url('/volunteer/local') %>) to help run an Hour of Code in your class.
...
Nous fournissons une variété de [tutoriels auto-guidés](<%= resolve_url('/learn') %>) amusants pour des groupes de tous âges et de tous niveaux d'expérience. Les élèves pratiquent les activités par eux-même, toutefois bon nombre d'entre elles incluent un plan de cours à destination des enseignants (vous trouverez le lien lorsque vous cliquerez sur l'activité) pour guider les échanges et enrichir l'expérience. [![](/images/fit-700/tutorials.png)](<%=resolve_url('/learn') %>)

## 3. Faire la promotion de votre Heure de Code

Faites la promotion de votre Heure de Code [grâce à ces outils](<%= resolve_url('/promote/resources') %>), et encouragez d'autres personnes à organiser leurs propres événements.
