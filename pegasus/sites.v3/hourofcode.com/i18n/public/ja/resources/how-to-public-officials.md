...
### すべての学校が参加して、 すべての生徒が学ぶ！
  ...
  1. **地域の学校や教育機関に参加を呼びかけてください。**[この推薦文](<%= hoc_uri('/resources#sample-emails') %>このメール</a>や<a href=)を参考に使ってください。また、この学校・教育機関向けの[手引き](<%= hoc_uri('/resources/how-to') %>)を参照してください。 Let them know **every** participating educator will receive a [thank you gift](<%= hoc_uri('/resources/how-to') %>) and in **each state**, one participating school will win **$10,000 in technology**!

  2. **Hour of Code（コードの１時間）のイベントを自分で開きましょう！**[イベントのガイドブック](<%= hoc_uri('/resources/how-to-event') %>)にイベントの内容、準備、マスコミへのコミュニケーションの説明を参考してください。 イベントを開くのが関心を高めて、これからの参加者を増やす効果があります。
