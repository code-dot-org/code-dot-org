...
## 6) Započnite svoj Sat Kodiranja
...
Svaki student dobiva priliku da dobije potvrdu putem e-pošte kada završe [Code.org tutoriali](http://studio.code.org). Možete kliknuti na certifikat da bi ga tiskali. Ukoliko pak želite nove certifikate za svoje učenike, posjetite stranicu [Certifikati](http://<%= codeorg_url() %>/certificates) da biste otisnuli koliko god certifikata želite odjednom!

**Što poslije Sata Kodiranja?**

Sat Kodiranja je samo prvi korak na putu da bismo saznali više o načinu funkcioniranja tehnologije i kako napraviti softver aplikacije. <% if @country == 'uk' %> Sat Kodiranja je samo prvi korak na putu učenja o tome kako tehnologija funkcionira i kako izraditi softverske aplikacije. Da biste nastavili ovo putovanje, [Potičite svoju djecu da uče online](http://uk.code.org/learn/beyond). < % drugog %> da nastaviš s ovim, pronađi dodatne resurse za nastavnike [ ovdje](http://<%= codeorg_url() %>/ educirati). Ili ohrabrite svoju djecu da uče putem [interneta](http://<%= codeorg_url() %>/learn/beyond). <% end %>