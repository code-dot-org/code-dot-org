...
def all_unique_events
  DB[:forms].where(kind: 'HocSignup2015').group(:email, :name).select(:name, :email, :processed_data, :data)
end
...
def total_hoc_count
  DB[:forms].where(kind: 'HocSignup2015').group(:email, :name).select(:email, :name).count
end
