...
exports.install = function(blockly, blockInstallOptions) {
  ...
  generator.draw_move_by_constant = function() {
    ...
    return 'Calc.' + this.getTitleValue('DIR') +
  ...
  blockly.Blocks.draw_a_snowman = {
    // Draw a circle in front of the Calc, ending up on the opposite side.
    init: function() {
      this.setHSV(94, 0.84, 0.60);
      this.appendDummyInput()
  ...
  generator.draw_a_snowman = function() {
    // Generate JavaScript for drawing a snowman in front of the Calc.
  ...
  var SimpleMove = {
    ...
    generateCodeGenerator: function(direction, hasLengthInput, length) {
      return function() {
        ...
        return 'Calc.' + SimpleMove.DIRECTION_CONFIGS[direction].moveFunction + '(' + length + ',' + '\'block_id_' + this.id + '\');\n';
      };
    }
  };
  ...
  generator.draw_width = function() {
    ...
    return 'Calc.penWidth(' + width + ', \'block_id_' + this.id + '\');\n';
  };
  ...
  generator.draw_pen = function() {
    ...
    return 'Calc.' + this.getTitleValue('PEN') +
        '(\'block_id_' + this.id + '\');\n';
  };
  ...
  generator.draw_colour = function() {
    ...
    return 'Calc.penColour(' + colour + ', \'block_id_' +
        this.id + '\');\n';
  };
  ...
  blockly.Blocks.up_big = {
    ...
    init: function() {
      ...
      this.setTooltip(msg.CalcVisibilityTooltip());
    }
  };
  ...
  blockly.Blocks.Calc_visibility = {
    // Block for changing Calc visiblity.
    helpUrl: '',
    init: function() {
      this.setHSV(184, 1.00, 0.74);
      this.setPreviousStatement(true, null);
  ...
  blockly.Blocks.Calc_visibility.STATE =
      [[msg.hideCalc(), 'hideCalc'],
       [msg.showCalc(), 'showCalc']];

  generator.Calc_visibility = function() {
    // Generate JavaScript for changing Calc visibility.
    return 'Calc.' + this.getTitleValue('VISIBILITY') +
        '(\'block_id_' + this.id + '\');\n';
  };
