...
`adhoc`, and `production`. One layer shared by web and worker beats two
near-duplicate private ones on a node running both, so the union wins even
...
libraries. So mysql2, rmagick and nokogiri are each loaded for real. The toolchain checks are inverted — a passing `command -v cc` is a
...
`ruby` variant of every platform-specific gem, building from source what
amd64 takes prebuilt. That is a different artifact, not the same image for
