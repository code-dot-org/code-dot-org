...
**Note:** We are moving to a new naming convention in order to use different docker images for different purposes. What was the `code-dot-org` image will become the `cdo-drone` image. For now, these docs advise publishing both names.
...
docker push codedotorg/cdo-drone:<version>
```
