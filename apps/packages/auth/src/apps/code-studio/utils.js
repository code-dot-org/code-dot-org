...
// Provide methods that allow tests to mock global.location
let globalLocation = global.location;
export function setWindowLocation(fakeLocation) {
  globalLocation = fakeLocation;
}
...
export function queryParams(name) {
  const parsed = queryString.parse(globalLocation.search);
  if (name) {
    return parsed[name];
  }
  return parsed;
...
export function updateQueryParam(param, value, useReplaceState = false) {
  const newString = queryString.stringify({
    ...queryString.parse(globalLocation.search),
    [param]: value,
  });

  let newLocation = globalLocation.pathname;
  ...
  global.history[method](null, document.title, newLocation);
}
...
export function environmentSpecificCookieName(name) {
  const rack_env = global.dashboard.rack_env;
