...
var displayComplexUserExpressions =
  Calc.__testonly__.displayComplexUserExpressions;
...
function customValidator(assert) {
  ...
  var validateTextElementContainer = function (element, items) {
    for (var i = 0; i < items.length; i++) {
      ...
      assert.equal(
        element.childNodes[i].getAttribute('class'),
        items[i][1],
        'token #' + i
      );
    }
  ...
  displayComplexUserExpressionTest(assert, 'wrong variable name', function () {
    ...
    var userSet = new EquationSet();
    userSet.addEquation_(
      new Equation(null, [], new ExpressionNode('age_in_months2'))
    );
    userSet.addEquation_(new Equation('age', [], new ExpressionNode(17)));
    userSet.addEquation_(
      new Equation('age_in_months2', [], new ExpressionNode('*', ['age', 12]))
    );
    setEquationSets(targetSet, userSet);
  ...
  displayComplexUserExpressionTest(assert, 'divide by zero error', function () {
    ...
    var targetSet = new EquationSet();
    targetSet.addEquation_(
      new Equation(null, [], new ExpressionNode('f', [10]))
    );
    targetSet.addEquation_(
      new Equation(
        'f',
  ...
  displayComplexUserExpressionTest(
    ...
    function () {
      ...
      var userSet = new EquationSet();
      userSet.addEquation_(
        new Equation(null, [], new ExpressionNode('/', [1, 4]))
      );
      var targetSet = new EquationSet(); // simulate free play
      setEquationSets(targetSet, userSet);
  ...
  displayComplexUserExpressionTest(assert, 'repeating fraction', function () {
    ...
    var userSet = new EquationSet();
    userSet.addEquation_(
      new Equation(null, [], new ExpressionNode('/', [1, 9]))
    );
    var targetSet = new EquationSet(); // simulate free play
    setEquationSets(targetSet, userSet);
    ...
    assert.equal(
      text.childNodes[1].getAttribute('style'),
    ...
    function () {
      ...
      userSet.addEquation_(
        new Equation(
          null,
          [],
          new ExpressionNode('f', [new ExpressionNode('+', [1, 1])])
        )
      );
  ...
  displayComplexUserExpressionTest(assert, 'pow block', function () {
    ...
    var userSet = new EquationSet();
    userSet.addEquation_(
      new Equation(null, [], new ExpressionNode('pow', [2, 3]))
    );
  ...
  displayComplexUserExpressionTest(
    ...
    function () {
      ...
      userSet.addEquation_(new Equation('f', ['x'], new ExpressionNode('x')));
      userSet.addEquation_(
        new Equation(null, [], new ExpressionNode('f', [3]))
      );
  ...
  displayComplexUserExpressionTest(
    ...
    function () {
      ...
      assert.equal(userExpression.childNodes.length, 1);
      validateTextElementContainer(userExpression.childNodes[0], [['5', null]]);
    }
  );
