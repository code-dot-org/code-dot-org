...
module.exports = {
  ...
  tests: [
    ...
    {
      description:
        'button image name with spaces and parens is loaded and resized',
      editCode: true,
    ...
    },
    {
      ...
      runBeforeClick: function(assert) {
        var shouldBeResizable = function() {
          ...
          assert.equal(
            resizable.hasClass('ui-resizable'),
            true,
            'is resizable'
          );
          assert.equal(
            resizable.hasClass('ui-draggable'),
            true,
            'is draggable'
          );
      ...
      runBeforeClick: function(assert) {
        tickWrapper.runOnAppTick(Applab, 2, function() {
          ...
          assert.equal(
            $('#my_button').length,
            0,
            'API created element should be gone'
          );
          assert.equal(
            $('#design_my_button').length,
            0,
            'API created element should not appear in design mode'
          );
          assert.equal(
          ...
          assert.equal(
            $('#screenSelector:disabled').length,
            0,
            'screen select enabled when designing'
          );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          $('#design-mode-dimmed').length,
          1,
          'transparency layer visible when running in design mode'
        );
        assert.equal(
          $('#screenSelector').length,
          0,
          'no screen selector while running'
        );
        ...
        assert.equal(
          $('#design-mode-dimmed').length,
          0,
          'transparency layer not visible after resetting'
        );
        assert.equal(
          $('#screenSelector:disabled').length,
          0,
          'screen select enabled after'
        );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.isTrue(
          manipulator.hasClass('ui-draggable'),
          'chart is draggable'
        );

        // Make sure it's resizable
        assert.isTrue(
          manipulator.hasClass('ui-resizable'),
          'chart is resizable'
        );
        ...
        // Delete the chart
        var deleteButton = $('#designWorkspaceBody').find(
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          $('#designModeViz .textArea').length,
          1,
          'element should exist'
        );
        assert.equal(
          $('#designModeViz .textArea').attr('id'),
          'design_text_area1',
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        // Duplicate the image
        var imageDuplicateButton = $('#designWorkspaceBody').find(
        ...
        // Duplicate the chart
        var chartDuplicateButton = $('#designWorkspaceBody').find(
          'button:contains(Duplicate)'
        )[0];
        ...
        var buttonDuplicateButton = $('#designWorkspaceBody').find(
          'button:contains(Duplicate)'
        )[0];
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          ...
          redImage,
          'after init, design mode img src prefixed with new channel id'
        );
        assert.equal(
          $('#image1').attr('src'),
          redImage,
          'after init, code mode img src prefixed with new channel id'
        );
        ...
        assert.include(
          $('#design_button1').css('background-image'),
          yellowImageUrl,
          'after init, design mode button image prefixed with new channel id: ' +
            $('#design_button1').css('background-image')
        );
        assert.include(
          $('#button1').css('background-image'),
          yellowImageUrl,
          'after init, code mode button image prefixed with new channel id: ' +
            $('#button1').css('background-image')
        );

        // a legacy image element whose image url was never set.
        assert.equal(
          $('#design_image2').attr('src'),
          '/blockly/media/1x1.gif',
          'after init, in design mode, empty image has placeholder image src'
        );
        assert.equal(
          $('#design_image2').attr('data-canonical-image-url'),
          '',
          'after init, in design mode, empty image has empty canonical image url'
        );
        assert.equal(
          $('#image2').attr('src'),
          '/blockly/media/1x1.gif',
          'after init, in code mode, empty image has placeholder image src'
        );

        // a legacy image element whose image url was set to ''.
        assert.equal(
          $('#design_image3').attr('src'),
          '/blockly/media/1x1.gif',
          'after init, in design mode, erased image has placeholder image src'
        );
        assert.equal(
          $('#design_image3').attr('data-canonical-image-url'),
          '',
          'after init, in design mode, erased image has empty canonical image url'
        );
        assert.equal(
          $('#image3').attr('src'),
          '/blockly/media/1x1.gif',
          'after init, in code mode, erased image has placeholder image src'
        );
        var images = $('#designModeViz').find('img');
        ...
        assert.equal(
          $('#design_image4').attr('src'),
          '/blockly/media/1x1.gif',
          'in design mode, new image has placeholder image src'
        );
        assert.equal(
          $('#design_image4').attr('data-canonical-image-url'),
          '',
          'in design mode, new image has empty canonical image url'
        );
        images = $('#designModeViz').find('img');
        ...
        tickWrapper.runOnAppTick(Applab, 1, function() {
          assert.equal(
            $('#design_image1').attr('src'),
            redImage,
            'after run, design mode img src prefixed with new channel id'
          );
          assert.equal(
            $('#image1').attr('src'),
            redImage,
            'after run, code mode img src prefixed with new channel id'
          );

          assert.include(
            $('#design_button1').css('background-image'),
            yellowImageUrl,
            'after run, design mode button image prefixed with new channel id'
          );
          assert.include(
            $('#button1').css('background-image'),
            yellowImageUrl,
            'after run, code mode button image prefixed with new channel id'
          );

          assert.equal(
            $('#image2').attr('src'),
            '/blockly/media/1x1.gif',
            'after run, empty image has placeholder image src'
          );
          assert.equal(
            $('#image3').attr('src'),
            '/blockly/media/1x1.gif',
            'after run, erased image has placeholder image src'
          );
          assert.equal(
            $('#image4').attr('src'),
            '/blockly/media/1x1.gif',
            'arter run, new image has placeholder image src'
          );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          $('#design_image1').attr('src'),
          flappyUrl,
          'after init, design mode img src prefixed with new prefix'
        );
        assert.equal(
          $('#image1').attr('src'),
          flappyUrl,
          'after init, code mode img src prefixed with new prefix'
        );
        ...
        assert.equal(
          designImage.getAttribute('data-canonical-image-url'),
          'flappy_promo.png'
        );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        var phoneUrl =
          '/base/test/integration/assets/applab-channel-id/phone_purple.png';
        var designButtonBg = $('#design_button1').css('background-image');
        assert.include(
          designButtonBg,
          phoneUrl,
          'after init, design mode button image url has new prefix: ' +
            phoneUrl +
            ' vs ' +
            designButtonBg
        );
        assert.include(
          $('#button1').css('background-image'),
          phoneUrl,
          'after init, code mode button image url has new prefix'
        );
        ...
        img.onload = function() {
          ...
          setTimeout(function() {
            ...
            assert.include(
              buttonElement.style.backgroundImage,
              phoneUrl,
              'on load, design mode button image is unchanged'
            );
            assert.equal(
              buttonElement.style.width,
              '120px',
              'on load, design mode button width is unchanged'
            );
            assert.equal(
              buttonElement.style.height,
              '130px',
              'on load, design mode button height is unchanged'
            );
            assert.equal(
              buttonElement.style.backgroundSize,
              '120px 130px',
              'on load, design mode background size is unchanged'
            );
            ...
            assert.include(
              buttonElement.style.backgroundImage,
              phoneUrl,
              'on load, code mode button image is unchanged'
            );
            assert.equal(
              buttonElement.style.width,
              '120px',
              'on load, code mode button width is unchanged'
            );
            assert.equal(
              buttonElement.style.height,
              '130px',
              'on load, code mode button height is unchanged'
            );
            assert.equal(
              buttonElement.style.backgroundSize,
              '120px 130px',
              'on load, code mode background size is unchanged'
            );
            ...
            assert.include(
              imageElement.src,
              flappyUrl,
              'on load, design mode image source is unchanged'
            );
            assert.equal(
              imageElement.style.width,
              '100px',
              'on load, design mode image width is unchanged'
            );
            assert.equal(
              imageElement.style.height,
              '105px',
              'on load, design mode image height is unchanged'
            );
            ...
            assert.include(
              imageElement.src,
              flappyUrl,
              'on load, code mode image source is unchanged'
            );
            assert.equal(
              imageElement.style.width,
              '100px',
              'on load, code mode image width is unchanged'
            );
            assert.equal(
              imageElement.style.height,
              '105px',
              'on load, code mode image height is unchanged'
            );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          firstButton.id,
          'design_button1',
          'first button is design_button1'
        );
        assert.equal(
          targetButton.id,
          'design_button2',
          'second button is design_button2'
        );
        assertPropertyRowValue(
          ...
          'button2',
          assert,
          'button2 is selected property tab'
        );
        ...
        assert(
          $('#divApplab #button1'),
          'button1 appears in divApplab after Run/Reset'
        );
        assert(
          $('#divApplab #button2'),
          'button2 appears in divApplab after Run/Reset'
        );
        assertPropertyRowValue(
          0,
          'id',
          'button2',
          assert,
          'button2 is still selected in property tab'
        );
        ...
        assert(
          $('#divApplab #button2'),
          'button2 still appears inside divApplab'
        );
        ...
        assert.equal(
          firstScreen.id,
          'design_screen1',
          'first screen is design_screen1'
        );
        assert.equal(
          targetScreen.id,
          'design_screen2',
          'second screen is design_screen2'
        );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          radio1.name,
          'radio_group1',
          'default group id generated for new radio button'
        );
        ...
        assert.equal(
          radio2.name,
          radio1.name,
          'new radio button reused group id of existing radio button'
        );
        assertPropertyRowValue(1, 'group id', 'radio_group1', assert);
        ...
        assert.equal(
          radio3.name,
          radio2.name,
          'new radio button reused updated group id of existing radio button'
        );
...

    {
      ...
      runBeforeClick: function(assert) {
        ...
        assert.equal(
          designModeViz.find('#design_button1').length,
          0,
          'button was deleted'
        );
        ...
        assert.equal(
          designModeViz.find('#design_label1').length,
          0,
          'label was deleted'
        );
        ...
        assert.equal(text_input.length, 1, 'text input was not deleted');
        assert.equal(
          text_input.parent().position().left,
          160,
          'text input element left position is 160'
        );
        assert.equal(
          text_input.parent().position().top,
          440,
          'text input element top position is 440'
        );
        assertPropertyRowValue(
          4,
          'x position (px)',
          160,
          assert,
          'text input x pos property value is 160'
        );
        assertPropertyRowValue(
          5,
          'y position (px)',
          440,
          assert,
          'text input y pos property value is 440'
        );
