...
describe('getPromptNameFromMode', () => {
  it('returns default prompt name for undefined', () => {
    expect(getPromptNameFromMode(undefined)).toBe('weblab2-suggest-structured');
  });

  it('returns default prompt name for invalid mode', () => {
    expect(getPromptNameFromMode('invalid')).toBe('weblab2-suggest-structured');
    expect(getPromptNameFromMode('')).toBe('weblab2-suggest-structured');
  });
