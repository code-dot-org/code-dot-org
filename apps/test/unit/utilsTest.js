...
describe('utils modules', () => {
  ...
  describe('extractTextFromCode', () => {
    ...
    it('extracts multi-line comments', () => {
      ...
      expect(result).to.include('multi-line comment');
    });
    ...
    it('removes parentheses that affect tokenization', () => {
      ...
      expect(result).to.not.include('(');
      expect(result).to.not.include(')');
      expect(result).to.include('artistList');
    ...
    it('extracts text from complex code with multiple elements', () => {
      ...
      `;
      const result = extractTextFromCode(code);
      expect(result).to.include('Check user input');
      expect(result).to.include('invalid');
      expect(result).to.include('validateInput');
    ...
    it('extracts identifiers with numbers', () => {
      ...
      const result = extractTextFromCode(code);
      expect(result).to.include('num1');
      expect(result).to.include('var2');
    });

    it('keeps underscores in identifiers for obfuscation detection', () => {
      const code = 'let my_var = 5; let sh_t = 10;';
      const result = extractTextFromCode(code);
      expect(result).to.include('my_var');
      expect(result).to.include('sh_t');
    });
    ...
    it('extracts only "var" from "var x = 1;"', () => {
      const code = 'var x = 1;';
      const result = extractTextFromCode(code);
      expect(result).to.include('var');
      // x is only 1 character, should not be included
      expect(result.split(/\s+/)).to.not.include('x');
    });
  });
});
