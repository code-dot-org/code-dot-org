    ...
    levels: [
      {
        ...
        url: 'http://localhost:3000/s/course3/lessons/1/levels/1',
        previous: false,
      ...
      },
      {
        ...
        icon: null,
        title: 1,
        url: 'http://localhost:3000/s/course3/lessons/1/levels/2',
        is_concept_level: false,
        bonus: false,
      ...
      },
      {
        ...
        icon: null,
        title: 2,
        url: 'http://localhost:3000/s/course3/lessons/1/levels/3',
        next: [2, 1],
        is_concept_level: false,
        bonus: true,
        display_as_unplugged: false,
        ...
        sublevels: [
          {
            ...
            position: 1,
            icon: null,
            url: 'http://localhost:3000/s/course3/lessons/1/levels/3/sublevel/1',
          ...
          },
          {
            ...
            position: 2,
            icon: null,
            url: 'http://localhost:3000/s/course3/lessons/1/levels/3/sublevel/2',
    ...
    levels: [
      {
        ...
        icon: null,
        title: 1,
        url: 'http://localhost:3000/s/course3/lessons/2/levels/1',
        previous: [1, 3],
        is_concept_level: false,
        bonus: false,
        display_as_unplugged: false,
      ...
      },
      {
        ...
        icon: null,
        title: 2,
        url: 'http://localhost:3000/s/course3/lessons/2/levels/2',
        is_concept_level: false,
        bonus: false,
        display_as_unplugged: false,
        sublevels: [],
      ...
      },
      {
        ...
        icon: null,
        title: 3,
        url: 'http://localhost:3000/s/course3/lessons/2/levels/3',
        is_concept_level: false,
        bonus: false,
        display_as_unplugged: false,
        sublevels: [],
...
describe('progressReduxTest', () => {
  describe('reducer', () => {
    ...
    it('can update progress', () => {
      // Construct state with a single lesson/level that has progress, but is
      // not perfect
      const state = {
        levelResults: {
          341: TestResults.MISSING_RECOMMENDED_BLOCK_UNFINISHED,
    ...
    it('cannot move progress backwards', () => {
      // Construct state with a single lesson/level that has progress, which is
      // perfect
      const state = {
        levelResults: {
          339: TestResults.ALL_PASS,
    ...
    it('can enable lesson extras', () => {
      assert.strictEqual(initialState.lessonExtrasEnabled, false);

      const nextState = reducer(initialState, setLessonExtrasEnabled(true));
      assert.strictEqual(nextState.lessonExtrasEnabled, true);
    });
    ...
    it('does not allow setCurrentLessonId to replace an existing lesson id', () => {
      const state = {
        ...initialState,
        currentLessonId: 111,
      };
  ...
  describe('with peer reviews', () => {
    ...
    it('can provide progress for peer reviews', () => {
      ...
      assert.equal(
        ...
        'same number of peer review levels in lesson'
      );
      ...
      for (let i = 0; i < expected.length; i++) {
        ...
        for (let j = 0; j < expected[i].length; j++) {
          assert.deepEqual(
            expected[i][j],
            results[i][j],
            `Mismatch for lesson at index ${i}, level at index ${j}`
          );
        }
      }
    });
  ...
  describe('processedLessons', () => {
    it('strips "hidden" field from lessons', () => {
      const lessons = [
        {
          name: 'lesson1',
          id: 123,
  ...
  describe('lessonExtrasUrl', () => {
    it('derives url from state by lessonId', () => {
