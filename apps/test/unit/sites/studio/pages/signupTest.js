...
describe('registration', () => {
  describe('setSchoolInfoFormData', () => {
    it('school id of -1 does not get pushed', () => {
      ...
      expect(
        formData.find(
          x => x.name === 'user[school_info_attributes][school_name]'
        ).value
      ).to.equal('School Name');
      expect(
        formData.find(
          x => x.name === 'user[school_info_attributes][school_state]'
        ).value
      ).to.equal('Confusion');
      expect(
        formData.find(
          x => x.name === 'user[school_info_attributes][school_zip]'
        ).value
      ).to.equal('90210');
      expect(
        formData.find(
          x => x.name === 'user[school_info_attributes][full_address]'
        ).value
      ).to.equal('Someplace Located');
      expect(
        formData.find(x => x.name === 'user[school_info_attributes][country]')
          .value
      ).to.equal('US');
    });
    ...
    it('with school id other attibutes do not get pushed', () => {
      ...
      expect(
        formData.find(x => x.name === 'user[school_info_attributes][school_id]')
          .value
      ).to.equal('123456');
      expect(
        formData.find(x => x.name === 'user[school_info_attributes][country]')
