...
describe('GameLabP5', function() {
  ...
  describe('createEdgeSprites method', function() {
    ...
    it('creates edge sprites off screen', function() {
      assert.containSubset(gameLabP5.p5.leftEdge, {
        position: {x: -50, y: 200},
        width: 100,
        height: 400
      });
      assert.containSubset(gameLabP5.p5.rightEdge, {
        position: {x: 450, y: 200},
        width: 100,
        height: 400
      });
      assert.containSubset(gameLabP5.p5.bottomEdge, {
        position: {x: 200, y: 450},
        width: 400,
        height: 100
      });
      assert.containSubset(gameLabP5.p5.topEdge, {
        position: {x: 200, y: -50},
        width: 400,
        height: 100
      });
  ...
  describe('rgb method', function() {
    it('returns the same as color method for rgb values', function() {
      expect(gameLabP5.p5.color(255, 255, 255)).to.deep.equal(
        gameLabP5.p5.rgb(255, 255, 255)
      );
      expect(gameLabP5.p5.color(0, 0, 0)).to.deep.equal(
        gameLabP5.p5.rgb(0, 0, 0)
      );
    });

    it('converts 0 to 1 alpha to 255 color value', function() {
      expect(gameLabP5.p5.color(255, 255, 255, 255).maxes.rgb).to.deep.equal(
        gameLabP5.p5.rgb(255, 255, 255, 1).maxes.rgb
      );
      expect(gameLabP5.p5.color(0, 0, 0, 0).maxes.rgb).to.deep.equal(
        gameLabP5.p5.rgb(0, 0, 0, 0).maxes.rgb
      );
      expect(gameLabP5.p5.color(255, 255, 255, 127.5).maxes.rgb).to.deep.equal(
        gameLabP5.p5.rgb(255, 255, 255, 0.5).maxes.rgb
      );
      expect(gameLabP5.p5.color(10, 20, 30, 63.75).maxes.rgb).to.deep.equal(
        gameLabP5.p5.rgb(10, 20, 30, 0.25).maxes.rgb
      );
    });
  });
  ...
  describe('ellipse', function() {
    ...
    it('draws the same ellipse when called with all arguments', function() {
      ...
      expect(gameLabP5.p5.originalEllipse_.args[0]).to.deep.equal(
        gameLabP5.p5.originalEllipse_.args[1]
      );
    });
  ...
  describe('rect', function() {
    ...
    it('draws the same rect when called with all arguments', function() {
      ...
      expect(gameLabP5.p5.originalRect_.args[0]).to.deep.equal(
        gameLabP5.p5.originalRect_.args[1]
      );
    });
