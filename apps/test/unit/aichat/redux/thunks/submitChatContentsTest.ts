...
describe('submitChatContents', () => {
  ...
  it('sends prior schema turns to the model as the reader saw them', async () => {
    // A stored schema response holds the model's raw JSON, code payload and
    // all. Sending that back as history re-injects code the student has
    // already accepted or rejected, and the model answers from it instead of
    // the current project. The transcript shows only the explanation, so the
    // model is given the same.
    const store = makeStore();
    store.dispatch(
      addEventToChatEventsCurrent({
        role: Role.ASSISTANT,
