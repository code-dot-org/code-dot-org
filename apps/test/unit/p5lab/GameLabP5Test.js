...
describe('GameLabP5', function () {
  ...
  describe('mouseIsOver method', function () {
    ...
    it('returns true when the mouse is within the circle collider', function () {
      ...
      expect(p5Wrapper.p5.mouseIsOver(sprite)).toBe(true);
    ...
    it('returns false when the mouse is outside the circle collider', function () {
      ...
      expect(p5Wrapper.p5.mouseIsOver(sprite)).toBe(false);
  ...
  describe('mouseDidMove method', function () {
    ...
    it('returns true when the mouse moved since the last frame', function () {
      ...
      expect(mouseX).to.equal(pmouseX).toBe(200);
      expect(mouseY).to.equal(pmouseY).toBe(210);
      expect(mouseDidMove).toBe(false);
      ...
      expect(mouseX).toBe(201);
      expect(pmouseX).toBe(200);
      expect(mouseY).toBe(211);
      expect(pmouseY).toBe(210);
      expect(mouseDidMove).toBe(true);
      ...
      expect(mouseX).to.equal(pmouseX).toBe(201);
      expect(mouseY).to.equal(pmouseY).toBe(211);
      expect(mouseDidMove).toBe(false);
    });
  });
  ...
  describe('createEdgeSprites method', function () {
    ...
    it('returns a group of sprites', function () {
      expect(edgeGroup).toBe(p5Wrapper.p5.edges);
      expect(edgeGroup.contains(p5Wrapper.p5.rightEdge)).toBe(true);
      expect(edgeGroup.contains(p5Wrapper.p5.leftEdge)).toBe(true);
      expect(edgeGroup.contains(p5Wrapper.p5.bottomEdge)).toBe(true);
      expect(edgeGroup.contains(p5Wrapper.p5.topEdge)).toBe(true);
      expect(edgeGroup.maxDepth()).toBe(4);
    });
  ...
  describe('rgb method', function () {
    it('returns the same as color method for rgb values', function () {
      expect(p5Wrapper.p5.color(255, 255, 255)).toEqual(p5Wrapper.p5.rgb(255, 255, 255));
      expect(p5Wrapper.p5.color(0, 0, 0)).toEqual(p5Wrapper.p5.rgb(0, 0, 0));
    });

    it('converts 0 to 1 alpha to 255 color value', function () {
      expect(p5Wrapper.p5.color(255, 255, 255, 255).maxes.rgb).toEqual(p5Wrapper.p5.rgb(255, 255, 255, 1).maxes.rgb);
      expect(p5Wrapper.p5.color(0, 0, 0, 0).maxes.rgb).toEqual(p5Wrapper.p5.rgb(0, 0, 0, 0).maxes.rgb);
      expect(p5Wrapper.p5.color(255, 255, 255, 127.5).maxes.rgb).toEqual(p5Wrapper.p5.rgb(255, 255, 255, 0.5).maxes.rgb);
      expect(p5Wrapper.p5.color(10, 20, 30, 63.75).maxes.rgb).toEqual(p5Wrapper.p5.rgb(10, 20, 30, 0.25).maxes.rgb);
    });
  });
