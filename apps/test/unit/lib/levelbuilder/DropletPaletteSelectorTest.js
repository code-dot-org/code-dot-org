...
describe('DropletPaletteSelector', () => {
  ...
  describe("when the editor is empty and no blocks are provided in the palette", () => {
    ...
    it("renders a select box", () => {
      const selector = mount(<DropletPaletteSelector editor={editor} palette={{}} />);
      expect(selector.containsMatchingElement(
        <select disabled>
  ...
  describe("when blocks are provided", () => {
    ...
    it("the select blocks contains a list of blocks that can be added", () => {
      const selector = mount(<DropletPaletteSelector editor={editor} palette={{a: null, b: null}} />);
      expect(selector.containsMatchingElement(
        <select>
    ...
    describe("and the editor contains some existing json", () => {
      ...
      beforeEach(() => {
        ...
        selector = mount(<DropletPaletteSelector editor={editor} palette={{a: null, b: null}} />);
      });
    ...
    describe("When selecting an item", () => {
      ...
      it("updates the editor to include that value", () => {
        const selector = mount(<DropletPaletteSelector editor={editor} palette={{a: null, b: null}} />);
        selector.find('select').props().onChange({target: {value: 'b'}});
  ...
  "b": null
        ...
        selector.find('select').props().onChange({target: {value: 'a'}});
