...
describe('maker/commands.js - CircuitPlayground', () => {
  ...
  describe('pinMode(pin, mode)', () => {
    it('delegates to makerBoard.pinMode with mapped mode id', () => {
      ...
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(0, 0);
    });
    ...
    it('display error when invalid pin 13 is used', () => {
      ...
      expect(errorHandler.outputError).toHaveBeenCalledWith(
    ...
    it(`maps 'output' mode to 1`, () => {
      ...
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(0, 1);
    });

    it(`maps 'analog' mode to 2`, () => {
      pinMode({pin: 0, mode: 'analog'});
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(0, 2);
    });

    it(`maps 'pwm' mode to 3`, () => {
      pinMode({pin: 0, mode: 'pwm'});
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(0, 3);
    });

    it(`maps 'servo' mode to 4`, () => {
      pinMode({pin: 0, mode: 'servo'});
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(0, 4);
    });
  });
  ...
  describe('digitalWrite(pin, value)', () => {
    it('delegates to makerBoard.digitalWrite', () => {
      ...
      expect(stubBoardController.digitalWrite).toHaveBeenCalledWith(0, 1);
    });
  });
  ...
  describe('digitalRead(pin)', () => {
    it('delegates to makerBoard.digitalRead', () => {
      digitalRead({pin: 0});
      expect(stubBoardController.digitalRead).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('analogWrite(pin, value)', () => {
    it('delegates to makerBoard.analogWrite', () => {
      ...
      expect(stubBoardController.analogWrite).toHaveBeenCalledWith(0, 33);
    });
  });
  ...
  describe('analogRead(pin)', () => {
    it('delegates to makerBoard.analogRead', () => {
      analogRead({pin: 0});
      expect(stubBoardController.analogRead).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('boardConnected()', () => {
    it('delegates to makerBoard.boardConnected', () => {
      boardConnected();
      expect(stubBoardController.boardConnected).toHaveBeenCalledTimes(1);
    });
  ...
  describe('createLed(pin)', () => {
    it('delegates to makerBoard.createLed', () => {
      createLed({pin: 0});
      expect(stubBoardController.createLed).toHaveBeenCalledWith(0);
    });
  });
  ...
  describe('createButton(pin)', () => {
    it('delegates to makerBoard.createButton', () => {
      createButton({pin: 2});
      expect(stubBoardController.createButton).toHaveBeenCalledWith(2);
    });
  });
  ...
  describe('onBoardEvent(pin)', () => {
    it('forwards the call to the component', () => {
      ...
      expect(component.on).toHaveBeenCalledWith(event, callback);
    });
    ...
    describe(`event aliases`, () => {
      ...
      it(`aliases 'tap:single' event to 'singleTap'`, function () {
        ...
        expect(component.on).toHaveBeenCalledWith('tap:single', callback);
      });

      it(`aliases 'tap:double' event to 'doubleTap'`, function () {
        onBoardEvent({component, event: 'doubleTap', callback});
        expect(component.on).toHaveBeenCalledWith('tap:double', callback);
      });
    });
  });
});
...
describe('maker/commands.js - MicroBit', () => {
  ...
  describe('pinMode(pin, mode)', () => {
    it('delegates to makerBoard.pinMode with mapped mode id', () => {
      pinMode({pin: 1, mode: 'input'});
      expect(stubBoardController.pinMode).toHaveBeenCalledWith(1, 0);
    });
