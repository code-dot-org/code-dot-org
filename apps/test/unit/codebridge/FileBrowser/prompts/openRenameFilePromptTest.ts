...
describe('openRenameFilePrompt', function () {
  ...
  it('calls onFileRename with the old file and the new name', async function () {
    const [, sendLab2AnalyticsEvent] = getAnalyticsMock();
    const fileId = '1';
    const newFileName = 'valid_file_name.txt';
    const [, renameFileDataMock] = getRenameFileMock();
    ...
    expect(onFileRename).toHaveBeenCalledWith(
      testProject.files[fileId],
      newFileName
    );
  });

  it('does not call onFileRename when the name is unchanged', async function () {
    const [, sendLab2AnalyticsEvent] = getAnalyticsMock();
    const fileId = '1';
    const [, renameFileDataMock] = getRenameFileMock();
    const onFileRename = jest.fn();
    ...
    await openRenameFilePrompt({
      fileId,
      dialogControl: getDialogControlMock(testProject.files[fileId].name),
      renameFile: renameFileDataMock,
      projectFiles: testProject.files,
      sendLab2AnalyticsEvent,
      isStartMode: false,
