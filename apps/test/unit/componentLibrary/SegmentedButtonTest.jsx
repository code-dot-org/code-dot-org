...
describe('Design System - Segmented Buttons', () => {
  it('SegmentedButtons - renders with correct label', () => {
  ...
  it('SegmentedButtons - selects button when clicked, once selected - remains selected if clicked again', async () => {
    ...
    // SegmentedButtons's onChange is only called when radioButton is not checked. Second click won't call onChange.
    expect(spyOnChange).to.have.been.calledOnce;
    expect(radioButton.checked).to.be.true;
    expect(radioButton.disabled).to.be.false;
  });

  it("SegmentedButtons - renders disabled radio button, doesn't change on click", async () => {
    const user = userEvent.setup();
    const spyOnChange = sinon.spy();
