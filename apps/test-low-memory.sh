...
(PORT=9879 LEVEL_TYPE='turtle|studio' $GRUNT_CMD karma:integration && ${CODECOV} -cF integration)
(PORT=9880 LEVEL_TYPE='maze|bounce|calc|eval|flappy' $GRUNT_CMD karma:integration && ${CODECOV} -cF integration)
(PORT=9881 LEVEL_TYPE='gamelab|craft' $GRUNT_CMD karma:integration && ${CODECOV} -cF integration)
(PORT=9883 LEVEL_TYPE='applab1|applab2' $GRUNT_CMD karma:integration && ${CODECOV} -cF integration)
SCRIPT
