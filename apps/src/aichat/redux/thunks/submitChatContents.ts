...
export const submitChatContents = createAsyncThunk(
  ...
  async (
    ...
    // Stored messages hold the model's raw response; a schema lab's transcript
    // shows formatForDisplay's rendering of it. The model is given what the
    // reader was shown, so a prior turn's code payload does not re-enter the
    // context it was already merged into or rejected from.
    const modelHistory = () =>
