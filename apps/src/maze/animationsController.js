...
export default class AnimationsController {
  ...
  reset(first) {
    if (first) {
      ...
      timeoutList.setTimeout(() => {
        ...
        this._scheduleTurn(this.maze.startDirection);
      }, danceTime + 150);
    } else {
   ...
   */
  _createPegmanAnimation(options) {
    // Create clip path.
  ...
  _updatePegmanAnimation(options) {
    ...
    var x = this.maze.SQUARE_SIZE * options.col -
        options.direction * this.maze.PEGMAN_WIDTH + 1 + this.maze.PEGMAN_X_OFFSET;
    img.setAttribute('x', x);
    var y = getPegmanYForRow(this.maze.skin, options.row) - this._getPegmanFrameOffsetY(options.animationRow);
  ...
  scheduleLook(x, y, d) {
    ...
    for (var i = 0; i < paths.length; i++) {
      var path = paths[i];
      this._scheduleLookStep(path, this.maze.stepSpeed * i);
    }
  }
  ...
  scheduleAnimations(singleStep, onAnimationEnd) {
    ...
    this._scheduleSingleAnimation(0, actions, singleStep, timePerAction, onAnimationEnd);
  }
   ...
   */
  _scheduleSingleAnimation(index, actions, singleStep, timePerAction, onAnimationEnd) {
    if (index >= actions.length) {
      this._finishAnimations(singleStep, onAnimationEnd);
      return;
    }
   ...
   */
  _finishAnimations(singleStep, onAnimationEnd) {
