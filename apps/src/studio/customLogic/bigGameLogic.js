...
BigGameLogic.prototype.onTick = function () {
  ...
  if (
    ...
    dangerSprite.visible &&
    this.collide(playerSprite.x, playerSprite.y, dangerSprite.x, dangerSprite.y)
  ) {
    ...
    setTimeout(
      ...
      20 * 40 + 50
    ); // 40ms for each of 20 frames, plus some buffer
  ...
  if (
    playerSprite.visible &&
    targetSprite.visible &&
    this.collide(playerSprite.x, playerSprite.y, targetSprite.x, targetSprite.y)
  ) {
...
BigGameLogic.prototype.handleUpdatePlayer_ = function (key) {
  ...
  // reinvertY
  playerSprite.y =
    this.studio_.MAZE_HEIGHT - newUserSpaceY - playerSprite.height / 2;
...
BigGameLogic.prototype.resetSprite_ = function (sprite) {
  ...
  sprite.y = Math.floor(
    Math.random() * (this.studio_.MAZE_HEIGHT - sprite.height)
  );
  this.studio_.setSprite({
    spriteIndex: this.studio_.sprite.indexOf(sprite),
    value: 'hidden',
