...
function buildSystemPrompt(items: Game2ItemEntry[]): string {
  const blockItems = items.filter(i => (i.itemType ?? 'sprite') === 'block');
  const spriteItems = items.filter(i => (i.itemType ?? 'sprite') === 'sprite');

  const blockNames =
    blockItems.length > 0
      ? blockItems.map(i => `"${i.name}"`).join(', ')
      : '(none)';
  const spriteNames =
    spriteItems.length > 0
      ? spriteItems.map(i => `"${i.name}"`).join(', ')
      : '(none)';

  const primaryBlock = blockItems[0]?.name;
...
{
  "cells": [
    {"r": <row>, "c": <col>, "t": "<itemName>"},
    ...
  ]
}

Valid block item names (impassable platforms the player stands on): ${blockNames}
${
  spriteItems.length > 0
    ? `Valid sprite item names (collectible/interactive): ${spriteNames}`
    : ''
}
${
  primaryBlock
    ? `Use "${primaryBlock}" for all platform/ground blocks.`
    : 'No block items are available — you cannot place platforms.'
}
...
${
  primaryBlock
    ? `- Place a "${primaryBlock}" ground near row ${
        GRID_ROWS - 5
...
${
  spriteItems.length > 0
    ? `- Scatter sprite items on top of platforms every so often (every 5-10 platforms). Place items 1 row ABOVE a block so they sit on the platform surface.`
    : ''
}
- Keep the design interesting but playable — every platform should be reachable by jumping.
- Aim for roughly 100-200 block cells total for a good density.
...
- Every cell must have a valid item name from the lists above.
`;
}
...
function parseWorldResponse(
  ...
  const validNames = new Set(
    items
      .filter(i => (i.itemType ?? 'sprite') !== 'background')
      .map(i => i.name)
  );
