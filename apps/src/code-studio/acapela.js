...
export default {
    ...
    $('button.tts').click(function () {
      ...
      switch (this.id) {
        case 'tts-short-instructions':
          sourceText =
            $('#level_tts_short_instructions_override').val() ||
            $('#level_short_instructions').val();
          break;
        case 'tts-long-instructions':
          sourceText =
            $('#level_tts_long_instructions_override').val() ||
            $('#level_long_instructions_preview').text();
          break;
      }
  ...
  populateVoicesList(config) {
    $.getJSON(
      'https://vaas.acapela-group.com/Services/Voices.json?jsoncallback=?',
      {
        cl_login: config.login,
        cl_app: config.app,
