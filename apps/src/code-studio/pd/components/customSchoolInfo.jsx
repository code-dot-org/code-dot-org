...
export default class CustomSchoolInfo extends React.Component {
  ...
  handleSchoolTypeChange = change => {
    const school_info = {...this.props.school_info, ...change};
    if (
      [SCHOOL_TYPES.PRIVATE, SCHOOL_TYPES.OTHER].includes(change['school_type'])
    ) {
  ...
  render() {
    return (
      <FormGroup>
        ...
        />
        {this.props.school_info &&
          [SCHOOL_TYPES.PUBLIC, SCHOOL_TYPES.CHARTER].includes(
            this.props.school_info.school_type
          ) && (
            <FieldGroup
        ...
        {this.props.school_info && this.props.school_info.school_type && (
          <FormGroup>
            ...
            >
              <ControlLabel>
                School State<span className="form-required-field"> *</span>
              </ControlLabel>
              <Select
                value={
                  this.props.school_info
                    ? this.props.school_info.school_state
                    : null
                }
