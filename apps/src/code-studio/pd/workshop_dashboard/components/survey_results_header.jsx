...
const organizerViewApiRoot =
  '/api/v1/pd/workshop_organizer_survey_report_for_course/';
...
const rowOrder = [
  ...
  {
    heading: true,
    text: 'Facilitator Effectiveness (out of 5)',
    key: 'facilitator_effectiveness',
    score_base: '5'
  },
  {
    text:
      'Overall, how much have you learned from your workshop about computer science?',
    key: 'how_much_learned_s',
    score_base: '5'
  },
  {
    text:
      'During your workshop, how motivating were the activities that this program had you do?',
    key: 'how_motivating_s',
    score_base: '5'
  },
  {
    text:
      'For this workshop, how clearly did your facilitator present the information that you needed to learn?',
    key: 'how_clearly_presented_s',
    score_base: '5'
  },
  {
    text:
      'How interesting did your facilitator make what you learned in the workshop?',
    key: 'how_interesting_s',
    score_base: '5'
  },
  {
    text:
      'How often did your facilitator give you feedback that helped you learn?',
    key: 'how_often_given_feedback_s',
    score_base: '5'
  },
  {
    text:
      'How comfortable were you asking your facilitator questions about what you were learning in his or her workshop?',
    key: 'how_comfortable_asking_questions_s',
    score_base: '5'
  },
  {
    text:
      "How often did your facilitator teach you things that you didn't know before taking this workshop?",
    key: 'how_often_taught_new_things_s',
    score_base: '5'
  },
  {
    heading: true,
    text: 'Teacher Engagement (out of 5)',
    key: 'teacher_engagement',
    score_base: '5'
  },
  {
    text: 'During your workshop, how much did you participate?',
    key: 'how_much_participated_s',
    score_base: '5'
  },
  {
    text:
      'When you are not in Code.org workshops how often do you talk about the ideas from the workshops?',
    key: 'how_often_talk_about_ideas_outside_s',
    score_base: '5'
  },
  {
    text:
      'How often did you get so focused on Code.org workshop activities that you lost track of time?',
    key: 'how_often_lost_track_of_time_s'
  },
  {
    text:
      'Before the workshop, how excited were you about going to your Code.org workshop?',
    key: 'how_excited_before_s',
    score_base: '5'
  },
  {
    text:
      'Overall, how interested were you in the Code.org in-person workshop?',
    key: 'overall_how_interested_s',
    score_base: '5'
  },
  {
    heading: true,
    text: 'Overall Success Score (out of 6)',
    key: 'overall_success',
    score_base: '6'
  },
  {
...
const freeResponseQuestions = [
  {
    text: 'What were two things your facilitator(s) did well?',
    key: 'things_facilitator_did_well_s'
  },
  {
    text: 'What were two things your facilitator(s) could do better?',
    key: 'things_facilitator_could_improve_s'
  },
  {
    text:
      'What were the two things you liked most about the activities you did in this workshop and why?',
    key: 'things_you_liked_s'
  },
  {
    text:
      'What are the two things you would change about the activities you did in this workshop?',
    key: 'things_you_would_change_s'
  },
  {
    text:
...
export default class SurveyResultsHeader extends React.Component {
  ...
  filterWorkshops(course, preselectedWorkshop = undefined) {
    ...
    if (preselectedWorkshop) {
      ...
      selectedWorkshopId = this.props.organizerView
        ? undefined
        : filteredWorkshops[0]
      ...
      if (!this.props.organizerView || workshopId) {
        $.ajax({
          ...
          url: `${facilitatorViewApiRoot}${workshopId}/workshop_survey_report${
            this.props.organizerView ? '?organizer_view=1' : ''
          }`,
          dataType: 'json'
        }).done(data => {
          this.setState({
  ...
  handleOnClickDownloadCsv = () => {
    if (this.props.organizerView && !this.state.selectedWorkshopId) {
      ...
      window.open(
        `${facilitatorViewApiRoot}${
          this.state.selectedWorkshopId
        }/workshop_survey_report.csv`
      );
    }
  };
  ...
  renderSurveyResults() {
    ...
    const allMyWorkshopsForCourse = this.state.workshopSurveyData[
      'all_my_workshops_for_course'
    ];
    const allWorkshopsForCourse = this.state.workshopSurveyData[
    ...
    return (
      ...
      >
        <thead>
          <tr>
            <th />
            <th>{this.state.workshopName}</th>
            <th>Across all my workshops for {this.state.selectedCourse}</th>
            <th>Across all workshops for {this.state.selectedCourse}</th>
          </tr>
        </thead>
        <tbody>
          {rowOrder.map((row, i) => {
            return (
              <tr key={i}>
                <td
                  style={
                    row['heading']
                      ? styles.questionGroupHeader
                      : styles.individualQuestion
                  }
                >
                  {row['text']}
                </td>
                <td style={row['heading'] && styles.questionGroupCell}>
                  {this.renderScore(row, thisWorkshop[row['key']])}
                </td>
                <td style={row['heading'] && styles.questionGroupCell}>
                  {this.renderScore(row, allMyWorkshopsForCourse[row['key']])}
                </td>
                <td style={row['heading'] && styles.questionGroupCell}>
                  {this.renderScore(row, allWorkshopsForCourse[row['key']])}
                </td>
              </tr>
            );
          })}
  ...
  renderOrganizerSurveyResults() {
    ...
    const breakoutData = _.omit(this.state.workshopSurveyData, [
      'all_workshops_for_course',
      'all_my_workshops_for_course'
    ]);
    ...
    let headings = [
      ...
      'Across all workshops I organized for ' + this.state.selectedCourse
    ];
            ...
            return (
              <td key={j} style={row['heading'] && styles.questionGroupCell}>
                {this.renderScore(
                  row,
                  this.state.selectedWorkshopId
                    ? breakoutData['this_workshop'][row['key']]
                    : breakoutData[heading][row['key']]
                )}
              </td>
            );
          })}
  ...
  renderSurveyPanel() {
    if (this.state.workshopSurveyData === undefined) {
      ...
      return this.props.organizerView
        ? this.renderOrganizerSurveyResults()
  ...
  getWorkshopFriendlyName(workshop) {
    return (
      ...
      ' - ' +
      (workshop.sessions[0]
        ? moment.utc(workshop.sessions[0].start).format(DATE_FORMAT)
        : 'no sessions')
    );
  }
  ...
  getCoursesForWorkshops() {
    const courses = Array.from(
      new Set(this.props.workshops.map(workshop => workshop.course))
    );
    return courses.filter(course => !['Admin', 'Counselor'].includes(course));
  }

  render() {
    const courseOptions = this.getCoursesForWorkshops().map(function(
      course,
      i
    ) {
      return (
