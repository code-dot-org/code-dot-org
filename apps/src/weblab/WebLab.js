...
export default class WebLab {
  ...
  deleteProjectFile(filename, callback) {
    filesApi.deleteFile(
      filename,
      xhr => {
  ...
  renameProjectFile(filename, newFilename, callback) {
    filesApi.renameFile(
      filename,
      newFilename,
      xhr => {
  ...
  changeProjectFile(filename, fileData, callback, skipPreWriteHook) {
    filesApi.putFile(
      filename,
      fileData,
      xhr => {
  ...
  onFilesReady(files, filesVersionId) {
  ...
  openFatalErrorDialog(message, type = FatalErrorType.Default) {
  ...
  openDisallowedHtmlDialog(filename, disallowedTags, onClose) {
    const handleClose = () => {
      this.closeDialog();
      onClose();
    };
   ...
   */
  onBrambleMountable(brambleHost) {
  ...
  brambleApi() {
    return {
      ...
      registerBeforeFirstWriteHook: this.registerBeforeFirstWriteHook.bind(
        this
      ),
