...
class LiveCameraProperties extends React.Component {
  static propTypes = {
...
class LiveCameraEvents extends React.Component {
  static propTypes = {
    element: PropTypes.instanceOf(HTMLElement).isRequired,
    handleChange: PropTypes.func.isRequired,
  ...
  getInitLiveCameraCode() {
    const id = elementUtils.getId(this.props.element);
    const code = 'initLiveCamera("' + id + '");\n';
    return code;
  }
  ...
  insertInitLiveCamera = () => {
    this.props.onInsertEvent(this.getInitLiveCameraCode());
  };
  ...
  render() {
    const element = this.props.element;
    const initName = 'Init';
    const initDesc = 'Init the live camera';
    const clickName = 'Click';
    const clickDesc =
      'Triggered when the live camera is clicked with a mouse or tapped on a screen.';
    ...
    return (
      <div id="eventRowContainer">
        ...
        <EventRow
          name={initName}
          desc={initDesc}
          handleInsert={this.insertInitLiveCamera}
        />
        <EventHeaderRow />
        <EventRow
          name={clickName}
...
export default {
  PropertyTab: LiveCameraProperties,
  EventTab: LiveCameraEvents,
  ...
  create: function() {
    ...
    /*if (navigator.userAgent.indexOf('Firefox') > -1) {
      element.srcObject = element.mozCaptureStream();
    } else {
      element.srcObject = element.captureStream();
    }
    element.autoplay = true;
