...
function renderSectionProgress(section, validScripts) {
  ...
  store.dispatch(setSection(section));
  store
    .dispatch(loadValidScripts(section, validScripts))
    .then(() => renderSectionProgressReact(store));
}
...
function main() {
    ...
    .config([
      ...
      function($routeProvider) {
        ...
        $routeProvider.when('/sections/:id', {
          templateUrl: '/teacher-dashboard/section',
          controller: 'SectionDetailController'
        });
        $routeProvider.when('/sections/:id/print_signin_cards', {
          templateUrl: '/teacher-dashboard/signin_cards',
          controller: 'SectionSigninCardsController',
          tab: 'print_signin_cards'
        });
        $routeProvider.when('/sections/:id/progress', {
          templateUrl: '/teacher-dashboard/section',
          controller: 'SectionProgressController',
          tab: 'progress'
        });
        $routeProvider.when('/sections/:id/projects', {
          templateUrl: '/teacher-dashboard/section',
          controller: 'SectionProjectsController',
          tab: 'projects'
        });
        $routeProvider.when('/sections/:id/responses', {
          templateUrl: '/teacher-dashboard/section',
          controller: 'SectionResponsesController',
          tab: 'responses'
        });
        $routeProvider.when('/sections/:id/assessments', {
          templateUrl: '/teacher-dashboard/section',
          controller: 'SectionAssessmentsController',
          tab: 'assessments'
        });
        ...
        $routeProvider.when(
          '/sections/:sectionid/student/:studentid/script/:scriptid',
          {
            templateUrl: '/teacher-dashboard/student',
            controller: 'StudentDetailController'
          }
        ...
        $routeProvider.when(
          '/sections/:sectionid/student/:studentid/course/:scriptid',
          {
            templateUrl: '/teacher-dashboard/student',
            controller: 'StudentDetailController'
          }
  ...
  var directives = angular
    .module('teacherDashboard.directives', [])
    .value('version', '0.1');

  directives.directive('teacherNav', [
    '$location',
    '$routeParams',
  ...
  // FILTERS
  var filters = angular
    .module('teacherDashboard.filters', [])
    .value('version', '0.1');
  ...
  var services = angular
    .module('teacherDashboard.services', [])
    .value('version', '0.1');
      ...
      function($resource) {
        return $resource(
          ...
          {},
          {
            ...
            progress: {
              method: 'GET',
              url: '/dashboardapi/section_progress/:id'
            },
            studentProgress: {
              method: 'GET',
              url: '/dashboardapi/student_progress/:id/:studentId'
            },
            responses: {
              method: 'GET',
              url: '/dashboardapi/section_text_responses/:id',
              isArray: true
            },
            validScripts: {
              method: 'GET',
  ...
  services.factory('paginatedSectionProgressService', [
    '$http',
    '$q',
    function($http, $q) {
          ...
          students: data.students.map(student => ({
            ...
            levels: student.levels.map(([className, title, url]) => ({
              ...
              url:
                studioUrlPrefix +
                url +
                `?section_id=${sectionId}&user_id=${student.id}`
            }))
          }))
        };
      }
      ...
      return {
        get: (id, script_id = undefined) => {
          ...
          const getNextPage = () => {
            ...
            return $http
              .get(`/dashboardapi/section_progress/${id}?${queryParams}`)
  ...
  app.controller('SectionsController', [
    '$scope',
    '$window',
    'sectionsService',
    function($scope, $window, sectionsService) {
      ...
      logToCloud.addPageAction(
        logToCloud.PageAction.PegasusSectionsRedirect,
        {}
      );
  ...
  app.controller('StudentDetailController', [
    ...
    function($scope, $routeParams, sectionsService) {
      ...
      $scope.progress = sectionsService.studentProgress({
  ...
  app.controller('SectionDetailController', [
    '$scope',
    '$routeParams',
    '$window',
    '$q',
    ...
    function($scope, $routeParams, $window, $q, $location, sectionsService) {
      ...
      $scope.section.$promise.catch(function(result) {
        if (result.status == 403 || result.status == 404) {
          $window.alert(
            'You are not the owner of this section or this section doesn’t exist.'
          );
        } else {
      ...
      $scope.section.$promise.then(function(section) {
        if (!$scope.tab) {
      ...
      $scope.gender_list = {
        f: i18n.dashboard_students_female,
        m: i18n.dashboard_students_male
      };
      ...
      if ($scope.tab === 'manage') {
        ...
        $scope.$on('student-table-react-rendered', () => {
          $scope.section.$promise.then(section =>
            renderSectionTable(section, scriptData.studiourlprefix)
          );
          firehoseClient.putRecord({
            study: 'teacher-dashboard',
            study_group: 'react',
  ...
  app.controller('SectionSigninCardsController', [
    '$scope',
    '$routeParams',
    '$window',
    '$q',
    ...
    function($scope, $routeParams, $window, $q, sectionsService) {
      ...
      $scope.print = function() {
        const content = document.getElementsByClassName('all_cards')[0]
          .innerHTML;
        ...
        const cards = $window.frames.print_frame.document.getElementsByClassName(
          'signin_card'
        );
  ...
  app.controller('SectionProjectsController', [
    '$scope',
    '$routeParams',
    'sectionsService',
    function($scope, $routeParams, sectionsService) {
  ...
  app.controller('SectionProgressController', [
    '$scope',
    '$routeParams',
    '$window',
    '$q',
    ...
    '$interval',
    'sectionsService',
    'paginatedSectionProgressService',
    function(
      $scope,
      $routeParams,
      $window,
      ...
      firehoseClient.putRecord({
        study: 'teacher-dashboard',
        study_group: experiments.isEnabled(experiments.TEACHER_EXP_2018)
          ? 'react'
          : 'angular',
      ...
      $scope.sections.$promise.then(sections => {
        $scope.selectedSection = sections.find(
          section => section.id.toString() === $routeParams.id
        );
      });
  ...
  app.controller('SectionResponsesController', [
    '$scope',
    '$routeParams',
    '$window',
    '$q',
  ...
  app.controller('SectionAssessmentsController', [
    '$scope',
    '$routeParams',
    '$window',
    '$q',
