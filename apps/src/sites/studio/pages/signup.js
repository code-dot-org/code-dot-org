...
export function setSchoolInfoFormData(signupForm, formData) {
  ...
  if (ncesSchoolElement && ncesSchoolElement.value !== '-1') {
    ...
    formData.splice(
      formData.findIndex(
        el => el.name === 'user[school_info_attributes][school_type]'
      ),
      1
    );
  } else {
  ...
  signupForm.forEach(function(el) {
    ...
    if (match) {
      ...
      formData.push({
        name: 'user[school_info_attributes][' + match.to + ']',
        value: value
      });
    }
  });
...
window.SignupManager = function(options) {
    ...
    ReactDOM.render(
      <div>
        ...
        <div className="itemblock" style={{minHeight: 42}}>
          <div className="school-info-labelblock">
  ...
  $('.signupform').submit(function() {
    ...
    if (isTeacherSelected()) {
      ...
      $.grep(formData, e =>
        e.name.startsWith('user[school_info_attributes]')
      ).forEach(x => (x.value = ''));
    }
  ...
  $('#user_password').on('input', function(e) {
    ...
    var password_message_confirmation = $(
      '#password_confirmation-block .error_in_field'
    );
...
class SignupSchoolNotFound extends React.Component {
  ...
  render() {
    const {isUS, data, schoolDataErrors, onSchoolNotFoundChange} = this.props;
