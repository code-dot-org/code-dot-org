  ...
  const previewUrl = useMemo(() => {
    ...
    if (!prefix) {
      if (isViewingExemplar || isEditingExemplar) {
        ...
        // Widget embeds land here (widget2 levels have no channel), so two embeds of
        // different widgets get distinct origins from their distinct level ids. Two
        // embeds of the *same* widget share one, which is harmless: identical files,
        // and the embed layout renders no console or network panel to cross-talk.
