...
class StudioApp {
  ...
  getVersionHistoryHandler(config) {
    return () => {
  ...
  startIFrameEmbeddedApp(config, onTooYoung) {
   ...
   *   to be played.
   */
  reset(shouldPlayOpeningAnimation) {
   ...
   *    block position.
   */
  arrangeBlockPosition(startBlocks, arrangement) {
   ...
   */
  showInstructionsDialog_(level, autoClose) {
   ...
   */
  shouldDisplayFeedbackDialog(options) {
   ...
   */
  getTestResults(levelComplete, options) {
  ...
  // to the server.
  builderForm_(onAttemptCallback) {
   ...
   */
  clearAndAttachRuntimeAnnotations() {
   ...
   *
   */
  fixViewportForSmallScreens_(viewport, config) {
    var deviceWidth;
    var desiredWidth;
    var minWidth;
   ...
   */
  setCheckForEmptyBlocks(checkBlocks) {
   ...
   */
  setStartBlocks_(config, loadLastAttempt) {
   ...
   * @param {AppOptionsConfig}
   */
  openFunctionDefinition_(config) {
   ...
   */
  getFilteredUnfilledFunctionalBlock_(filter) {
    var unfilledBlock;
   ...
   */
  getUnfilledFunctionalBlockError(topLevelType) {
   ...
   */
  checkForFailingExamples(failureChecker) {
   ...
   */
  hasEmptyFunctionOrVariableName() {
  ...
  createCoordinateGridBackground(options) {
   ...
   */
  displayWorkspaceAlert(type, alertContents) {
   ...
   * @param {React.Component} alertContents
   */
  displayPlayspaceAlert(type, alertContents) {
   ...
   * @param {React.Component} alertContents
   */
  displayAlert(selector, props, alertContents) {
    var parent = $(selector);
   ...
   */
  alertIfProfaneOrPrivacyViolatingProject() {
    if (window.dashboard && dashboard.project &&
   ...
   */
  forLoopHasDuplicatedNestedVariables_(block) {
   ...
   */
  polishGeneratedCodeString(code) {
   ...
   *   are app specific.
   */
  setPageConstants(config, appSpecificConstants) {
