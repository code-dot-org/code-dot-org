...
export function sendCustomEventAsync(type: string, payload: Uint8Array) {
  if (wrapper) return wrapper.sendCustomEventAsync(type, payload);
...
function wrapperAsync(): Promise<PacketIOWrapper | void> {
  ...
  return mkPacketIOAsync().then(io => {
    ...
    if (onSerialHandler) wrapper.onSerial = onSerialHandler;
    if (onCustomEventHandler) wrapper.onCustomEvent = onCustomEventHandler;
    // trigger ui update
    if (onConnectionChangedHandler) onConnectionChangedHandler();
    return wrapper;
  });
}
