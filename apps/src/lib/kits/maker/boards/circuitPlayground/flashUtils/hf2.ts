...
export class Wrapper implements pxt.packetio.PacketIOWrapper {
  ...
  sendCustomEventAsync(type: string, payload: Uint8Array): Promise<void> {
    if (type == CUSTOM_EV_JACDAC)
      if (this.jacdacAvailable)
        return this.talkAsync(HF2_CMD_JDS_SEND, payload).then(() => {});
      else return Promise.resolve(); // ignore
  ...
  isConnecting(): boolean {
    return (
      this.io.isConnecting() || (this.io.isConnected() && !this.initialized)
    );
  }
    ...
    return this.io
      ...
      .catch(e => {
        if (this.reconnectTries < 5) {
          this.reconnectTries++;
          log(
            `error ${e.message}; reconnecting attempt #${this.reconnectTries}`
          );
          return U.delay(500).then(() => this.reconnectAsync());
        } else {
          throw e;
        }
      });
  ...
  talkAsync(cmd: number, data?: Uint8Array) {
    if (this.io.talksAsync)
      return this.io.talksAsync([{cmd, data}]).then(v => v[0]);
    ...
    write16(pkt, 4, seq);
    write16(pkt, 6, 0);
    if (data) U.memcpy(pkt, 8, data, 0, data.length);
    let numSkipped = 0;
    ...
    return this.sendMsgAsync(pkt).then(handleReturnAsync);
  }
  ...
  sendSerialAsync(buf: Uint8Array, useStdErr = false) {
    if (this.io.sendSerialAsync) return this.io.sendSerialAsync(buf, useStdErr);
  ...
  switchToBootloaderAsync() {
    ...
    return this.maybeReconnectAsync()
      .then(() =>
        this.talkAsync(HF2_CMD_START_FLASH).then(
          () => {},
          err =>
  ...
  pingAsync() {
    if (this.rawMode) return Promise.resolve();
    return this.talkAsync(HF2_CMD_BININFO).then(buf => {});
  }

  maybeReconnectAsync() {
    return this.pingAsync().catch(e =>
      this.reconnectAsync().then(() => this.pingAsync())
    );
  }
  ...
  flashAsync(blocks: pxtc.UF2.Block[]) {
    ...
    let fstart = 0;
    const loopAsync = (pos: number): Promise<void> => {
      if (pos >= blocks.length) return Promise.resolve();
      let b = blocks[pos];
      ...
      return this.talkAsync(HF2_CMD_WRITE_FLASH_PAGE, buf).then(() =>
        loopAsync(pos + 1)
      );
    };
    return this.switchToBootloaderAsync()
      .then(() => {
        const size = blocks.length * 256;
        log(`Starting flash (${Math.round(size / 1024)}kB).`);
        fstart = Date.now();
        // only try partial flash when page size is small
        if (this.pageSize > 16 * 1024) return blocks;
        return onlyChangedBlocksAsync(blocks, (a, l) =>
          this.readWordsAsync(a, l)
        );
      })
      .then(res => {
      ...
      .then(() => {
        ...
        const t1 = n - fstart;
        log(
          `Flashing done at ${Math.round(
            (((blocks.length * 256) / t1) * 1000) / 1024
          )} kB/s in ${t0}ms (reset ${t0 - t1}ms). Resetting.`
        );
      })
      .then(() =>
        this.talkAsync(HF2_CMD_RESET_INTO_APP).catch(e => {
  ...
  private initAsync() {
    ...
    return Promise.resolve()
      ...
      .then(binfo => {
        ...
        log(
          `Connected; msgSize ${this.maxMsgSize}B; flash ${
            this.flashSize / 1024
          }kB; ${
            this.bootloaderMode ? 'bootloader' : 'application'
          } mode; family=0x${this.familyID.toString(16)}`
        );
      ...
      .then(buf => {
        ...
        this.info = info;
        const m = /v(\d\S+)(\s+(\S+))?/.exec(this.info.Header);
        if (m)
          this.info.Parsed = {
            Version: m[1],
            Features: m[3] || '',
        ...
        log(
          `Board-ID: ${this.info.BoardID} v${this.info.Parsed.Version} f${this.info.Parsed.Features}`
        );
      })
      .then(() =>
        this.talkAsync(HF2_CMD_JDS_CONFIG, new Uint8Array([1])).then(
          () => {
...
export function mkHF2PacketIOWrapper(
...
export type ReadAsync = (
  addr: number,
  len: number
) => Promise<ArrayLike<number>>;
function readChecksumBlockAsync(
  readWordsAsync: ReadAsync
): Promise<pxtc.ChecksumBlock> {
  if (!pxt.appTarget.compile.flashChecksumAddr)
    return Promise.resolve(null as pxtc.ChecksumBlock);
  return readWordsAsync(pxt.appTarget.compile.flashChecksumAddr, 12).then(
    buf => {
      const blk = pxtc.parseChecksumBlock(buf);
      if (!blk) return null;
      return readWordsAsync(blk.endMarkerPos, 1).then(w => {
...
export function onlyChangedBlocksAsync(
  blocks: pxtc.UF2.Block[],
  readWordsAsync: ReadAsync
) {
  if (!pxt.appTarget.compile.flashChecksumAddr) return Promise.resolve(blocks);
  const blBuf = pxtc.UF2.readBytes(
    blocks,
    pxt.appTarget.compile.flashChecksumAddr,
    12 * 4
  );
  const blChk = pxtc.parseChecksumBlock(blBuf);
  if (!blChk) return Promise.resolve(blocks);
  return readChecksumBlockAsync(readWordsAsync).then(devChk => {
    if (!devChk) return blocks;
    const regionsOk = devChk.regions.filter(r => {
      let hasMatching = blChk.regions.some(
        r2 =>
    ...
    if (regionsOk.length == 0) return blocks;
    log(
      'skipping flash at: ' +
        regionsOk
          .map(r => `${pxtc.assembler.tohex(r.start)} (${r.length / 1024}kB)`)
          .join(', ')
    );
    const unchangedAddr = (a: number) =>
