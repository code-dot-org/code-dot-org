...
export default class ProgramExecutor {
  ...
  async execute(songMetadata: SongMetadata) {
    ...
    if (this.validationCode) {
      this.nativeAPI.registerValidation(
        getValidationCallback(this.validationCode)
      );
    }
  ...
  private async preloadSpritesAndCompileCode(
    ...
    const charactersReferenced = computeCharactersReferenced(code);
  ...
  private async init(
    ...
    if (isReadOnlyWorkspace) {
      ...
      const charactersReferenced = computeCharactersReferenced(this.getCode());
