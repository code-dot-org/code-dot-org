...
export default class Dance {
   ...
   */
  init(config) {
    if (!this.studioApp_) throw new Error('Dance requires a StudioApp');
    ...
    const onMount = () => {
      ...
      if (finishButton) dom.addClickTouchEvent(finishButton, () => this.onPuzzleComplete(true));
    };
  ...
  */
  reset() {
    const clickToRunImage = document.getElementById('danceClickToRun');
    if (clickToRunImage) clickToRunImage.style.display = 'block';

    Sounds.getSingleton().stopAllAudio();

    this.nativeAPI.reset();

    let softButtonCount = 0;
    for (let i = 0; i < this.level.softButtons.length; i++) {
...
onPuzzleComplete(result, message) {
    ...
    const program = encodeURIComponent(Blockly.Xml.domToText(xml));
   ...
   */
  async runButtonClick() {
    const clickToRunImage = document.getElementById('danceClickToRun');
    if (clickToRunImage) clickToRunImage.style.display = 'none';

    // Block re-entrancy since starting a run is async
    // (not strictly needed since we disable the run button,
    // but better to be safe)
    if (getStore().getState().songs.runIsStarting) return;
  ...
  computeCharactersReferenced(studentCode) {
