...
export default class Dance {
  ...
  onArrowButtonDown(buttonId, e) {
    ...
    this.nativeAPI.onKeyDown(this.keyCodeFromArrow(buttonId));
  }
  ...
  onArrowButtonUp(buttonId, e) {
    ...
    this.nativeAPI.onKeyUp(this.keyCodeFromArrow(buttonId));
  }
  ...
  afterInject_() {
    ...
    const recordReplayLog =
      this.shouldShowSharing() || this.level.isProjectLevel;
  ...
  reset() {
    ...
    for (let i = 0; i < this.level.softButtons.length; i++) {
      document.getElementById(this.level.softButtons[i]).style.display =
        'inline';
      softButtonCount++;
    }
    if (softButtonCount) {
  ...
  async runButtonClick() {
    ...
    trackEvent(
      'HoC_Song',
      'Play-2019',
      getStore().getState().songs.selectedSong
    );
