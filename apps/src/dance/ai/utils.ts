...
const doAi = async (input: string) => {
  ...
  // Will add all the new effects and colors once they are added to the dance-party repo.
  ...
  const danceExampleJson =
    '{backgroundColor: "tropical", backgroundEffect: "swirl", foregroundEffect: "hearts_red"}';
  const numWords = 40;
  const systemPrompt = `You are a helper which can accept a request for a mood or atmosphere, and you then generate JSON like the following format: ${danceExampleJson}.  The only valid values for backgroundEffect are ${oldDanceBackgroundEffects}. The only valid values for backgroundColor are ${oldDanceBackgroundColors}. The only valid values for foregroundEffect are ${danceForegroundEffects}. Make sure you always generate all three of these values.  Also, add a field called "explanation" to the result JSON, which contains a simple explanation of why you chose the values that you did, at the reading level of a 5th-grade school student, in one sentence of less than ${numWords} words.`;
  const messages = [
    {
      role: 'system',
