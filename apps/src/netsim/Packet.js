...
export default function Packet(formatSpec, binary) {
  ...
  /** @type {Packet.Encoder} */
  this.encoder = new Packet.Encoder(
    level.addressFormat,
...
Packet.isAddressField = function (headerType) {
  return (
    headerType === Packet.HeaderType.TO_ADDRESS ||
...
Packet.isPacketField = function (headerType) {
  return (
    headerType === Packet.HeaderType.PACKET_INDEX ||
...
Packet.Encoder.prototype.validateSpec = function () {
  ...
  for (var i = 0; i < this.headerSpec_.length; i++) {
    ...
    if (isAddressField && this.addressBitWidth_ === 0) {
      throw new Error(
        'Invalid packet format: Includes an address field but ' +
    ...
    if (isPacketField && this.packetCountBitWidth_ === 0) {
      throw new Error(
        'Invalid packet format: Includes a packet count field ' +
    ...
    if (!isAddressField && !isPacketField) {
      throw new Error(
        'Invalid packet format: Unrecognized packet header field ' +
          this.headerSpec_[i]
      );
    }
...
Packet.Encoder.prototype.getHeaderLength = function () {
  return this.headerSpec_.reduce(
    function (prev, cur) {
    ...
    if (Object.prototype.hasOwnProperty.call(headers, headerField)) {
      ...
      if (Packet.isAddressField(headerField)) {
        ...
        binaryHeaders[headerField] = DataConverters.intToBinary(
          headers[headerField],
          this.getFieldBitWidth(headerField)
        );
      }
    }
  }, this);
