 ...
 */
DropletAutocompleteParameterTooltipManager.prototype.installTooltipsForEditor_ =
  function (dropletEditor) {
 ...
 * @private
 */
DropletAutocompleteParameterTooltipManager.prototype.onCursorMovement_ =
  function (editor, changeEvent) {
    this.hideTooltip_();
 ...
 * @private
 */
DropletAutocompleteParameterTooltipManager.prototype.showParamDropdownIfNeeded_ =
  function (editor, paramInfo) {
    ...
    getAllAvailableDropletBlocks(
      ...
      if (
        typeof block.dropdown[paramInfo.currentParameterIndex] === 'function'
      ) {
...
DropletAutocompleteParameterTooltipManager.prototype.updateParameterTooltip_ =
  function (aceEditor, paramInfo) {
...
DropletAutocompleteParameterTooltipManager.prototype.hideTooltip_ =
  function () {
...
DropletAutocompleteParameterTooltipManager.prototype.getDesiredTooltipPosition_ =
  function () {
    ...
    const showOnLeft =
      ...
      (cursorRect.top - aceRect.top < SAFE_VERTICAL_DISTANCE_FOR_TOOLTIP ||
        aceRect.bottom - cursorRect.bottom <
          SAFE_VERTICAL_DISTANCE_FOR_TOOLTIP);
    let offsetX = 0;
    ...
    if (showOnLeft) {
      ...
      offsetX = Math.max(
        ...
        cursorRect.left -
          (window.innerWidth -
            LEFT_OFFSET_EXTRA_MARGIN -
            ACE_AUTOCOMPLETE_WIDTH)
      );
    }
    return {position, offsetX};
  };

DropletAutocompleteParameterTooltipManager.prototype.createOrUpdateCursorTooltip_ =
  function () {
    if (!this.cursorTooltip_) {
 ...
 * @private
 */
DropletAutocompleteParameterTooltipManager.prototype.installAceCompleterHooks_ =
  function (editor) {
    if (
    ...
    if (!editor.completer.insertMatchOverride) {
      editor.completer.insertMatchOverride =
        DropletAutocompleteParameterTooltipManager.insertMatch.bind(
          editor.completer,
          this
        );
    }
 ...
 */
DropletAutocompleteParameterTooltipManager.retrieveToEndOfQuotedText =
  function (line, pos, character) {
 ...
 */
DropletAutocompleteParameterTooltipManager.prototype.setTooltipsEnabled =
  function (enabled) {
