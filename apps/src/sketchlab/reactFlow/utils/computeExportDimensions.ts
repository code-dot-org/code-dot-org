// Pure math for sizing the backpack PNG export: given a bounding box (in flow
// space) containing every node + edge, compute the output image dimensions
// and the transform to apply to the React Flow viewport so the content fills
// the image with `padding` pixels of margin.
//
...
// exceeds `maxDim` is scaled down proportionally to fit.
...
export function computeExportDimensions(
  ...
  const scale = Math.min(1, maxDim / Math.max(contentWidth, contentHeight));
  return {
