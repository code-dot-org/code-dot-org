import _ from 'lodash';

// Generate n fake personal projects for the project widget.
// updatedAt data is normally a timestamp, but for storybook
// display purposes, use a descriptive string
export function generateFakePersonalProjects(n) {
  return _.range(n).map(i => (
    {
      name: "Personal " + i,
      updatedAt: 'a day ago',
      projectType: 'gamelab',
      id: 'abcd'
    }
  ));
}

export const stubFakeFeaturedProjectData = [
  {
    projectName: "(Un)Featured Project 1",
    type: 'gamelab',
    channel: 'abcd1',
    publishedAt: '2015-02-04T23:59:59.999-08:00',
    featuredAt: '2016-12-31T23:59:59.999-08:00',
    unfeaturedAt: '2017-12-31T23:59:59.999-08:00'
  },
  {
    projectName: "(Un)Featured Project 2",
    updatedAt: '2016-11-30T23:59:59.999-08:00',
    type: 'applab',
    channel: 'abcd2',
    publishedAt: '2015-02-04T23:59:59.999-08:00',
    featuredAt: '2016-12-31T23:59:59.999-08:00',
    unfeaturedAt: '2017-12-31T23:59:59.999-08:00'
  },
  {
    projectName: "(Un)Featured Project 3 has a super extra long name let's see how it looks in the table",
    updatedAt: '2016-10-31T23:59:59.999-08:00',
    type: 'artist',
    channel: 'abcd3',
    publishedAt: '2015-02-04T23:59:59.999-08:00',
    featuredAt: '2016-12-31T23:59:59.999-08:00',
    unfeaturedAt: '2017-12-31T23:59:59.999-08:00'
  },
  {
    projectName: "(Un)Featured Project 4",
    updatedAt: '2016-09-14T23:59:59.999-08:00',
    type: 'playlab',
    channel: 'abcd4',
    publishedAt: '2015-02-04T23:59:59.999-08:00',
    featuredAt: '2016-12-31T23:59:59.999-08:00',
    unfeaturedAt: '2017-12-31T23:59:59.999-08:00'
  },
];
