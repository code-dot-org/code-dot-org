...
export function loadScript(scriptId, sectionId) {
  ...
  if (
    state.studentLevelStatusByScript[scriptId] &&
  ...
  const requests = _.range(1, numPages + 1).map(currentPage => {
    ...
    return fetch(url, {credentials: 'include'})
      .then(response => response.json())
      .then(data => {
        sectionProgress.studentLevelStatusByScript = {
          [scriptId]: {
            ...sectionProgress.studentLevelStatusByScript[scriptId],
        ...
        sectionProgress.studentLastUpdateByScript = {
          [scriptId]: {
            ...
            ...processStudentTimestamps(data.students_last_update)
          }
        };
      });
  });
