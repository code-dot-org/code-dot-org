...
export default function CensusTeacherBanner({
  ...
  //const [ncesSchoolId, setNcesSchoolId] = useState(null);
  ...
  const schoolInfo = useSchoolInfo({
    schoolId: ncesSchoolId,
    country: 'US',
  });

  console.log('schoolInfo', schoolInfo);

  const loadSchoolNameSuccess = useCallback(
    response => {
      //setSchoolDisplayName(response.name);
      //setSchoolType(response.school_type);
      schoolInfo.setSchoolName(response.name);
      schoolInfo.setSchoolZip(response.zip);
    },
    [schoolInfo]
  );

  const loadSchoolNameError = useCallback(
    error => {
      //setSchoolDisplayName('your school');
      schoolInfo.setSchoolName('your school');
    },
    [schoolInfo]
  );

  const loadSchoolName = useCallback(
    schoolId => {
    ...
    [loadSchoolNameError, loadSchoolNameSuccess]
  );

  useEffect(() => {
    if (
      schoolInfo.schoolId !== NonSchoolOptions.SELECT_A_SCHOOL &&
      schoolInfo.schoolId !== NonSchoolOptions.NO_SCHOOL_SETTING &&
      schoolInfo.schoolId !== NonSchoolOptions.CLICK_TO_ADD
    ) {
      loadSchoolName(schoolInfo.schoolId);
    }
  }, [schoolInfo.schoolId, loadSchoolName]);

  const hideSchoolInfoForm = () => {
    setShowSchoolInfoForm(false);
    //setSchoolInfoErrors(false);
  };
  ...
  const handleSchoolInfoSubmit = () => {
    /*if (schoolInfoInputs.isValid()) {
      let schoolData;
      if (ncesSchoolId === '-1') {
        schoolData = {
          _method: 'patch',
          'user[school_info_attributes][country]': schoolInfo.country,
          'user[school_info_attributes][school_type]': schoolInfo.schoolType,
          'user[school_info_attributes][school_name]': schoolInfo.schoolName,
          'user[school_info_attributes][school_state]': schoolInfo.schoolState,
          'user[school_info_attributes][school_zip]': schoolInfo.schoolZip,
