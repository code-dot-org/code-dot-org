 ...
 */
const PixelEditor: React.FunctionComponent<PixelEditorProps> = ({
  title,
  imageUrl,
  knownPixelGrid,
  initialRecentColors,
  ...
  useLayoutEffect(() => {
    ...
    if (display && backing) {
      ...
      const resizeDisplay = () => {
        ...
        const maxW = Math.min(
          MAX_CSS_WIDTH_PX,
          availableWidth ? Math.max(200, availableWidth - 24) : Infinity,
  ...
  const handleCanvasFocus = useCallback(
    (e: ReactFocusEvent<HTMLCanvasElement>) => {
  ...
  return (
    ...
    >
      <span id="pixel-editor-description" className={moduleStyles.srOnly}>
