...
// How many levels the outline AI may plan; course drafting rules unlock
// the higher ceiling.
export const LESSON_LEVEL_COUNT = {min: 2, max: 8, maxWithDraftingRules: 12};
export const SUBLEVEL_COUNT = {min: 2, max: 6};

export function lessonLevelCap(draftingRules?: string): number {
  return draftingRules?.trim()
...
const outlineLevelSchema = z.object({
  ...
  sublevels: z
    ...
    .optional()
    .describe(
      `REQUIRED for bubbleChoice levels; MUST be omitted for every other labType. ${SUBLEVEL_COUNT.min}-${SUBLEVEL_COUNT.max} sublevel entries in the order the student will see them on the picker page.`
    ),
});

const lessonOutlineSchema = (maxLevels: number) =>
  Output.object({
    schema: z.object({
      levels: z
        .array(outlineLevelSchema)
        .min(LESSON_LEVEL_COUNT.min)
        .max(maxLevels),
    }),
  });
...
export async function generateLessonOutline(
  ...
  const maxLevels = lessonLevelCap(ctx.draftingRules);
  const countLine =
    maxLevels > LESSON_LEVEL_COUNT.max
      ? `${LESSON_LEVEL_COUNT.min} to ${LESSON_LEVEL_COUNT.max} levels, or up to ${maxLevels} where the course drafting rules below demand it.`
      : `${LESSON_LEVEL_COUNT.min} to ${LESSON_LEVEL_COUNT.max} levels.`;
  const prompt = [
    ...
    `order, take the student through the learning experience: ${countLine}`,
    'Each level is one of:',
    ...labTypeBullets(),
    '',
  ...
  const response = await generateText({
    model: getTextModel(),
    prompt,
    output: lessonOutlineSchema(maxLevels),
  });
