  ...
  const WEBPACK_CONFIG = {
    ...
    plugins: [
      ...
      ...(watch && watchNotify
        ...
        : []),
      // This custom webpack plugin checks for the presence of files
      // generated by a production webpack build, which can produce unexpected
      // behavior in development if left hanging around.
      // More detail on custom plugins here: https://webpack.js.org/contribute/writing-a-plugin/
      {
        apply: compiler => {
          compiler.hooks.afterEmit.tap('AfterEmitPlugin', compilation => {
            if (
              ...
              console.log(
                chalk.yellow(
                  '\nblah blah, stale file, blah blah, run yarn clean, etc etc'
                )
              );
            }
          });
