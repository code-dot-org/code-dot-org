...
# fetch most recent version of compiled css (legacy-styles.css) that is generated
# by rails
echo "Compiling legacy-styles.css using rails asset pipeline"
pushd ../dashboard
bundle install
bundle exec rake assets:precompile
MOST_RECENT_APPLICATION_CSS=$(ls -t public/assets/legacy-styles-*.css | head -1)
cp $MOST_RECENT_APPLICATION_CSS ../apps/build/package/css/legacy-styles.css
popd
...
sed -i -E "s#url\\(([\"']?)/fonts/#url(\\1../fonts/#g" "$DIR_TO_DEPLOY/css/legacy-styles.css"
...
rm ./build/package/css/legacy-styles.css
