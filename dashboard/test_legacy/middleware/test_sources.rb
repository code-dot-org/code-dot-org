...
MAIN_JSON = 'main.json'
COMMENT_BLOCK_SOURCES = File.join(__dir__, '..', '..', 'test', 'fixtures', 'middleware', 'comment-block-sources.json')
...
class SourcesTest < FilesApiTestBase
  ...
  def test_get_source_blocks_profanity_violations
    ...
    filename = MAIN_JSON
    file_data = File.read(File.expand_path('../../../test/fixtures/middleware/privacy-profanity/playlab-normal-source.json', __FILE__))
    file_headers = {'CONTENT_TYPE' => 'application/json'}
    @api.put_object(filename, file_data, file_headers)
    assert successful?
  ...
  def test_get_source_blocks_privacy_violations
    filename = MAIN_JSON
    file_data = File.read(File.expand_path('../../../test/fixtures/middleware/privacy-profanity/playlab-privacy-violation-source.json', __FILE__))
    file_headers = {'CONTENT_TYPE' => 'application/json'}
    @api.put_object(filename, file_data, file_headers)
    assert successful?
