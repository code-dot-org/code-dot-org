...
// ToDo: Move debug logger from browser console to student console
...
function addBehavior(sprite, behavior) {
  ...
  if (DEBUG) {
    var spritei = sprites.indexOf(sprite);
    log(spritei + " begins " + behavior.func.name);
    log(spritei + " is " + sprite.reportBehaviors());
  }
}
...
function removeBehavior(sprite, behavior) {
  ...
  if (DEBUG) {
    var spritei = sprites.indexOf(sprite);
    log(spritei + " stops " + behavior.func.name);
    log(spritei + " is " + sprite.reportBehaviors());
  }
}
...
function makeNewSprite(animation, x, y) {
  ...
  sprite.reportBehaviors = function() {
    return sprite.behaviors.map(function(b){
      return b.func.name;
    }).join(",");
  };
  return sprite;
...
function draw() {
  ...
  if (shouldUpdate()) {
    ...
    for (i = 0; i < inputEvents.length; i++) {
      ...
      if (eventType(param)) {
        if (DEBUG) log("event " + eventType + " " + param);
        event();
      }
    }
    ...
    var createCollisionHandler = function (collisionEvent) {
      return function (sprite1, sprite2) {
        if (!collisionEvent.touching || collisionEvent.keepFiring) {
          if (DEBUG) {
            var sprite1i = sprites.indexOf(sprite1);
            var sprite2i = sprites.indexOf(sprite2);
            log("event " + sprite1i + " touched " + sprite2i);
          }
