...
puts "script_preload.rb: rake? #{File.basename($0) == 'rake'} Should cache? #{Unit.should_cache?} skip? #{!!ENV['SKIP_SCRIPT_PRELOAD']}"
...
if File.basename($0) != 'rake' &&
  ...
  puts "script_preload.rb: Populating cache..."
  start_time = Time.now
  ...
  puts "script_preload.rb: Cache populated in #{(Time.now - start_time).to_i} seconds"
end
