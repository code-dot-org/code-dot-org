module Cdo
  ...
  module RefreshActiveModelCachedAttributes
    def initialize(*args, **kwargs)
      count_and_remember_database_migrations unless defined?(@@current_number_of_database_migrations)
      super
    end
    ...
    private def new_database_migration_since_initialization?
      current_num_migrations = ApplicationRecord.connection.migration_context.get_all_versions.length
      return current_num_migrations != @@current_number_of_database_migrations
    end

    private def count_and_remember_database_migrations
      @@current_number_of_database_migrations = ApplicationRecord.connection.migration_context.get_all_versions.length
    end
  end
end
