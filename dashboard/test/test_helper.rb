...
module FakeSQSService
  def self.create
    ...
    service = FakeSQS::TestIntegration.new(
        database: ":memory#{ENV['TEST_ENV_NUMBER']}:",
        sqs_endpoint: 'localhost',
        sqs_port: 4568 + ENV['TEST_ENV_NUMBER'].to_i,
        debug: true
    )
    sleep(14) # add a sleep to fix test failures with 'RuntimeError: FakeSQS didn't start in time'
    service
  end
end
