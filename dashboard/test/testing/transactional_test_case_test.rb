...
class ActiveSupport::Testing::TransactionalTestCaseTest < ActiveSupport::TestCase
  ...
  setup_all do
    # Asserts that exactly one database transaction is open on the current connection during setup_all,
  ...
  shared_examples_for 'shares setup_all context' do
    # Verifies that each example runs inside its own database transaction,
    # in addition to the class level transaction opened by the parent `setup_all`.
    it 'opens transaction per test' do
  ...
  it_behaves_like 'shares setup_all context'

  # Minitest::Spec `describe` and `context` blocks are not isolated and share the parent test class context.
  describe 'not isolated describe block' do
    it_behaves_like 'shares setup_all context'

    context 'not isolated context block' do
      it_behaves_like 'shares setup_all context'
    end
  end
  ...
  class IsolationTest < ActiveSupport::TestCase
    ...
    test 'opens transaction per test' do
      assert_equal 2, ActiveRecord::Base.connection.open_transactions
    end
