...
class ApplicationJobTest < ActiveJob::TestCase
  ...
  describe '#unique?' do
    subject(:unique?) {described_instance.unique?}

    let(:described_instance) {TestableJob.new('foo', bar: 2)}

    shared_examples_for 'unique job' do
      it 'returns true' do
        _unique?.must_equal true
      end
    end

    shared_examples_for 'non unique job' do
      it 'returns false' do
        _unique?.must_equal false
      end
    end

    it_behaves_like 'unique job'

    context 'when similar job with same arguments is already enqueued' do
      let!(:delayed_job) {ActiveJob::QueueAdapters::DelayedJobAdapter.new.enqueue(described_instance)}

      it_behaves_like 'non unique job'
    ...
    context 'when similar job with different arguments is already enqueued' do
      let!(:delayed_job) {ActiveJob::QueueAdapters::DelayedJobAdapter.new.enqueue(TestableJob.new('bar', foo: 1))}

      it_behaves_like 'unique job'
    end

    describe 'with failed' do
      subject(:unique?) {described_instance.unique?(with_failed: true)}
