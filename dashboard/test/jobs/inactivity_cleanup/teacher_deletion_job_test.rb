...
class InactivityCleanup::TeacherDeletionJobTest < ActiveJob::TestCase
  let(:described_class) {InactivityCleanup::TeacherDeletionJob}
  ...
  describe '#perform' do
    ...
    before do
      InactivityCleanup::TeacherDeleter.any_instance.stubs(:call)
    end

    it 'calls inactive teacher deleter with default arguments' do
      InactivityCleanup::TeacherDeleter.
        expects(:new).
        with(dry_run: false, limit: nil).
        returns(mock('inactive teacher deleter', call: nil)).
        once
    ...
    context 'when :dry_run is provided' do
      ...
      it 'calls inactive teacher deleter with provided :dry_run argument' do
        InactivityCleanup::TeacherDeleter.
          expects(:new).
