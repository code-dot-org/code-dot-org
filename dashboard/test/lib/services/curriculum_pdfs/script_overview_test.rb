...
class Services::CurriculumPdfs::ScriptOverviewTest < ActiveSupport::TestCase
  ...
  test 'get_script_overview_url returns nil if we did not generate a pdf' do
    ...
    unit_with_lesson_plans = create(:script, :in_single_unit_course, seeded_from: Time.at(1), name: "test-pdf-path1")
    ...
    unit_without_lesson_plans = create(:script, :in_single_unit_course, seeded_from: Time.at(1), name: "test-pdf-path2")
  ...
  test 'PDF paths (and urls) are versioned' do
    script = create(:script, :in_single_unit_course, seeded_from: Time.at(1), name: "test-pdf-path")
