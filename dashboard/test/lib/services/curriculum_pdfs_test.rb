...
class Services::CurriculumPdfsTest < ActiveSupport::TestCase
  setup do
  ...
  test 'PDF paths (and urls) are versioned' do
    ...
    original_pathname = Services::CurriculumPdfs.get_pathname(lesson)
    unmodified_pathname = Services::CurriculumPdfs.get_pathname(lesson)
    assert_equal original_pathname, unmodified_pathname

    script.update!(seeded_from: Time.at(234_567_890))
    new_pathname = Services::CurriculumPdfs.get_pathname(lesson)
  ...
  test 'Lesson PDFs are generated into the given directory' do
    ...
    Dir.mktmpdir('lesson_plan_pdfs_test') do |tmpdir|
      ...
      filename = File.join(tmpdir, Services::CurriculumPdfs.get_pathname(lesson))
  ...
  test 'Student Lesson PDFs are generated into the given directory' do
    ...
    Dir.mktmpdir('lesson_plan_pdfs_test') do |tmpdir|
      ...
      filename = File.join(tmpdir, Services::CurriculumPdfs.get_pathname(lesson, true))
      PDF.expects(:generate_from_url).with(url, filename)
