...
module Services
  class CurriculumPdfsTest < ActiveSupport::TestCase
    ...
    test 'will not generate a overview PDF when unit does not have lesson plans' do
      CDO.stubs(:rack_env).returns(:staging)
      unit_with_lesson_plans = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.beta)
      ...
      unit_without_lesson_plans = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.beta)
    ...
    test 'will not generate a overview PDF when unit is in pilot or in-development published state' do
      CDO.stubs(:rack_env).returns(:staging)
      pilot_unit = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.pilot)
      pilot_lg = create(:lesson_group, script: pilot_unit)
      create(:lesson, script: pilot_unit, lesson_group: pilot_lg, has_lesson_plan: true)

      in_development_unit = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.in_development)
      ...
      beta_unit = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.beta)
      beta_lg = create(:lesson_group, script: beta_unit)
      create(:lesson, script: beta_unit, lesson_group: beta_lg, has_lesson_plan: true)

      preview_unit = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.preview)
      ...
      stable_unit = create(:script, is_migrated: true, published_state: SharedConstants::PUBLISHED_STATE.stable)
