...
class ResourcesAutocompleteTest < ActiveSupport::TestCase
  ...
  test "finds resource with matching name" do
    matches = ResourcesAutocomplete.get_search_matches('Studi', 5, @coursea_2017.id)
    assert_equal 1, matches.length
    assert_equal 'Code Studio', matches[0][:name]
  end

  test "finds resource with matching url" do
    matches = ResourcesAutocomplete.get_search_matches("wikip", 5, @coursea_2017.id)
    assert_equal 1, matches.length
    assert_equal 'wiki', matches[0][:name]
  end

  test "finds multiple matches" do
    matches = ResourcesAutocomplete.get_search_matches("class", 5, @coursea_2017.id)
  ...
  test "limits matches by course version id" do
    ...
    matches = ResourcesAutocomplete.get_search_matches("class", 5, @courseb_2017.id)
    assert_equal 1, matches.length
    assert_equal 'resource_105', matches[0][:key]
  end

  test "only returns up to limit matches" do
    matches = ResourcesAutocomplete.get_search_matches("class", 1, @coursea_2017.id)
    assert_equal 1, matches.length
  end

  test "returns empty list for short query" do
    matches = ResourcesAutocomplete.get_search_matches("cl", 5, @coursea_2017.id)
