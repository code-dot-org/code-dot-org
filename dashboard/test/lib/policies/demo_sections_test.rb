...
class Policies::DemoSectionsTest < ActiveSupport::TestCase
  ...
  test 'get_preset uses ui test curriculum names in test' do
    ...
    assert_equal 'ui-test-all-the-levels', preset[:unit_name]
    assert_equal 'ui-test-original-all-the-levels', preset[:unit_group_name]
  end
  ...
  test 'preset_view returns a display projection for a valid preset' do
    ...
    assert_equal({name: 'ui-test-all-the-levels', display_name: 'All The Levels!'}, view[:unit])
    assert_equal(
      {name: 'ui-test-original-all-the-levels', display_name: 'ui-test-original-all-the-levels'},
      view[:unit_group]
    )
  end
