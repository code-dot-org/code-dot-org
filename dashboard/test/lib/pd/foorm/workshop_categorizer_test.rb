...
module Pd::Foorm
  class WorkshopCategorizerTest < ActiveSupport::TestCase
    test 'initialize_categories creates correct structure' do
      categories = WorkshopCategorizer.initialize_categories
    ...
    test 'determine_category falls back to other for invalid categories' do
      assert_equal :other, WorkshopCategorizer.determine_category('invalid_category')
      assert_equal :other, WorkshopCategorizer.determine_category('random')
    ...
    test 'categorize_survey_data processes questions into correct categories' do
      ...
      assert_equal [], result[:logistics][:questions]
