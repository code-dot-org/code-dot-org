...
class InactivityCleanup::TeacherDeleterTest < ActiveSupport::TestCase
  subject(:described_instance) {InactivityCleanup::TeacherDeleter.new(**described_instance_args)}
  ...
  describe '#call' do
    subject(:delete_inactive_teachers) {described_instance.call}
    ...
    let!(:user_data_retention_status) {create(:user_data_retention_status, user: account, deletion_warning_email_sent_at:)}
    ...
    shared_examples_for 'deletes account' do
      it 'deletes account' do
        _ {delete_inactive_teachers}.must_change -> {account.reload.deleted?}, from: false, to: true
      ...
      it 'logs to slack' do
        ChatClient.expects(:message).with do |channel, message, options|
          ...
          _(message).must_equal <<~MSG.chomp
            *InactivityCleanup::TeacherDeleter Cronjob*
      ...
      it 'returns summary' do
        _delete_inactive_teachers.must_equal <<~MSG.chomp
          Deleted 1 accounts
          Duration 00:00:00
        MSG
      end
    ...
    shared_examples_for 'ignores account' do |num_errors: 0|
      it 'does not delete account' do
        _ do
          _ {delete_inactive_teachers}.wont_change -> {account.reload.deleted?}
      ...
      it 'returns summary' do
        ...
        _delete_inactive_teachers.must_equal expected_summary
      end
    end

    it_behaves_like 'deletes account'

    context 'when teacher has been inactive longer than allowed period' do
      let!(:account) {create(:teacher, created_at: InactivityCleanup::INACTIVITY_THRESHOLD.ago - 1.minute, current_sign_in_at: nil)}
    ...
    context 'when teacher has been signed in within allowed period' do
    ...
    context 'when inactive teacher is already deleted' do
      before do
        account.destroy!
      end
    ...
    context 'when account is inactive student' do
    ...
    context 'when something goes wrong during account deletion' do
      ...
      before do
        Teacher.any_instance.expects(:destroy!).raises(error)
      end
    ...
    context 'when dry_run' do
      ...
      it 'does not upload metrics' do
        Metrics::Events.expects(:log_event).with(event_name: 'inactive_teacher_deleter', metadata: anything).never
        delete_inactive_teachers
      end
      ...
      it 'logs to slack' do
        ChatClient.expects(:message).with do |channel, message, options|
          _(channel).must_equal 'cron-daily'
          _(message).must_equal <<~MSG.chomp
            *InactivityCleanup::TeacherDeleter Cronjob* (dry-run)
            <https://github.com/code-dot-org/code-dot-org/blob/production/dashboard/lib/inactivity_cleanup|(source)>
            Deleted 1 accounts
            Duration 00:00:00
