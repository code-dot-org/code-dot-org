...
class FeatureModeManagerTest < ActiveSupport::TestCase
  ...
  HOC_AND_CSF_SCRIPTS = HOC_SCRIPTS + CSF_SCRIPTS
  ...
  def test_round_trip_feature_modes
    %w{normal scale emergency}.each do |mode|
      FeatureModeManager.set_mode(mode, @gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
      assert_equal mode, FeatureModeManager.get_mode(@gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    end
  end
  ...
  def test_get_mode_returns_nil_if_dcdo_property_does_not_match
    ...
    assert_nil FeatureModeManager.get_mode(@gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    # Set the DCDO property back to the original value and make sure we're in normal mode.
    @dcdo.set('hoc_activity_sample_weight', 1)
    assert_equal 'normal', FeatureModeManager.get_mode(@gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
  end

  def test_get_mode_returns_nil_if_hoc_gatekeeper_property_does_not_match
    FeatureModeManager.set_mode('normal', @gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    @gatekeeper.set('postMilestone', where: {script_name: HOC_SCRIPT}, value: false)
    assert_nil FeatureModeManager.get_mode(@gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    @gatekeeper.set('postMilestone', where: {script_name: HOC_SCRIPT}, value: true)
    assert_equal 'normal', FeatureModeManager.get_mode(@gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
  end
  ...
  def test_allows
    ...
    assert FeatureModeManager.allows(@gatekeeper, 'scale', 'postMilestone', HOC_SCRIPT)
    assert FeatureModeManager.allows(@gatekeeper, 'scale', 'hint_view_request', HOC_SCRIPT)
    assert FeatureModeManager.allows(@gatekeeper, 'scale', 'hint_view_request', nil)
    refute FeatureModeManager.allows(@gatekeeper, 'emergency', 'postMilestone', HOC_SCRIPT)
    assert FeatureModeManager.allows(@gatekeeper, 'emergency', 'postMilestone', nil)
    refute FeatureModeManager.allows(@gatekeeper, 'emergency', 'hint_view_request', HOC_SCRIPT)
    ...
    # setting.
    refute FeatureModeManager.allows(@gatekeeper, 'normal', 'newFeature', HOC_SCRIPT)
  ...
  def test_fallback_mode
    FeatureModeManager.set_mode('fallback', @gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    refute @gatekeeper.allows('puzzle_rating')
    assert @gatekeeper.allows('hint_view_request')
    HOC_SCRIPTS.each do |script|
      assert @gatekeeper.allows('postMilestone', where: {script_name: script})
  ...
  def test_raises_on_invalid_mode
    assert_raises RuntimeError do
      FeatureModeManager.set_mode('invalid', @gatekeeper, @dcdo, HOC_SCRIPTS, CSF_SCRIPTS)
    end
  end
end
