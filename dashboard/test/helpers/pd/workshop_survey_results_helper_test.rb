  ...
  test 'averaging across multiple surveys' do
    ...
    workshop_1.survey_responses.each do |response|
      response.update_form_data_hash(
        {
          how_happy_after:  'Extremely happy'
        }
      )
      response.save
    end
    ...
    workshop_2.survey_responses.each do |response|
      response.update_form_data_hash(
        {
          how_happy_after: 'Not at all happy'
        }
      )
      response.save
    end

    # With 5 people saying "Extremely happy" (5 out of 5 score) and 10 people saying "Not at all happy" (1 out of 5 score),
    # we'd expect the answer to be [(10 * 1) + (5 * 5)] / (15 surveys) = 2.33
    result_hash = summarize_workshop_surveys(workshops: [workshop_1, workshop_2], facilitator_breakdown: false)
    assert_equal 2.33, result_hash[:how_happy_after]
  end
end
