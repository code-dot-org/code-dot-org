...
class AichatSagemakerHelperTest < ActionView::TestCase
  ...
  test 'Testing format_inputs_for_sagemaker_request with Karen model' do
    karen_model_customizations = @common_model_customizations.merge(selectedModelId: SharedConstants::AI_CHAT_MODEL_IDS[:KAREN]).stringify_keys
    ...
    assert_equal expected_karen_model_inputs, inputs[:inputs]
  end

  test 'Testing format_inputs_for_sagemaker_request with Arithmo model' do
    arithmo_model_customizations = @common_model_customizations.merge(selectedModelId: SharedConstants::AI_CHAT_MODEL_IDS[:ARITHMO]).stringify_keys
  ...
  test 'Testing format_inputs_for_sagemaker_request with Pirate model for custom stop string' do
    pirate_model_customizations = @common_model_customizations.merge(selectedModelId: SharedConstants::AI_CHAT_MODEL_IDS[:PIRATE]).stringify_keys
    ...
    assert_equal expected_pirate_stop_strings, inputs[:parameters][:stop]
  end
