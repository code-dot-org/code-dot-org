...
class TeacherApplicationHelperTest < ActionView::TestCase
  ...
  setup_all do
    ...
    regional_partner_with_closed_apps = create :regional_partner, apps_close_date_teacher: (Time.zone.current - 3.days).strftime("%Y-%m-%d")
    ...
    regional_partner_with_open_apps = create :regional_partner, apps_close_date_teacher: (Time.zone.current + 3.days).strftime("%Y-%m-%d")
