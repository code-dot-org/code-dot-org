# Blockly workspace setups for UI tests.
# To generate new JSON for a level, follow these steps:
# 1. Manually add the needed blocks to the level's workspace.
# 2. Run in the browser console:
#    Blockly.serialization.workspaces.save(Blockly.getMainWorkspace())
# 3. (Optional) Strip out any unnecessary fields from the JSON, such as 'id', 'deletable', 'movable'.
# 4. Minify the resulting JSON and pass it to `load_json_blocks(...)`.
