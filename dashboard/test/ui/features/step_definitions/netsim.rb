...
# Which lesson of ui-test-student-labs contains the netsim levels; this way we
# only have to update in one place if this changes.
NETSIM_STUDENT_LABS_LESSON = 14

# Given I am on the 1st NetSim test level
# Navigates to the requested level in the ui-test-student-labs unit.
Given /^I am on the (\d+)(?:st|nd|rd|th)? (?:Net ?Sim|Internet Simulator) test level$/ do |level_index|
  steps <<-STEPS
    And I am on "http://studio.code.org/courses/ui-test-student-labs/units/1/lessons/#{NETSIM_STUDENT_LABS_LESSON}/levels/#{level_index}?noautoplay=true"
  STEPS
end
