Feature: Swapped levels
  Scenario: Signed-out user sees active version
    ...
    When I am on "http://studio.code.org/s/allthethings/stage/29/puzzle/4"
    Then I wait until element ".standalone-video>h1" contains text "Video: Artist Intro"
    When I am on "http://studio.code.org/s/allthethings/stage/29/puzzle/5"
  ...
  Scenario: Student with progress sees old version
    ...
    And I verify the bubble for level 5 is an concept bubble
    When I am on "http://studio.code.org/s/allthethings/stage/29/puzzle/1"
