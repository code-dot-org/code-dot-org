Feature: Dance Party
  ...
  Scenario: Restricted audio content is protected
    ...
    When I am on "http://studio.code.org/s/dance/lesson/1/puzzle/1"
  ...
  Scenario: Can toggle run/reset in Dance Party
    Given I am on "http://studio.code.org/s/allthethings/lesson/37/puzzle/2?noautoplay=true"
  ...
  Scenario: Can get to level success in Dance Party
    Given I am on "http://studio.code.org/s/allthethings/lesson/37/puzzle/1?noautoplay=true"
    And I rotate to landscape
    And I wait for the page to fully load
    And I wait for 3 seconds
    And I wait until I don't see selector "#p5_loading"
  ...
  Scenario: Dance Party 12 loads
    Given I am on "http://studio.code.org/s/dance/lesson/1/puzzle/12?noautoplay=true"
    And I wait for the page to fully load

  Scenario: Dance Party 8 runs new set tint block
    Given I am on "http://studio.code.org/s/dance/lesson/1/puzzle/8?noautoplay=true"
    And I wait for the page to fully load
    And I select age 10 in the age dialog
    And I close the instructions overlay if it exists
  ...
  Scenario: Dance Party Share
    Given I am on "http://studio.code.org/s/dance/lesson/1/puzzle/13?noautoplay=true"
    And I rotate to landscape
    And I wait for the page to fully load
