...
Scenario: Applab debugging
  ...
  And I see no difference for "stepped in once" in the current viewport
  Then I press "stepInButton"
  And I see no difference for "stepped in twice" in the current viewport
  Then I press "stepInButton"
  And I see no difference for "stepped in thrice" in the current viewport
  Then I press "show-code-header"
  And I wait to see Droplet block mode
  And I press "resetButton"
  And I click droplet gutter line 1
  And I see no difference for "droplet breakpoint" in the current viewport
...
Scenario: Drag to delete
  ...
  Then I see no difference for "dragging in app doesn't delete button" in the current viewport

  When I drag element "#design_button1" 250 horizontally and 100 vertically
  Then I see no difference for "dragging slightly out of app leaves element partially out of bounds" in the current viewport

  When I drag element "#design_button1" 100 horizontally and 100 vertically
  And I wait until element "#design_button1" is gone
  Then I see no difference for "dragging out of app deletes button" in the current viewport
