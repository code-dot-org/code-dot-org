...
Feature: Using the manage students tab of the teacher dashboard
  Scenario: Teacher bulk updates US state for all section students
    ...
    And I click selector "button:contains(Set state for all students)"
    Then I wait until element "#us-state-column-bulk-set-modal" is visible
    And element "#us-state-column-bulk-set-modal h3" contains text "Set state for all students"
    And element "#us-state-column-bulk-set-modal label:contains(State)" is visible
    ...
    When I click selector "#uitest-manage-students-table tbody tr:nth-child(1) button:contains(Save)"
