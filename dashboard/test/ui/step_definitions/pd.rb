Given(/^I am a facilitator with started and completed courses$/) do
  random_name = "TestFacilitator" + SecureRandom.hex[0..5]
  steps %Q{
...
Given(/^I am an organizer with started and completed courses$/) do
  random_name = "TestOrganizer" + SecureRandom.hex[0..5]
  steps %Q{
    And I create a teacher named "#{random_name}"
...
And(/^I create a workshop for course "([^"]*)" ([a-z]+) by "([^"]*)" with (\d+) people and ([a-z]+) it$/) do |course, role, name, number, state|
  if role == 'organized'
