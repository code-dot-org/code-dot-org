...
class UserLevelTest < ActiveSupport::TestCase
  ...
  test "by_stage" do
    ...
    assert_includes UserLevel.by_stage(lesson), lesson_user_level
    refute_includes UserLevel.by_stage(lesson), other_user_level
  end
  ...
  test "by_stage will find all levels for each script_level" do
    ...
    assert_equal UserLevel.by_stage(lesson), []

    first_user_level = create :user_level, script: script, level: first_level

    assert_equal UserLevel.by_stage(lesson), [first_user_level]

    second_user_level = create :user_level, script: script, level: second_level

    assert_equal UserLevel.by_stage(lesson), [first_user_level, second_user_level]
  end
