...
class UserTest < ActiveSupport::TestCase
  ...
  test 'send reset password for student' do
    ...
    assert_include(mail.body.to_s, 'Change my password')
  ...
  test 'send reset password to parent for student without email address' do
    ...
    assert_include(mail.body.to_s, 'Change password for')
  ...
  test 'generate_progress_from_storage_id' do
    ...
    [regular_level, template_level].each do |level|
      ...
      if level == regular_level
        ...
        assert_include(user.user_levels.map(&:level_id), template_backed_level1.id)
        assert_include(user.user_levels.map(&:level_id), template_backed_level2.id)
      end
    end
