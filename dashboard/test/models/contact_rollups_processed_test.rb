...
class ContactRollupsProcessedTest < ActiveSupport::TestCase
  ...
  test 'extract_field' do
    ...
    test_cases = [
      ...
      },
      {
        ...
        input: [{'dashboard.another_table' => {field => [{'value' => 'WA'}]}}, table, field],
        expected_output: []
      },
      {
  ...
  test 'extract_roles' do
    ...
    tests = [
      ...
      {
        input: all_inputs,
        expected_output: {roles: 'CSD Teacher,CSF Teacher,Facilitator,Form Submitter,Parent,Teacher,Workshop Organizer'}
      },
    ]
  ...
  test 'extract_state' do
    ...
    tests = [
      ...
      {
        input: users_input.merge(schools_input),
        expected_output: {state: 'Illinois'}
      }
    ]
  ...
  test 'extract_forms_submitted' do
    contact_data = {'dashboard.census_submissions' => {}}
    expected_output = {forms_submitted: 'Census'}

    output = ContactRollupsProcessed.extract_forms_submitted contact_data
    assert_equal expected_output, output

    assert_equal({}, ContactRollupsProcessed.extract_forms_submitted({}))
  end
  ...
  test 'extract_form_roles' do
    ...
    expected_output = {form_roles: 'parent,teacher'}

    output = ContactRollupsProcessed.extract_form_roles contact_data
    assert_equal expected_output, output

    assert_equal({}, ContactRollupsProcessed.extract_form_roles({}))
  end
