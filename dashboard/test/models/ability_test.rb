...
class AbilityTest < ActiveSupport::TestCase
  ...
  test 'verified teacher can access javabuilder' do
    verified_teacher = create(:authorized_teacher)
    assert Ability.new(verified_teacher).can? :get_access_token, :javabuilder_session
  end

  test 'student of verified teacher in CSA section can access javabuilder' do
  ...
  test 'invited co-teacher cannot access javabuilder' do
    teacher = create(:authorized_teacher)
    csa_script = create(:csa_script, :in_single_unit_course)
    section = create(:section, user: teacher, script: csa_script)
    co_teacher = create(:teacher)
    ...
    refute Ability.new(co_teacher).can? :get_access_token, :javabuilder_session
  end

  test 'co-teacher of verified teacher in non-CSA section cannot access javabuilder' do
    teacher = create(:authorized_teacher)
    section = create(:section, user: teacher)
    co_teacher = create(:teacher)
    create(:section_instructor, section: section, instructor: co_teacher, status: :active)
  ...
  test 'co-teacher of unverified teacher in CSA section cannot access javabuilder' do
    teacher = create(:teacher)
    csa_script = create(:csa_script, :in_single_unit_course)
    section = create(:section, user: teacher, script: csa_script)
    co_teacher = create(:teacher)
  ...
  test 'student in section of unverified teacher cannot access javabuilder' do
    student = create(:student)
    section = create(:section)
    create(:follower, section: section, student_user: student)
    refute Ability.new(student).can? :get_access_token, :javabuilder_session
  end
