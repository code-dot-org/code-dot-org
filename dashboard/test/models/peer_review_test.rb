...
class PeerReviewTest < ActiveSupport::TestCase
  ...
  test 'assignments are logged to the audit trail' do
    ...
    assert_include(peer_review.audit_trail.lines.last, "ASSIGNED to user id #{user1.id}")

    peer_review.reviewer = user2
    peer_review.update! reviewer: user2
    assert_equal 2, peer_review.audit_trail.lines.count
    assert_include(peer_review.audit_trail.lines.last, "ASSIGNED to user id #{user2.id}")

    peer_review.update! reviewer: nil
    assert_equal 3, peer_review.audit_trail.lines.count
    assert_include(peer_review.audit_trail.lines.last, "UNASSIGNED")
  end
  ...
  test 'reviews are logged to the audit trail' do
    ...
    assert_include(peer_review.audit_trail.lines.last, "REVIEWED by user id #{user.id}")
  end
  ...
  test 'accepted reviews are logged to the audit trail' do
    ...
    assert_include(reviews.last.audit_trail.lines.last, "ACCEPTED by instructor #{@instructor.id}")
  end
  ...
  test 'rejected reviews are logged to the audit trail' do
    ...
    assert_equal 3, reviews.last.audit_trail.lines.count
    assert_include(reviews.last.audit_trail.lines.last, "REJECTED by instructor #{@instructor.id}")
  end
  ...
  test 'instructor reviews log to the audit trail' do
    ...
    assert_include(review.audit_trail.lines.last, "ACCEPTED by instructor #{@instructor.id} #{@instructor.name}")

    review.update!(reviewer: @instructor, status: 'rejected', from_instructor: true)
    assert_include(review.audit_trail.lines.last, "REJECTED by instructor #{@instructor.id} #{@instructor.name}")
  end
