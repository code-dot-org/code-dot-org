...
class ScriptLevelTest < ActiveSupport::TestCase
  ...
  test 'summarize with default route' do
    ...
    assert_match Regexp.new("^#{root_url(protocol: '//').chomp('/')}/s/bogus-script-[0-9]+/lessons/1/levels/1$"), summary[:url]
    ...
    assert_match Regexp.new("^#{root_url(protocol: '//').chomp('/')}/s/bogus-script-[0-9]+/lessons/1/levels/2$"), summary[:url]
  ...
  test 'summarize with custom route' do
    ...
    assert_equal "#{root_url(protocol: '//').chomp('/')}/hoc/1", summary[:url]  # Make sure we use the canonical /hoc/1 URL.
    assert_equal false, summary[:previous]
    assert_equal 1, summary[:position]
    assert_equal LEVEL_KIND.puzzle, summary[:kind]
    assert_equal 1, summary[:title]
