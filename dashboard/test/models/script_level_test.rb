...
class ScriptLevelTest < ActiveSupport::TestCase
  ...
  test 'summarize with default route' do
    ...
    assert_match Regexp.new("^#{root_url.chomp('/')}/s/bogus-script-[0-9]+/lesson/1/puzzle/1$"), summary[:url]
    ...
    assert_match Regexp.new("^#{root_url.chomp('/')}/s/bogus-script-[0-9]+/lesson/1/puzzle/2$"), summary[:url]
  ...
  test 'next_level_or_redirect_path_for_user returns to lesson extras for bonus levels' do
    script_level = create_script_level_with_ancestors({bonus: true})
    assert_equal "/s/#{script_level.script.name}/lesson/1/extras", script_level.next_level_or_redirect_path_for_user(nil)
  end
  ...
  test 'next_level_or_redirect_path_for_user returns to bubble choice activity page for BubbleChoice levels' do
    ...
    assert_equal "/s/#{script_level.script.name}/lesson/1/puzzle/1", script_level.next_level_or_redirect_path_for_user(nil)
  end
