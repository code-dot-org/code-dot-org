  ...
  test 'concise_summarize: section with a course assigned' do
    ...
    Timecop.freeze(Time.zone.now) do
      ...
      expected = {
        ...
        sectionInstructors: [{id: section.section_instructors[0].id, status: "active", instructorName: section.teacher.name, instructorEmail: section.teacher.email}],
        sync_enabled: nil,
  ...
  test 'concise_summarize: section with a coteacher' do
    ...
    Timecop.freeze(Time.zone.now) do
      ...
      expected = {
        ...
        is_assigned_csa: false,
        participant_type: 'student',
        sectionInstructors: [{id: primary_section_instructor_id, status: "active", instructorName: section.teacher.name, instructorEmail: section.teacher.email},
                             {id: coteacher_section_instructor.id, status: "invited", instructorName: nil, instructorEmail: coteacher_user.email}],
        sync_enabled: nil,
        ai_tutor_enabled: false,
      }
      # Compare created_at separately because the object's created_at microseconds
