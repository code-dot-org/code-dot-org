...
module Pd
  ...
  class JotFormBackedFormTest < ActiveSupport::TestCase
    ...
    test 'response_exists? raises error when missing form_id' do
      ...
      assert_include(e.message, 'Missing required attributes [:form_id]')
    end
    ...
    test 'response_exists? raises error when missing a unique attribute' do
      ...
      assert_include(e.message, 'Missing required attributes [:unique_key]')
    end
    ...
    test 'create_placeholder! raises error when missing submission_id' do
      ...
      assert_include(e.message, 'Expected submission_id')
    end
    ...
    test 'fill_placeholders collects errors and re-raises at the end' do
      ...
      assert_include(e.message, 'Errors filling Pd::DummyForm placeholders:')
      assert_include(e.message, "  Submission #{failed_submission_ids[0]}: Test error 1")
      assert_include(e.message, "  Submission #{failed_submission_ids[1]}: Test error 2")
    end
    ...
    test 'jotform sync collects errors and updates last processed submission id' do
      ...
      assert_include(e.message, "Error syncing JotForm submissions for forms [#{form_id}]. Errors:")
      assert_include(e.message, "Processing error 1")
    end
    ...
    test 'jotform sync stops processing when an entire batch fails' do
      ...
      Array.new(2) do |i|
        assert_include(e.message, "Process error #{i}")
      end
      assert_include(e.message, 'Submission all: Entire batch failed. Aborting')
    end
