...
class CourseTypesTests < ActiveSupport::TestCase
  ...
  test 'create unit_group with invalid instruction type raises error' do
    e = assert_raises do
      create(:unit_group, name: 'fake-instruction-type', instruction_type: "NOT REAL")
    end
    assert_equal "Validation failed: Instruction type must be teacher_led or self_paced", e.message
  end

  test 'create unit_group with invalid instructor audience raises error' do
    e = assert_raises do
      create(:unit_group, name: 'fake-instructor-audience', instructor_audience: "NOT REAL")
    end
    assert_equal "Validation failed: Instructor audience must be universal instructor, plc reviewer, facilitator, or teacher", e.message
  end

  test 'create unit_group with invalid participant audience raises error' do
    e = assert_raises do
      create(:unit_group, name: 'fake-participant-audience', participant_audience: "NOT REAL")
    end
  ...
  test 'create script with invalid instructor audience raises error' do
    e = assert_raises do
      create(:script,  name: 'fake-instructor-audience', instructor_audience: "NOT REAL")
    end
    assert_equal "Validation failed: Instructor audience must be universal instructor, plc reviewer, facilitator, or teacher", e.message
  end

  test 'create script with invalid participant audience raises error' do
    e = assert_raises do
      create(:script, name: 'fake-participant-audience', participant_audience: "NOT REAL")
    end
    assert_equal "Validation failed: Participant audience must be facilitator, teacher, or student", e.message
  end
  ...
  test 'create script with nil instructor audience raises error' do
    e = assert_raises do
      create(:script, name: 'no-instructor-audience', instructor_audience: nil)
    end
    assert_equal e.message, "Validation failed: Instructor audience is required"
  end

  test 'create script with nil instruction type raises error' do
    e = assert_raises do
