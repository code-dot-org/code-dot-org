...
module Pd::Application
  class TeacherApplicationTest < ActiveSupport::TestCase
    ...
    test 'columns_to_remove' do
      ['csp', 'csd'].each do |course|
        ...
        columns.keys.each do |k|
          columns[k].each {|c| refute_include(c.to_s, course)}
        end
      end
    end
    ...
    test 'csv_filtered_labels' do
      ...
      assert_include(csv_filtered_labels_csd[:teacher], :csd_which_grades)
      refute_include(csv_filtered_labels_csd[:teacher], :csp_which_grades)
      refute_include(csv_filtered_labels_csd[:teacher], :csa_which_grades)

      csv_filtered_labels_csp = TeacherApplication.csv_filtered_labels('csp')
      refute_include(csv_filtered_labels_csp[:teacher], :csd_which_grades)
      assert_include(csv_filtered_labels_csp[:teacher], :csp_which_grades)
      refute_include(csv_filtered_labels_csp[:teacher], :csa_which_grades)

      csv_filtered_labels_csa = TeacherApplication.csv_filtered_labels('csa')
      refute_include(csv_filtered_labels_csa[:teacher], :csd_which_grades)
      refute_include(csv_filtered_labels_csa[:teacher], :csp_which_grades)
      assert_include(csv_filtered_labels_csa[:teacher], :csa_which_grades)
    end
    ...
    test 'csv_header' do
      ...
      assert_include(csv_header_csd, csd_plan_offer_question)
      refute_include(csv_header_csd, csp_plan_offer_question)
      refute_include(csv_header_csd, csa_plan_offer_question)
      assert_equal 90, csv_header_csd.length

      csv_header_csp = CSV.parse(TeacherApplication.csv_header('csp'))[0]
      refute_include(csv_header_csp, csd_plan_offer_question)
      assert_include(csv_header_csp, csp_plan_offer_question)
      refute_include(csv_header_csp, csa_plan_offer_question)
      assert_equal 92, csv_header_csp.length

      csv_header_csa = CSV.parse(TeacherApplication.csv_header('csa'))[0]
      refute_include(csv_header_csa, csd_plan_offer_question)
      refute_include(csv_header_csd, csp_plan_offer_question)
      assert_include(csv_header_csa, csa_plan_offer_question)
    ...
    test 'memoized filtered_labels' do
      ...
      assert_include(filtered_labels_csd, :csd_which_grades)
      refute_include(filtered_labels_csd, :csp_which_grades)
      refute_include(filtered_labels_csd, :csa_which_grades)
      assert_equal ['csd'], TeacherApplication::FILTERED_LABELS.keys

      filtered_labels_csp = TeacherApplication.filtered_labels('csp')
      refute_include(filtered_labels_csp, :csd_which_grades)
      assert_include(filtered_labels_csp, :csp_which_grades)
      refute_include(filtered_labels_csp, :csa_which_grades)
      assert_equal ['csd', 'csp'], TeacherApplication::FILTERED_LABELS.keys

      filtered_labels_csa = TeacherApplication.filtered_labels('csa')
      refute_include(filtered_labels_csa, :csd_which_grades)
      refute_include(filtered_labels_csa, :csp_which_grades)
      assert_include(filtered_labels_csa, :csa_which_grades)
    ...
    test 'principal_approval_state' do
      ...
      assert_include(application.reload.principal_approval_state, 'Complete - Admin said Yes on')
    end
