...
module Pd::Application
  class Teacher2021ApplicationTest < ActiveSupport::TestCase
    ...
    test 'application guid is generated on create' do
      teacher_application = build :pd_teacher2021_application
    ...
    test 'existing guid is preserved' do
      ...
      teacher_application = create :pd_teacher2021_application, application_guid: guid
    ...
    test 'principal_greeting' do
      hash_with_principal_title = build :pd_teacher2021_application_hash
      hash_without_principal_title = build :pd_teacher2021_application_hash, principal_title: nil

      application_with_principal_title = build :pd_teacher2021_application, form_data_hash: hash_with_principal_title
      application_without_principal_title = build :pd_teacher2021_application, form_data_hash: hash_without_principal_title
    ...
    test 'meets criteria says an application meets critera when all YES_NO fields are marked yes' do
      teacher_application = build :pd_teacher2021_application, course: 'csd',
        response_scores: {
      ...
      teacher_application = build :pd_teacher2021_application, course: 'csp',
        response_scores: {
    ...
    test 'school_info_attr for specific school' do
      ...
      form_data_hash = build :pd_teacher2021_application_hash, school: school
      application = create :pd_teacher2021_application, form_data_hash: form_data_hash
      assert_equal({school_id: school.id}, application.school_info_attr)
    end

    test 'school_info_attr for custom school' do
      application = create :pd_teacher2021_application, form_data_hash: (
      build :pd_teacher2021_application_hash,
        :with_custom_school,
    ...
    test 'update_user_school_info with specific school overwrites user school info' do
      ...
      application = create :pd_teacher2021_application, user: user, form_data_hash: (
        build :pd_teacher2021_application_hash, school: application_school_info.school
      )
    ...
    test 'update_user_school_info with custom school does nothing when the user already a specific school' do
      ...
      application = create :pd_teacher2021_application, user: user, form_data_hash: (
        build :pd_teacher2021_application_hash, :with_custom_school
      )
    ...
    test 'get_first_selected_workshop multiple local workshops' do
      ...
      application = create :pd_teacher2021_application, form_data_hash: (
        build(:pd_teacher2021_application_hash, :with_multiple_workshops,
    ...
    test 'get_first_selected_workshop picks correct workshop even when multiple are on the same day' do
      ...
      application_2 = create :pd_teacher2021_application, form_data_hash: (
        build(:pd_teacher2021_application_hash, :with_multiple_workshops,
          regional_partner_workshop_ids: [workshop_1.id, workshop_2.id],
    ...
    test 'can_see_locked_status? is always false' do
      ...
      [teacher, g1_program_manager, g3_program_manager, workshop_admin].each do |user|
        refute Teacher2021Application.can_see_locked_status?(user)
      end
    end

    test 'columns_to_remove' do
      ['csp', 'csd'].each do |course|
        columns = Teacher2021Application.columns_to_remove(course)
    ...
    test 'csv_filtered_labels' do
      csv_filtered_labels_csd = Teacher2021Application.csv_filtered_labels('csd')
      ...
      csv_filtered_labels_csp = Teacher2021Application.csv_filtered_labels('csp')
    ...
    test 'cache prefetch' do
      ...
      form_data_hash = build :pd_teacher2021_application_hash, school: school
      application = create :pd_teacher2021_application, form_data_hash: form_data_hash, pd_workshop_id: workshop.id

      # Workshop, Session, Enrollment, School, SchoolDistrict
      assert_queries 5 do
        Teacher2021Application.prefetch_associated_models([application])
      end
    ...
    test 'memoized filtered_labels' do
      Teacher2021Application::FILTERED_LABELS.clear

      filtered_labels_csd = Teacher2021Application.filtered_labels('csd')
      assert filtered_labels_csd.include? :csd_which_grades
      refute filtered_labels_csd.include? :csp_which_grades
      assert_equal ['csd'], Teacher2021Application::FILTERED_LABELS.keys

      filtered_labels_csd = Teacher2021Application.filtered_labels('csp')
      refute filtered_labels_csd.include? :csd_which_grades
      assert filtered_labels_csd.include? :csp_which_grades
      assert_equal ['csd', 'csp'], Teacher2021Application::FILTERED_LABELS.keys
    end
    ...
    test 'test non course dynamically required fields' do
      application_hash = build :pd_teacher2021_application_hash,
        ...
        able_to_attend_multiple: [TEXT_FIELDS[:unable_to_attend_2021]]
    ...
    test 'test csd dynamically required fields' do
      application_hash = build :pd_teacher2021_application_hash_common,
        :csd,
    ...
    test 'Can create applications for the same user in 1819 and 2021' do
      teacher = create :teacher
      ...
      assert_creates Pd::Application::Teacher2021Application do
        create :pd_teacher2021_application, user: teacher
      end
    ...
    test 'autoscore with everything getting negative response for csd' do
      ...
      application = create :pd_teacher2021_application, regional_partner: nil, form_data_hash: application_hash
      application.auto_score!
    ...
    test 'require assigned workshop for registration-related statuses when emails sent by system' do
      statuses = Teacher2021Application::WORKSHOP_REQUIRED_STATUSES
    ...
    test 'do not require workshop for non-registration-related statuses' do
      statuses = Teacher2021Application.statuses - Teacher2021Application::WORKSHOP_REQUIRED_STATUSES
      partner = build :regional_partner, applications_decision_emails: RegionalPartner::SENT_BY_PARTNER
      application = create :pd_teacher2021_application, {
        regional_partner: partner
      }
    ...
    test 'associated models cache prefetch' do
      workshop = create :workshop
      application = create :pd_teacher2021_application, pd_workshop_id: workshop.id
