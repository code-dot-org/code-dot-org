...
module Pd::Application
  class Teacher1920ApplicationTest < ActiveSupport::TestCase
    ...
    test 'test allow_resending_principal_email' do
      # By default we can resend.
      application = create :pd_teacher1920_application
      assert application.allow_resending_principal_email

      # If we're no longer unreviewed/pending, we can't resend.
      application = create :pd_teacher1920_application
      application.update!(status: 'accepted_no_cost_registration')
      refute application.allow_resending_principal_email

      # If principal approval is not required, we can't resend.
      application = create :pd_teacher1920_application
      application.update!(principal_approval_not_required: true)
      refute application.allow_resending_principal_email

      # If we already have a principal response, we can't resend.
      application = create :pd_teacher1920_application
      create :pd_principal_approval1920_application, teacher_application: application
      refute application.allow_resending_principal_email

      # If we sent a principal email < 5 days ago, we can't resend.
      application = create :pd_teacher1920_application
      create :pd_application_email, application: application, email_type: 'principal_approval', sent_at: 1.day.ago
      refute application.allow_resending_principal_email

      # If we sent a principal email >= 5 days ago, we can resend.
      application = create :pd_teacher1920_application
