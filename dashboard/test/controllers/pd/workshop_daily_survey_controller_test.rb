...
module Pd
  class WorkshopDailySurveyControllerTest < ActionDispatch::IntegrationTest
    ...
    test 'daily workshop submit redirect creates placeholder and redirects to first facilitator form' do
      ...
      new_record = Pd::WorkshopDailySurvey.where(user: teacher).last
      assert new_record.placeholder?
      assert_equal workshop, new_record.pd_workshop
    ...
    test 'facilitator specific survey redirects to next facilitator when response exists' do
      ...
      teacher = enrollment.user
      create :pd_attendance, session: workshop.sessions[0], teacher: teacher, enrollment: enrollment
      create :pd_workshop_facilitator_daily_survey, pd_workshop: workshop, user: teacher,
        day: 1, form_id: FAKE_FACILITATOR_FORM_ID, pd_session: workshop.sessions[0], facilitator: workshop.facilitators[0]
    ...
    test 'last facilitator specific survey redirects to thanks when response exists' do
      ...
      create :pd_attendance, session: workshop.sessions[0], teacher: teacher, enrollment: enrollment
      create :pd_workshop_facilitator_daily_survey, pd_workshop: workshop, user: teacher,
        day: 1, form_id: FAKE_FACILITATOR_FORM_ID, pd_session: workshop.sessions[0], facilitator: workshop.facilitators[1]
    ...
    test 'facilitator specific survey with open session attendance displays embedded JotForm' do
      ...
      submit_redirect = facilitator_submit_redirect(day: 1, facilitator_index: 0, user: teacher, workshop: workshop)
    ...
    test 'facilitator specific submit redirect creates placeholder and redirects to next facilitator form' do
      ...
      assert_creates Pd::WorkshopFacilitatorDailySurvey do
        post '/pd/workshop_survey/facilitators/submit',
          params: facilitator_submit_redirect_params(day: 1, facilitator_index: 0, workshop: workshop, user: teacher).merge(
            submission_id: FAKE_SUBMISSION_ID,
          )
      ...
      new_record = Pd::WorkshopFacilitatorDailySurvey.where(user: teacher).last
      assert new_record.placeholder?
      assert_equal workshop, new_record.pd_workshop
      assert_equal 1, new_record.day
    ...
    test 'facilitator specific submit redirect creates placeholder and redirects to thanks for last facilitator' do
      ...
      assert_creates Pd::WorkshopFacilitatorDailySurvey do
        post '/pd/workshop_survey/facilitators/submit',
          params: facilitator_submit_redirect_params(day: 1, facilitator_index: 1, workshop: workshop, user: teacher).merge(
            submission_id: FAKE_SUBMISSION_ID,
          )
    ...
    test 'post workshop survey renders embedded JotForm' do
      ...
      enrollment = create :pd_enrollment, :from_user, workshop: workshop
      teacher = enrollment.user
      create :pd_attendance, session: workshop.sessions[4], teacher: teacher, enrollment: enrollment

      submit_redirect = general_submit_redirect(day: 5, enrollment_code: enrollment.code, user: teacher, workshop: workshop)
      assert_equal '/pd/workshop_survey/submit', URI.parse(submit_redirect).path
