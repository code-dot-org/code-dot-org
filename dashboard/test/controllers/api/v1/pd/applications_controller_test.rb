...
module Api::V1::Pd
  class ApplicationsControllerTest < ::ActionController::TestCase
    ...
    teardown do
      @csp_teacher_application.reload
    end
    ...
    test "quick view returns appropriate application type" do
      ...
      assert_equal TEACHER_APPLICATION_CLASS.csf.count, JSON.parse(@response.body).length
    end
    ...
    test 'regional partners cannot show other applications as workshop organizers' do
      sign_in @workshop_organizer
      get :show, params: {id: @csd_teacher_application_no_partner}
    ...
    test 'workshop admins see all applications in quick view' do
      ...
      expected_ids = [@csd_teacher_application_no_partner, @csd_teacher_application_with_partner].map(&:id).sort
    ...
    test 'regional partners cannot edit other applications as workshop organizers' do
      sign_in @workshop_organizer
      put :update, params: {id: @csd_teacher_application_no_partner, application: {status: 'accepted', notes: 'Notes'}}
      assert_response :forbidden
    end
    ...
    test 'update appends to the timestamp log if fit workshop is changed' do
      ...
      sign_in @program_manager
      @csp_teacher_application.update(status_timestamp_change_log: '[]')
      @csp_teacher_application.reload

      assert_equal [], @csp_teacher_application.sanitize_status_timestamp_change_log

      post :update, params: {id: @csp_teacher_application.id, application: {fit_workshop_id: fit_workshop.id, status: @csp_teacher_application.status}}
      @csp_teacher_application.reload

      assert_equal [
        {
          title: "Fit Workshop: #{@csp_teacher_application.fit_workshop_date_and_location}",
          changing_user_id: @program_manager.id,
          changing_user_name: @program_manager.name,
          time: Time.zone.now
        }
      ], @csp_teacher_application.sanitize_status_timestamp_change_log
    end
    ...
    test 'update appends to the timestamp log if summer workshop is changed' do
      ...
      post :update, params: {id: @csp_teacher_application.id, application: {pd_workshop_id: summer_workshop.id, status: @csp_teacher_application.status}}
      @csp_teacher_application.reload

      assert_equal [
        {
          title: "Summer Workshop: #{@csp_teacher_application.workshop_date_and_location}",
          changing_user_id: @program_manager.id,
          changing_user_name: @program_manager.name,
          time: Time.zone.now
        }
    ...
    test 'update does not append to the timestamp log if fit and summer workshop are not changed' do
      ...
      post :update, params: {id: @csp_teacher_application.id, application: {fit_workshop_id: fit_workshop.id, pd_workshop_id: summer_workshop.id, status: @csp_teacher_application.status}}
      @csp_teacher_application.reload
      ...
      assert_equal expected_log, @csp_teacher_application.sanitize_status_timestamp_change_log
    ...
    test 'update appends to timestamp log if workshop admin changes application from unlocked to locked' do
      sign_in @workshop_admin
      @csp_teacher_application.update(status_timestamp_change_log: '[]', locked_at: nil)
      @csp_teacher_application.reload

      assert_equal [], @csp_teacher_application.sanitize_status_timestamp_change_log
      refute @csp_teacher_application.locked?

      # Changing application from unlocked to locked
      post :update, params: {id: @csp_teacher_application.id, application: {status: @csp_teacher_application.status, locked: true}}
      @csp_teacher_application.reload
    ...
    test 'update appends to timestamp log if workshop admin changes application from locked to unlocked' do
      sign_in @workshop_admin
      @csp_teacher_application.update(status_timestamp_change_log: '[]', locked_at: Time.zone.now)
      @csp_teacher_application.reload

      assert_equal [], @csp_teacher_application.sanitize_status_timestamp_change_log
      assert @csp_teacher_application.locked?

      # Changing application from locked to unlocked
      post :update, params: {id: @csp_teacher_application.id, application: {status: @csp_teacher_application.status, locked: false}}
      @csp_teacher_application.reload
    ...
    test 'do not re-lock locked applications on update' do
      sign_in @workshop_admin
      @csp_teacher_application.update(status: 'declined', locked_at: Time.zone.now)
      @csp_teacher_application.reload
      assert @csp_teacher_application.locked?

      Pd::Application::TeacherApplication.any_instance.expects(:lock!).never

      # edit locked application
      post :update, params: {id: @csp_teacher_application.id, application: {locked: true}}
    end

    test 'do not re-unlock unlocked applications on update' do
      sign_in @workshop_admin
      @csp_teacher_application.update(status: 'declined')
      @csp_teacher_application.reload
      refute @csp_teacher_application.locked?

      Pd::Application::TeacherApplication.any_instance.expects(:unlock!).never

      # edit locked application
      post :update, params: {id: @csp_teacher_application.id, application: {locked: false}}
    end

    test 'workshop admins can lock and unlock applications' do
      sign_in @workshop_admin
      put :update, params: {id: @csd_teacher_application_no_partner, application: {status: 'accepted', locked: 'true'}}
      assert_response :success
      data = JSON.parse(response.body)
      assert data['locked']
    end
    ...
    test 'Regional partners cannot lock and unlock applications as workshop organizers' do
      sign_in @workshop_organizer
      put :update, params: {id: @csd_teacher_application_with_partner.id, application: {status: 'accepted', locked: 'true'}}
      assert_response :success
      data = JSON.parse(response.body)
      refute data['locked']
    end
    ...
    test 'cohort view returns teacher applications of correct statuses' do
      ...
      sign_in @workshop_admin
      get :cohort_view, params: {role: 'csd_teachers', regional_partner_value: 'none'}
      assert_response :success
