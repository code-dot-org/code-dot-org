    ...
    expected_response =
      [
       {'student' => {'id' => @student_1.id, 'name' => @student_1.name},
        'stage' => "Stage 1: translation missing: en-us.data.script.name.#{script.name}.Bogus Stage 1",
        'puzzle' => 1,
        'question' => 'Text Match 1',
        'response' => 'Here is the answer',
        'url' => "http://test.host/s/#{script.name}/stage/1/puzzle/1?section_id=#{@section.id}&user_id=#{@student_1.id}"},
       {'student' => {'id' => @student_1.id, 'name' => @student_1.name},
        'stage' => "Stage 2: translation missing: en-us.data.script.name.#{script.name}.Bogus Stage 2",
        'puzzle' => 1,
