...
class PasswordsControllerTest < ActionController::TestCase
  ...
  test "create with valid email includes link for admin" do
    ...
    assert_include(flash[:notice], 'Reset password link sent to user. You may also send this link directly:')
    assert_include(flash[:notice], 'http://test-studio.code.org/users/password/edit?reset_password_token=')
  end
  ...
  test "create with multiple associated accounts includes link for admin" do
    ...
    assert_include(flash[:notice], 'Reset password link sent to user. You may also send the link directly:')
    assert_include(flash[:notice], "#{user1.username}: <a href='http://test-studio.code.org/users/password/edit?reset_password_token=")
    assert_include(flash[:notice], "#{user2.username}: <a href='http://test-studio.code.org/users/password/edit?reset_password_token=")
  end
