...
class Plc::EnrollmentEvaluationsControllerTest < ActionController::TestCase
  setup do
    ...
    @module1 = create(:plc_learning_module, name: 'Getting thrown off cliffs', plc_course_unit: @course_unit, module_type: Plc::LearningModule::PRACTICE_MODULE)
    @module2 = create(:plc_learning_module, name: 'Advanced Ornithology', plc_course_unit: @course_unit, module_type: Plc::LearningModule::CONTENT_MODULE)
    @module3 = create(:plc_learning_module, name: 'Answering questions honestly', plc_course_unit: @course_unit, module_type: Plc::LearningModule::PRACTICE_MODULE)
    @module4 = create(:plc_learning_module, name: 'Admitting ignorance', plc_course_unit: @course_unit, module_type: Plc::LearningModule::CONTENT_MODULE)
  ...
  test "submit evaluation enrolls user in appropriate modules" do
    ...
    do_expected_answers_yield_expected_module_enrollments(
        [@answer1_3, @answer2_1, @answer3_2], [@module3])
    ...
    do_expected_answers_yield_expected_module_enrollments(
        [@answer1_4, @answer2_1, @answer5_1], [@module2, @module3])

    #Sir Edgecase of Edgecaseville should be enrolled in no modules
    do_expected_answers_yield_expected_module_enrollments(
        [@answer1_5, @answer3_3, @answer5_2], [])

    # Also try some mixing of nil answers
    do_expected_answers_yield_expected_module_enrollments(
        [@answer1_1, @answer3_3, @answer5_2], [@module3])
  end
  ...
  def do_expected_answers_yield_expected_module_enrollments(answers, expected_module_enrollments)
    post :submit_evaluation, unit_assignment_id: @unit_assignment.id, answer_module_list: answers.map(&:plc_learning_module_id)
