...
class MakerControllerTest < ActionController::TestCase
  ...
  test "shows devices-2022 when there are no relevant assignments" do
    assert_empty @student.scripts
    assert_empty @student.section_courses
    assert_nil @student.user_script_with_most_recent_progress

    assert_equal @devices_2022, MakerController.maker_script(@student)
  end

  test "shows devices-2022 if devices-2022 is assigned" do
    create :user_script, user: @student, script: @devices_2022, assigned_at: Time.now
    assert_includes @student.scripts, @devices_2022
