...
- Raw user ids must not be sent to Sentry. `Observability::Sentry.set_user_token` takes an opaque token; callers get it from `User::LogToken` in Dashboard, not from this engine. Don't move the lookup in here, and don't add a reversal path — reversal is audited and belongs in Rails.
