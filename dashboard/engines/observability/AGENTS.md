...
- Raw user ids must not be sent to Sentry. `Observability::Sentry.set_user_token` takes an opaque token; callers derive it with `Cdo::UserLogToken` (in `lib/cdo/`, not this engine, because every destination shares one key). Don't move the derivation in here, and don't add a reversal path — reversal is audited and belongs in Rails.
