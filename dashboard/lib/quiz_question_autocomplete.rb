class QuizQuestionAutocomplete < AutocompleteHelper
  ...
  # A question can be attached (via QuizLevelQuestion) to more than one Quiz,
  # and a Quiz's own script_levels can span more than one Unit, so the joins
  # below can multiply rows - .distinct collapses those back down to one row
  # per matching question, same as .limit above still expects.
