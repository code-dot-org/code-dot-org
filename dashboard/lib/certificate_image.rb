...
class CertificateImage
  ...
  def self.create_course_certificate_image(name, course = nil, course_title = nil)
    ...
    template_file = CertificateMetadata.certificate_template_for(course)

    path = dashboard_dir('public', 'blockly', 'media', 'certificates', template_file)
    if CertificateMetadata.prefilled_title_course?(course)
  ...
  # Metadata methods relocated to CertificateMetadata (no rmagick require);
  # delegated here so existing call sites are unaffected.
  def self.hoc_course?(course_name)
    CertificateMetadata.hoc_course?(course_name)
  end

  def self.course_type(course_name)
    CertificateMetadata.course_type(course_name)
  end

  def self.prefilled_title_course?(course)
    CertificateMetadata.prefilled_title_course?(course)
  end

  def self.certificate_template_for(course)
    CertificateMetadata.certificate_template_for(course)
  end
end
