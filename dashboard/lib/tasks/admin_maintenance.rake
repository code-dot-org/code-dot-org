...
namespace :admin do
  ...
  timed_task_with_logging remove_inactive_admins: :environment do
    ...
    # Helper scope to find inactive admin users without polluting the global namespace.
    inactive_admins_query = lambda do |cutoff|
      User.where(admin: true).
        where('current_sign_in_at < ? OR current_sign_in_at IS NULL', cutoff)
    end

    # Get inactive admins using reporting database (read replica) and materialize results
    ...
    if dry_run
      ...
      inactive_admins.each do |user|
        ...
        # Check if user is a teacher (admins should be teachers)
        unless user.teacher?
        ...
        # Update on primary database - this will trigger automatic security logging
