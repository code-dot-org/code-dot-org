...
namespace :seed do
  ...
  task dsls: :environment do
    DSLDefined.transaction do
      levels_by_hash = DSLDefined.all.index_by(&:md5)
      dsls_yml = File.expand_path('config/locales/dsls.en.yml')
      i18n_strings = File.exist?(dsls_yml) ? YAML.load_file(dsls_yml) : {}
      ...
      DSLS_GLOB.each do |filename|
        ...
        begin
          unless levels_by_hash[Digest::MD5.hexdigest(File.read(filename))]
