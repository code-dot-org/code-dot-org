...
namespace :seed do
  ...
  timed_task :import_pegasus_data do
    params = {db: DASHBOARD_DB, table_prefix: "google_sheets_shared_"}
    files_to_import = %w[data/cdo-languages.csv data/cdo-donors.csv]
    files_to_import.each {|file_to_import| CsvToSqlTable.new(pegasus_dir(file_to_import), params).import}
  end
