module Pd::SurveyPipeline
  module SurveyRollupDecorator
    def decorate_facilitator_rollup(data)
      ...
      report[:related_workshops][facilitator_name] = related_workshop_ids
      ...
      report[:facilitator_response_counts] = get_submission_counts submissions, data[:facilitator_id], data[:current_workshop_id]

      # Convert question names to the format that the view expects.
      # There is a different in the way we name sub-question between the controller and the (legacy) view.
      # In the controller, sub-question name has format <original_question>_<hash_of_sub_question_text>.
      # In the view, it expects question name in format <original_question>_<index_of_sub_question>.
      # Conversion examples:
      ...
      q_name_replacements = get_question_name_replacements data.get_values(:parsed_questions, :question_categories)
      ...
      # Replace question names if needed.
      # facilitator_averages: {facilitator_name => {question_name => {:this_workshop, :all_my_workshops => score}}}
      ...
      # facilitator_averages: {facilitator_name => {question_category => {:this_workshop, :all_my_workshops => score}}}
      report[:facilitator_averages][facilitator_name].merge! get_category_averages(data[:question_categories], question_averages)
    ...
    def get_category_averages(question_categories, question_averages)
      result = {}
    ...
    def get_question_averages(summaries, current_workshop_id)
      result = {}
    ...
    def map_question_name_to_text(parsed_questions)
      ...
      parsed_questions&.each_pair do |_, form_questions|
        form_questions.each_pair do |_, q_content|
          ...
          q_names_to_texts[q_content[:name]] = q_content[:text] unless q_names_to_texts.key?(q_content[:name])
        end
      end
    ...
    def get_question_names(parsed_questions)
      q_names = Set[]
    ...
    def get_question_name_replacements(parsed_questions, question_categories)
    ...
    def get_submission_counts(submissions, facilitator_id, current_workshop_id)
      {this_workshop: {}, all_my_workshops: {}}.tap do |result|
        next unless submissions.present?

        submission_type = submissions.first.is_a?(Pd::WorkshopDailySurvey) ? 'Workshop surveys' : 'Facilitator surveys'
