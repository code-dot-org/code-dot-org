...
class Policies::Ai
  ...
  def self.ai_rubrics_enabled_for_script_level?(user, script_level)
    ...
    # Get sections that contain the given script_level
    script_sections = sections.select {|s| s.script.try(:id) == script_level.script.id}

    # It is enabled if any of the sections have a teacher that has not opted-out
    script_sections.any? do |section|
