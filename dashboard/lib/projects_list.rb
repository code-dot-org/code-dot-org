module ProjectsList
  ...
  def self.fetch_section_projects(section)
  ...
  # e.g. '/p/applab' -> 'applab'
  private_class_method def self.project_type(level)
    level && level.split('/')[2]
  end
  ...
  # single project.
  private_class_method def self.get_project_row_data(student, project, channel_id)
