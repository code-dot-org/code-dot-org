...
"backToPreviousLevel":function(d){return "前（まえ）のレベルに戻（もど）る。"},
"blocklyMessage":function(d){return "ブロックリー"},
...
"completedWithoutRecommendedBlock":function(d){return "おめでとうございます！あなたはパズル"+common_locale.v(d,"puzzleNumber")+" を完了（かんりょう）しました。(他（た）のブロックを使（つか）ってより良（よ）いコードを書（か）くことも出来（でき）ます。)"},
...
"recommendedBlockContextualHintTitle":function(d){return "パズルを解決するためにこのブロックを使用してください。"},
...
"tooFewBlocksMsg":function(d){return "すべての必要（ひつよう）な種類（しゅるい）のブロックを使用（しよう）していますが、これらの種類（しゅるい）のブロックも使（つか）ってみてこのパズルを完成（かんせい）させてください。"},
...
"tryAgain":function(d){return "やり直（なお）す"},
