...
/*********BIG NOTE: THIS VERSION IS DEFFERENT FROM pixelationSCript4.js
 *    It assumes that only first two bytes, for w & h are formateed.
 */ 
function formatBits(bitString, chunkSize, chunksPerLine){
 ...
 var isHex = "hex" == document.querySelector('input[name="binHex"]:checked').value;
	...
	if(isHex){
	 ...
	 //remove first 16 bits from justBits
	 justBits = justBits.substring(16);
