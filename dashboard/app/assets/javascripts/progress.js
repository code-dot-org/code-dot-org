 ...
 * @return {string}
 */
dashboard.progress.activityCssClass = function (result) {
  if (!result) {
    return 'not_tried';
  }
  if (result >= 1000) {
 ...
 */
dashboard.progress.mergedActivityCssClass = function (a, b) {
  return dashboard.progress.activityCssClass(dashboard.clientState.mergeActivityResult(a, b));
}

dashboard.progress.populateProgress = function (scriptName) {
  ...
  $.ajax('/api/user_progress/' + scriptName).done(function (data) {
    ...
    Object.keys(serverProgress).forEach(function (levelId) {
      if (serverProgress[levelId].result !== clientProgress[levelId]) {
        var status = dashboard.progress.mergedActivityCssClass(clientProgress[levelId], serverProgress[levelId].result);
