class OpsMailer < ActionMailer::Base
  ...
  def district_contact_added_teachers(district_contact, cohort, added_teachers)
    @district_contact = district_contact
    @cohort = cohort
    @added_teachers = added_teachers

    subject = "[ops notification] #{district_contact.ops_first_name} #{district_contact.ops_last_name} added #{added_teachers.count} teachers to #{cohort.name}"
