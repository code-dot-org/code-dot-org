...
class ScriptsController < ApplicationController
  ...
  def show
    if Experiment.enabled?(user: current_user, experiment_name: 'teacher-local-nav-v2') && current_user.sections_instructed.any? {|s| s.script_id == @script.id || s.unit_group.default_units.any? {|u| u.id == @script.id}}
