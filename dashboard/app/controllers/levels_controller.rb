...
class LevelsController < ApplicationController
  ...
  def update
    if level_params[:name] && @level.name != level_params[:name]
      if @level.name.downcase == level_params[:name].downcase
        ...
        return
      elsif @level.name == level_params[:name].strip
        # do not allow leading or trailing spaces in the level name
        @level.errors.add(:name, 'Cannot change only the capitalization of the level name (it confuses git on OSX)')
        render json: @level.errors, status: :unprocessable_entity
        return
      end
