class Api::V1::Pd::WorkshopsController < ApplicationController
  ...
  def update
    ...
    if @workshop.update(new_workshop_params)
      ...
      # Only pass in the session info if it's changed.
      ...
      notify(general_detail_changes, sessions_have_changed, pre_update_session_info_for_email, post_update_session_info_for_email) if (!general_detail_changes.empty? || sessions_have_changed) && should_notify?
      render json: @workshop, serializer: Api::V1::Pd::WorkshopSerializer
    else
