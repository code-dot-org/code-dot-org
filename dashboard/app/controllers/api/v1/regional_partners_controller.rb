class Api::V1::RegionalPartnersController < ApplicationController
  ...
  def index
    regional_partners = (csf_facilitator?(current_user) || current_user.workshop_admin?) ? RegionalPartner.all : current_user.regional_partners
  ...
  def csf_facilitator?(user)
    Pd::CourseFacilitator.where(facilitator: current_user).pluck(:course).include? Pd::Workshop::COURSE_CSF
  end
end
