# A controller for proxying image and sound content so that it can be
# served without mixed-content warnings. The proxied content is
# intentionally given unlimited cache lifetime so users should ensure
# that media gets a distinct URL each time it changes. We assume the
# proxied content will be cached via a CDN to reduce the scale impact
# on Rails servers.
# 
# To reduce the likelihood of abuse, the code only proxies content
# with an allowed whitelist of media types. We will need to monitor
# usage to detect abuse and potentially add other abuse prevention
# measures (e.g. a signature based on a secret.)
