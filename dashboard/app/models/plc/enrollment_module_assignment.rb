...
class Plc::EnrollmentModuleAssignment < ActiveRecord::Base
  ...
  def self.stages_based_status(stages, user, script)
    ...
    levels_tracked = all_levels.reject {|level| [External, ExternalLink].include?(level.class) || level.try(:peer_reviewable).try(:to_bool) }
    ...
    peer_reviewable_levels = all_levels.select {|level| level.try(:peer_reviewable).try(:to_bool)}
