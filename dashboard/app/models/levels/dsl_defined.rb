...
class DSLDefined < Level
  ...
  def self.rewrite_i18n_file(i18n)
    ...
    data_file = Rails.root.join("config/locales/dsls.en.json")
    i18n_strings = File.exist?(data_file) ? JSON.parse(File.read(data_file)) : {}
    # Overwrite existing string keys with their new values
    i18n_strings.deep_merge! i18n
    File.write(data_file, JSON.pretty_generate(i18n_strings))
  end
