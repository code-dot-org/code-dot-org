...
class LessonGroup < ApplicationRecord
  ...
  def copy_to_unit(destination_script, new_level_suffix = nil, destination_professional_learning_course = nil)
    ...
    raise 'Destination script and lesson group must be in a course version' if destination_script.get_course_version.nil? && destination_professional_learning_course.nil?
    ...
    lessons.each do |original_lesson|
      copied_lesson = original_lesson.copy_to_unit(destination_script, new_level_suffix, destination_professional_learning_course)
