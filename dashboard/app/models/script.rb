...
class Script < ActiveRecord::Base
  ...
  def generate_plc_objects
    if professional_learning_course?
      ...
      raise 'For Professional Learning Scripts, either all stages must have flex categories that correspond to learning module types, or none of them should have categories' unless learning_module_types.uniq == [nil] || Set.new(learning_module_types) <= Set.new(Plc::LearningModule::MODULE_TYPES)

      raise 'For Professional Learning Scripts, all stages need to be grouped together in order' unless learning_module_types.compact.empty? || learning_module_types.chunk { |x| x }.map(&:first) == learning_module_types.uniq
