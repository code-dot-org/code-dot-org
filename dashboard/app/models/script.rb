...
class Script < ApplicationRecord
  ...
  # A lock to synchronize writes to the scripts.en.yml file in the levelbuilder
  # environment. Note that this same approach would not work in production,
  # because there are multiple frontend servers in production. It also won't
  # prevent conflicts between rails server, cronjobs, and manual invocations of
  # rails console.
  def self.script_yml_mutex
    @@script_yml_mutex = Mutex.new
  end
  ...
  def self.merge_and_write_i18n(lessons_i18n, unit_name = '', metadata_i18n = {})
    script_yml_mutex.synchronize do
      units_yml = File.expand_path("#{Rails.root}/config/locales/scripts.en.yml")
      i18n = File.exist?(units_yml) ? YAML.load_file(units_yml) : {}

      puts "loaded unit #{unit_name}"

      sleep 5

      puts "writing unit #{unit_name}"
