...
class Script < ActiveRecord::Base
  ...
  def copy_and_write_i18n(new_name)
    scripts_file = File.expand_path('config/locales/scripts.en.json')
    i18n = File.exist?(scripts_file) ? JSON.parse(File.read(scripts_file)) : {}
    i18n.deep_merge!(summarize_i18n_for_copy(new_name)) {|_, old, _| old}
    File.write(scripts_file, JSON.pretty_generate(i18n))
  end
  ...
  # This method updates scripts.en.json with i18n data from the scripts.
  ...
  def self.merge_and_write_i18n(stages_i18n, script_name = '', metadata_i18n = {})
    ...
    File.write(scripts_file, JSON.pretty_generate(updated_i18n))
  end
