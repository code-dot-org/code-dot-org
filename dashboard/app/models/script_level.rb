  ...
  def get_example_solutions(level, current_user, section_id = nil, unit_group_unit: nil)
    ...
    if level.try(:exemplar_sources).present? && current_user&.verified_instructor?
      ...
      level_example_links = level.examples.map do |example|
        ...
        case level
        when Dancelab
          dance_project_view_projects_url(channel_id: example)
        when Poetry
          send(:"#{level.standalone_app_name}_project_view_projects_url", channel_id: example)
        when GamelabJr
          send(:"#{level.standalone_app_name_or_default}_project_view_projects_url", channel_id: example)
        when Artist
          artist_type = (level.skin == 'elsa' || level.skin == 'anna') ? 'frozen' : 'artist'
          send(:"#{artist_type}_project_view_projects_url", channel_id: example)
        when Studio # playlab
          playlab_project_view_projects_url(channel_id: example)
        else
          send(:"#{level.game.app}_project_view_projects_url", channel_id: example)
        end
      end
