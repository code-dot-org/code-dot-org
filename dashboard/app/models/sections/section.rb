...
class Section < ActiveRecord::Base
  ...
  def has_sufficient_discount_code_progress?
    ...
    return false unless csd2 && csd3

    csd2_programming_level_ids = csd2.levels.select {|level| level.is_a?(Gamelab)}.map(&:id)
    csd3_programming_level_ids = csd3.levels.select {|level| level.is_a?(Weblab)}.map(&:id)
