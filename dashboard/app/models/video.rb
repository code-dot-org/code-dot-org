...
class Video < ActiveRecord::Base
  ...
  def self.check_i18n_names
    ...
    unless missing_keys.empty?
      raise "Missing strings for video.name.#{missing_keys} in config/locales/data.en.json, please add"
    end
  end
  ...
  def self.merge_and_write_i18n(videos_i18n)
    data_file = File.expand_path('config/locales/data.en.json')
    i18n = File.exist?(data_file) ? JSON.parse(File.read(data_file)) : {}

    updated_i18n = i18n.deep_merge({'en' => {'data' => {'video' => {'name' => videos_i18n}}}})
    File.write(data_file, JSON.pretty_generate(updated_i18n))
  end
