...
class Video < ApplicationRecord
  ...
  def self.setup(dashboard_root = '.')
    videos = CSV.read(curriculum_dir("dashboard/config/videos.csv"), headers: true).map.with_index(1) do |row, id|
  ...
  def self.merge_and_write_attributes(key, youtube_code, download, locale, videos_csv_path = 'dashboard/config/videos.csv')
    csv_path = File.expand_path(curriculum_dir(videos_csv_path))
