module LangfuseHelper
  ...
  # Sends a root observation + generation to the TA Langfuse project for a lesson insight call.
  ...
  # Sends a root observation + generation to the TA Langfuse project for a lesson feedback call.
  ...
  def self.trace_student_snapshot_call(trace_name:, model:, teacher_id:, lesson_id:, lesson_name:, unit_id:, unit_name:, section_id:, student_id:, variables:, output:, usage:, start_time:, end_time:, prompt_name: nil, prompt_version: nil)
    ta_client.export_generation_trace(
      ...
      output: output,
      # Legacy ingestion derived the total itself; usage_details is passed
      # through as sent, so send the key the existing cost dashboards read.
      usage: {
