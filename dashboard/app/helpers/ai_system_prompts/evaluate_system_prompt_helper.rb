module AiSystemPrompts::EvaluateSystemPromptHelper
  def self.get_system_prompt(level, evaluation_type)
    ...
    structure_with_skills = <<~TEXT
      ...
      skillEvaluations should be an array of objects based on #{skill_evaluations} with the following structure: {skillId: skillId, skillKey: skillKey, evaluationCriteria: evaluationCriteria, aiEvaluation: Respond with "Great", "Ok", "Needs revision" or "not attempted", aiReasoning: "reasoning for specific evaluation"} generated by iterating through the skills.
    TEXT
