    ...
    facilitators.each do |facilitator|
      ...
      QUESTIONS_FOR_FACILITATOR_AVERAGES.each do |category, questions|
        ...
        [:this_workshop, :all_my_workshops].each do |column|
          average = (facilitator_averages[facilitator].slice(*(questions.map {|question| question[:primary_id]})).values.map {|x| x[column]}.reject(&:nil?).reduce(:+) || 0) / questions.size.to_f
