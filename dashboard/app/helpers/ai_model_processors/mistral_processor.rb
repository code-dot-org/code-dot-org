class AiModelProcessors::MistralProcessor
  ...
  def format_model_inputs(instructions, new_message, stored_messages)
    ...
    inputs = (AiModelProcessors::MistralProcessor::SENTENCE_BEGIN_TOKEN + wrap_as_instructions(instructions))
    # Add user and assistant messages including most recent user message.
    all_messages = [*stored_messages, new_message]
    all_messages.each do |msg|
      if msg[:role] == AiModelProcessors::MistralProcessor::USER
        inputs << wrap_as_instructions(msg[:chatMessageText])
      elsif msg[:role] == AiModelProcessors::MistralProcessor::ASSISTANT
        ...
        inputs << (msg[:chatMessageText] + AiModelProcessors::MistralProcessor::SENTENCE_END_TOKEN)
      end
    end
    inputs
  end
  ...
  def wrap_as_instructions(message)
    return AiModelProcessors::MistralProcessor::INSTRUCTIONS_BEGIN_TOKEN + message + AiModelProcessors::MistralProcessor::INSTRUCTIONS_END_TOKEN unless message.empty?
    message
  end
