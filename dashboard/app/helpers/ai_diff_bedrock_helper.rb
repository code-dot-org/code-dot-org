module AiDiffBedrockHelper
  ...
  def self.request_bedrock_rag_chat(input, prompt, lesson_number, unit_num, course_name, session_id, section_contexts, labs = [], artifact_type = "AidiffExitTicket")
    ...
    begin
      ...
      puts "JSON MATCH"
      puts json_post_process(response.output.text.dup)
      if artifact_type && json_post_process(response.output.text.dup).nil?
        puts "raising"
      ...
      if (attempts += 1) < 2
        ...
        return format_rag_response(response, true)
      end
    end
  ...
  def self.json_post_process(text)
    match = text.match(/(\{(?:.|\n)*\})/)
    if match
      json_string = match[0]
      return json_string
  ...
  def self.format_rag_response(response, invalid = false)
    ...
    puts json_text ? json_text : "MATCH IS NIL"

    valid = JSON::Validator.validate(AidiffPromptHelper::EXIT_TICKET_SCHEMA, json_text)
    if json_text
      text = json_text
    end
    ...
    {
      content: format("%{text}\n\n%{valid}", text: text, valid: valid ? "THIS IS VALID" : "THIS JSON IS INVALID"),
