...
# Warn when a commit touches .gitattributes. Changing Git LFS rules does not
# rewrite already-committed blobs, so the committer should run `git add
# --renormalize .` to reconcile files with their new attributes and include any
# conversions in the same commit. CI (bin/drone/check-lfs-pointers) is the
# enforcing backstop; this is just a non-blocking hint to avoid CI failures.
def main
